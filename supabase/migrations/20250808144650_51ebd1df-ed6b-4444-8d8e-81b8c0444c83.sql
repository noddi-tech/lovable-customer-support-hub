-- Recreate get_conversations to ensure snooze_until is returned
CREATE OR REPLACE FUNCTION public.get_conversations()
 RETURNS TABLE(id uuid, subject text, status text, priority text, is_read boolean, is_archived boolean, channel text, updated_at timestamp with time zone, received_at timestamp with time zone, customer jsonb, assigned_to jsonb, snooze_until timestamp with time zone)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
