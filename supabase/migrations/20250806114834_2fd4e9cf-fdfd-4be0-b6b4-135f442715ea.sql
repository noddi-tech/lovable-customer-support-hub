-- Fix the trigger function to work without auth context
...
BEGIN
  ...
  IF NEW.is_internal = true AND OLD IS NOT NULL THEN
    ...
    -- Get the current user ID from the message sender_id (since auth.uid() might not be available in trigger context)
