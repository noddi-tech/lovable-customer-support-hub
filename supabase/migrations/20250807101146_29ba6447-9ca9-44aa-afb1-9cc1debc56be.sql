...
WITH CHECK (
  ...
  -- Prevent updating role fields by checking they haven't changed
  (role IS NOT DISTINCT FROM (SELECT role FROM public.profiles WHERE user_id = auth.uid())) AND
  (primary_role IS NOT DISTINCT FROM (SELECT primary_role FROM public.profiles WHERE user_id = auth.uid()))
);
