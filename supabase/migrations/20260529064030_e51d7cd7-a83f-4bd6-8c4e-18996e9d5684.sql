...
BEGIN
  ...
  UPDATE public.applications SET score_status = 'scoring' WHERE id = NEW.id;
  RETURN NEW;
END;
$function$;
...
BEGIN
  ...
  LOOP
    IF v_app.scoring_enabled IS FALSE THEN CONTINUE; END IF;
    SELECT public.position_has_resolvable_rubric(v_app.position_id) INTO v_has_rubric;
    IF NOT v_has_rubric THEN CONTINUE; END IF;
    SELECT EXISTS(SELECT 1 FROM public.application_scoring_queue WHERE application_id = v_app.id AND status IN ('pending','processing')) INTO v_already_pending;
    IF v_already_pending THEN CONTINUE; END IF;

    INSERT INTO public.application_scoring_queue
      (application_id, organization_id, trigger_reason, triggered_at_stage_id)
    VALUES (v_app.id, v_app.organization_id, 'data_change', v_app.current_stage_id);

    UPDATE public.applications SET score_status = 'scoring' WHERE id = v_app.id;
  END LOOP;
  RETURN NEW;
END;
$function$;
