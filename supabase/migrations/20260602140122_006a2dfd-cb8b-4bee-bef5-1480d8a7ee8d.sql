...
BEGIN
  ...
  -- (g) Applicant files — HARD DELETE (no deleted_at column, storage_path NOT NULL).
  --     Collect storage paths first so the caller can purge the bucket.
  SELECT COALESCE(array_agg(storage_path), ARRAY[]::text[])
    INTO v_file_paths
    FROM public.applicant_files
    WHERE applicant_id = p_applicant_id;

  DELETE FROM public.applicant_files WHERE applicant_id = p_applicant_id;
  GET DIAGNOSTICS v_count = ROW_COUNT;
