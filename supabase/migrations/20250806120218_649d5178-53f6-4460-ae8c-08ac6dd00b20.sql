-- Comprehensive fix for note edit notifications
...
DROP FUNCTION IF EXISTS public.set_sender_id_on_update() CASCADE;
DROP FUNCTION IF EXISTS public.send_note_edit_notification() CASCADE;

-- Create improved function to set sender_id with logging
...
AS $function$
BEGIN
  -- Always set sender_id to current user for internal notes being updated
  IF NEW.is_internal = true THEN
    NEW.sender_id := auth.uid();
    -- Log for debugging
    RAISE LOG 'Setting sender_id to % for message %', auth.uid(), NEW.id;
  END IF;
...
-- Create comprehensive notification function with detailed logging
...
BEGIN
  ...
  IF NEW.is_internal = true AND OLD IS NOT NULL THEN
    ...
    RAISE LOG 'Processing note edit notification for message: %', NEW.id;
    ...
    RAISE LOG 'Deleted old notifications for message: %', NEW.id;
    
    -- Check if assignment changed
    IF OLD.assigned_to_id IS DISTINCT FROM NEW.assigned_to_id THEN
      
      RAISE LOG 'Assignment changed from % to %', OLD.assigned_to_id, NEW.assigned_to_id;
    ...
    ELSIF OLD.content IS DISTINCT FROM NEW.content THEN
      ...
      RAISE LOG 'Content changed from "%" to "%"', LEFT(OLD.content, 50), LEFT(NEW.content, 50);
