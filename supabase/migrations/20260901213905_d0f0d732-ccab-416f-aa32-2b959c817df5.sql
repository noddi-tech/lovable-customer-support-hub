...
BEGIN
  ...
  WITH threaded AS (
    ...
    c.snooze_until, c.assigned_to_id, c.channel, c.inbox_id
  ...
  conv_counts AS (
    SELECT
      COUNT(*) FILTER (WHERE deleted_at IS NULL
        AND (snooze_until IS NULL OR snooze_until <= NOW()))::bigint as conversations_all,
      COUNT(*) FILTER (WHERE status = 'open' AND deleted_at IS NULL
        AND (snooze_until IS NULL OR snooze_until <= NOW()))::bigint as conversations_open,
      ...
      COUNT(*) FILTER (WHERE assigned_to_id = v_profile_id AND deleted_at IS NULL
        ...
        AND (snooze_until IS NULL OR snooze_until <= NOW()))::bigint as conversations_assigned,
      COUNT(*) FILTER (WHERE status = 'pending' AND deleted_at IS NULL
        AND (snooze_until IS NULL OR snooze_until <= NOW()))::bigint as conversations_pending,
      COUNT(*) FILTER (WHERE status = 'closed' AND deleted_at IS NULL
        AND (snooze_until IS NULL OR snooze_until <= NOW()))::bigint as conversations_closed,
  ...
  notif_counts AS (
    ...
    WHERE user_id = v_user_id
      ...
      AND (
        type ILIKE '%mention%'
        OR type ILIKE '%assign%'
        OR type ILIKE '%call%'
        OR type ILIKE '%voicemail%'
      )
  ),
  inbox_data AS (
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'open_count', COALESCE(conv_count.open_cnt, 0),
        'unread_count', COALESCE(conv_count.unread_cnt, 0),
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE status = 'closed' AND deleted_at IS NULL
      AND (snooze_until IS NULL OR snooze_until <= NOW()))::bigint,
    COUNT(*) FILTER (WHERE is_archived = true AND deleted_at IS NULL)::bigint,
    COUNT(*) FILTER (WHERE deleted_at IS NOT NULL)::bigint,
    COUNT(*) FILTER (WHERE channel = 'email' AND deleted_at IS NULL)::bigint,
    COUNT(*) FILTER (WHERE channel = 'facebook' AND deleted_at IS NULL)::bigint,
    COUNT(*) FILTER (WHERE channel = 'instagram' AND deleted_at IS NULL)::bigint,
    COUNT(*) FILTER (WHERE channel = 'whatsapp' AND deleted_at IS NULL)::bigint
  FROM threaded;
END;
$function$;