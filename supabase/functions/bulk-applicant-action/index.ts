...
Deno.serve(async (req) => {
  ...
  if (!UUID_RE.test(body.organization_id || ""))
    return jsonRes({ error: "organization_id invalid" }, 400)
  ...
  if (!VALID_ACTIONS.has(body.action))
    return jsonRes({ error: `Ukjent handling: ${body.action}` }, 400)
  ...
  if (action === "export_csv") {
    ...
    const { data: apps } = await adminClient
      .from("applications")
      .select(
        "applicant_id, current_stage_id, score, applied_at, position_id, job_positions(title)",
      )
    ...
    return jsonRes({
      ...
      errors: results
        .filter((r) => !r.ok)
        .map((r) => ({ applicant_id: r.applicant_id, message: r.message! })),
  ...
  if (action === "move_stage" || action === "reject" || action === "hire") {
    ...
    for (const id of validIds) {
      try {
        ...
        const evtType =
          action === "reject" ? "rejected" : action === "hire" ? "hired" : "stage_changed"
  ...
  if (action === "add_tags" || action === "remove_tags") {
    ...
    for (const t of tagIds)
      if (!UUID_RE.test(t)) return jsonRes({ error: `Ugyldig tag: ${t}` }, 400)
