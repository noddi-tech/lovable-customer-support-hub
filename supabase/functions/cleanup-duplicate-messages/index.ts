  ...
  try {
    ...
    while (hasMore) {
      ...
      if (batch && batch.length > 0) {
        ...
        console.log(
          `[cleanup-duplicate-messages] Page ${pageNumber}: fetched ${batch.length} messages. Total so far: ${allMessages.length}`,
        )
        offset += pageSize
    ...
    console.log(
      `[cleanup-duplicate-messages] Finished fetching. Total messages to analyze: ${allMessages.length}`,
    )
    ...
    console.log(
      `[cleanup-duplicate-messages] Will delete ${messagesToDelete.length} duplicate messages`,
    )
    ...
    for (let i = 0; i < messagesToDelete.length; i += batchSize) {
      ...
      if (elapsedTime > MAX_EXECUTION_TIME) {
        ...
        console.log(
          `[cleanup-duplicate-messages] Approaching timeout after ${elapsedTime}ms, stopping early`,
        )
        console.log(
          `[cleanup-duplicate-messages] Deleted ${totalDeleted} duplicates, ${remainingToDelete} remaining`,
        )
      ...
      console.log(
        `[cleanup-duplicate-messages] Processing batch ${batchNumber}: deleting ${batch.length} messages`,
      )
      ...
      console.log(
        `[cleanup-duplicate-messages] Batch ${batchNumber} complete. Total deleted: ${totalDeleted}/${messagesToDelete.length}`,
      )
