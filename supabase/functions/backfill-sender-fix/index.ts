...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const limitConversations: number = Math.min(
      Math.max(Number(body?.limitConversations) || 300, 1),
      2000,
    )
    ...
    if (orgErr || !orgIdData) {
      return new Response(
        JSON.stringify({ error: "Failed to resolve organization", details: orgErr?.message }),
    ...
    const inboundAddresses = new Set(
      (routes || []).map((r: any) => String(r.address).toLowerCase()),
    )
    ...
    if (!conversations || conversations.length === 0) {
      return new Response(JSON.stringify({ ok: true, processed: 0, updated: 0, skipped: 0 }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    const { data: msgs, error: msgFetchErr } = await supabase
      .from("messages")
      .select(
        "id, conversation_id, email_headers, is_internal, sender_type, created_at, email_subject",
      )
    ...
    for (const conv of conversations) {
      ...
      const xOriginalFromRaw =
        resolveHeader(headers, "X-Original-From") ||
        resolveHeader(headers, "X-Google-Original-From")
      ...
      const looksLikeGroup =
        (!!rcptEmail && fromEmail === rcptEmail) ||
        /\svia\s/i.test(fromRaw || "") ||
        (!!rcptEmail && senderHeaderEmail === rcptEmail)
      ...
      if (
        ...
        customerId &&
        existingCust &&
        (!existingCust.full_name || /\svia\s/i.test(existingCust.full_name))
      ) {
        await supabase
          .from("customers")
    ...
    return new Response(JSON.stringify({ ok: true, processed, updated, skipped }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    })
  } catch (error: any) {
