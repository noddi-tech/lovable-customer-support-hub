...
function extractAllThreadIds(headersRaw?: string | null): {
  helpScoutKey: string | null
  allIds: string[]
} {
  const references = parseHeaderValue(headersRaw, "References")
  const inReply = parseHeaderValue(headersRaw, "In-Reply-To")
  const messageId =
    parseHeaderValue(headersRaw, "Message-ID") || parseHeaderValue(headersRaw, "Message-Id")
...
Deno.serve(async (req: Request) => {
  ...
  console.log(
    `[SendGrid-Inbound][${requestId}] Headers: ${JSON.stringify(Object.fromEntries(req.headers.entries()))}`,
  )
  ...
  if (req.method === "GET") {
    const diagnosticInfo = {
      ...
      instructions:
        "POST requests from SendGrid Inbound Parse will be processed. Ensure SendGrid webhook URL matches expectedWebhookUrl with ?token=YOUR_TOKEN appended.",
    }
  ...
  try {
    ...
    console.log(
      `[SendGrid-Inbound] Authentication check - Header auth: ${!!providedToken}, Query auth: ${!!queryToken}`,
    )

    if (finalToken !== expected) {
      console.log(
        `[SendGrid-Inbound] Authentication failed - Invalid token. Expected length: ${expected.length}, Got length: ${finalToken?.length || 0}`,
      )
      await logIngestion(supabase, {
        source: "sendgrid",
        status: "auth_failed",
    ...
    if (queryToken && !providedToken) {
      console.warn(
        `[SendGrid-Inbound] ⚠️ Using deprecated query parameter authentication. Please migrate to header-based auth (Authorization: Bearer <token> or X-SendGrid-Token: <token>).`,
      )
    } else {
    ...
    console.log(
      `[SendGrid-Inbound] Parsed emails - Header To: ${headerTo}, From: ${fromEmail}, Subject: ${subject}`,
    )
    ...
    console.log(
      `[SendGrid-Inbound] Final recipient determination - RCPT: ${rcptEmail}, Tagged: ${rcptEmailTagged}, Header To: ${headerTo}, From: ${fromEmail}`,
    )

    if (!rcptEmail || !fromEmail) {
      console.log(
        `[SendGrid-Inbound] Missing required fields - rcptEmail: ${rcptEmail}, fromEmail: ${fromEmail}`,
      )
      await logIngestion(supabase, {
        source: "sendgrid",
        status: "failed",
      ...
      return new Response(JSON.stringify({ error: "Missing to/from", headerTo, rcptEmail }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (route && !routeError) {
      ...
      console.log(
        `[SendGrid-Inbound] Found direct inbound route for ${rcptEmail} - Org: ${route.organization_id}, Inbox: ${route.inbox_id}`,
      )
      ...
      if (!org) {
        ...
        return new Response(
          JSON.stringify({ error: "Organization not found", rcptEmail, domain }),
      ...
      const { data: routedInboxId } = await supabase.rpc("get_inbox_for_email", {
    ...
    const xOriginalFromRaw =
      parseHeaderValue(headersRaw, "X-Original-From") ||
      parseHeaderValue(headersRaw, "X-Google-Original-From")
    ...
    if (looksLikeGroup && (replyToEmail || xOriginalFromEmail)) {
      console.log(
        `[SendGrid-Inbound] Detected group forwarding - Original author: ${replyToEmail || xOriginalFromEmail}`,
      )
    ...
    console.log(
      `[SendGrid-Inbound] Final author determination - Email: ${authorEmail}, Display: ${displayName}`,
    )
    ...
    if (!customer_id) {
      console.log(
        `[SendGrid-Inbound] Creating new customer - Email: ${normalizedEmail}, Name: ${displayName}`,
      )
    ...
    if (helpScoutKey) {
      ...
      if (convByExtId && convByExtId.length > 0) {
        ...
        const orgMatch = msgMatch?.find(
          (m: any) => m.conversation?.organization_id === organization_id,
        )
        if (orgMatch) {
          conversation_id = orgMatch.conversation_id
          console.log(
            `[SendGrid-Inbound] Found conversation by message email_message_id: ${conversation_id}`,
          )
        }
      }
    } else {
    ...
    if (!conversation_id) {
      console.log(
        `[SendGrid-Inbound] Creating new conversation - Thread: ${threadKey}, Subject: ${subject}`,
      )
        ...
        if (inboxRow?.purpose === "recruitment") {
          ...
          if (fromEmail) {
            ...
            if (matches && matches.length === 1) {
              ...
              console.log(
                `[SendGrid-Inbound] Recruitment auto-link: ${fromEmail} -> applicant ${applicantIdForConv}`,
              )
            } else if (matches && matches.length === 0) {
              console.log(`[SendGrid-Inbound] Recruitment: no applicant match for ${fromEmail}`)
            } else {
              console.log(
                `[SendGrid-Inbound] Recruitment: multiple applicant matches for ${fromEmail} — manual review`,
              )
            }
          }
        }
      ...
      if (emailPriority === "high") {
        await supabase.from("conversations").update({ priority: "high" }).eq("id", conversation_id)
      }
      console.log(
        `[SendGrid-Inbound] Created conversation with ID: ${conversation_id} (type=${conversationType})`,
      )
    } else {
    ...
    if (attachmentInfoRaw) {
      try {
        ...
        console.log(
          `[SendGrid-Inbound] Found attachment-info with ${Object.keys(attachmentMeta).length} attachments`,
        )
        ...
        for (const [key, info] of Object.entries(attachmentMeta)) {
          ...
          if (file && typeof info === "object") {
            ...
            const mimeType =
              (attachmentData.type as string) || file.type || "application/octet-stream"
            ...
            while (uploadAttempt < MAX_RETRIES && !uploadSuccess) {
              ...
              try {
                ...
                if (uploadError) {
                  ...
                  console.warn(
                    `[SendGrid-Inbound] Upload attempt ${uploadAttempt}/${MAX_RETRIES} failed for ${filename}:`,
                    uploadError,
                  )
                  ...
                  uploadSuccess = true
                  console.log(
                    `[SendGrid-Inbound] Uploaded attachment (attempt ${uploadAttempt}): ${storagePath}`,
                  )
                }
              } catch (err) {
                lastError = err
                console.warn(
                  `[SendGrid-Inbound] Upload exception attempt ${uploadAttempt}/${MAX_RETRIES} for ${filename}:`,
                  err,
                )
                if (uploadAttempt < MAX_RETRIES) {
            ...
            if (!uploadSuccess) {
              console.error(
                `[SendGrid-Inbound] All ${MAX_RETRIES} upload attempts failed for ${filename}:`,
                lastError,
              )
    ...
    if (emailMessageId) {
      ...
      if (!existingMsg) {
        ...
        if (existingMsg2) {
          console.log(
            `[SendGrid-Inbound] LOOP DETECTED: Message-ID ${emailMessageId} already exists (msg ${existingMsg2.id}). Skipping bounce-back.`,
          )
          await logIngestion(supabase, {
            source: "sendgrid",
            ...
            metadata: {
              requestId,
              reason: "bounce_back_loop_detected",
              existingMessageId: existingMsg2.id,
            },
          })
        ...
        console.log(
          `[SendGrid-Inbound] LOOP DETECTED: Message-ID ${emailMessageId} already exists (msg ${existingMsg.id}). Skipping bounce-back.`,
        )
        await logIngestion(supabase, {
          source: "sendgrid",
          status: "skipped_duplicate",
    ...
    console.log(
      `[SendGrid-Inbound] Inserting message - Content type: ${contentType}, Length: ${contentHtml.length}, Message-ID: ${emailMessageId}, Attachments: ${attachments.length}`,
    )
    ...
    if (aiDraftEnabled) {
      ...
      console.log(
        `[SendGrid-Inbound] AI drafts disabled for inbox ${inbox_id} — skipping draft generation`,
      )
    }

    console.log(
      `[SendGrid-Inbound] Successfully processed email - Conversation: ${conversation_id}, Customer: ${customer_id}`,
    )
