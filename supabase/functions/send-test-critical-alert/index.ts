...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const adminRoles = (roles ?? []).filter(
      (r) =>
        (r as { role: string }).role === "admin" || (r as { role: string }).role === "super_admin",
    )
    ...
    const isAuthorized =
      ...
      adminRoles.some((r) => (r as { organization_id: string }).organization_id === organizationId)
    if (!isAuthorized) {
