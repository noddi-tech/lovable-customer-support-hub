...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const extractAllIds = (
      ...
      inReplyTo?: string,
      references?: string,
    ): { helpScoutKey: string | null; allIds: string[] } => {
    ...
    const { helpScoutKey, allIds } = extractAllIds(
      emailData.messageId,
      emailData.inReplyTo,
    ...
    if (helpScoutKey) {
      ...
      if (convByExtId && convByExtId.length > 0) {
        ...
        const { data: msgMatch } = await supabaseClient
          .from("messages")
          .select(
            "conversation_id, conversation:conversations!inner(id, organization_id, subject, customer_id, email_account_id, inbox_id, channel, status, is_read, external_id)",
          )
          .in("email_message_id", allIds)
          .limit(5)

        const orgMatch = (msgMatch as any[])?.find(
          (m: any) => m.conversation?.organization_id === emailAccount.organization_id,
        )
        if (orgMatch) {
          ...
          const { data: fullConv } = await supabaseClient
            .from("conversations")
            .select("*")
            .eq("id", orgMatch.conversation_id)
            .single()
