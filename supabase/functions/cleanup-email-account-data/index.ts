...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!emailAccountId) {
      return new Response(JSON.stringify({ error: "emailAccountId is required" }), {
        status: 400,
    ...
    for (let i = 0; i < conversationIds.length; i += batchSize) {
      ...
      if (messagesError) {
        ...
        console.log(
          `✅ Deleted ${count || 0} messages (batch ${i / batchSize + 1}/${Math.ceil(conversationIds.length / batchSize)})`,
        )
      }
    }
    ...
    for (let i = 0; i < conversationIds.length; i += batchSize) {
      ...
      if (conversationsDeleteError) {
        console.error(
          `❌ Error deleting conversations batch ${i / batchSize + 1}:`,
          conversationsDeleteError,
        )
      } else {
        totalConversationsDeleted += count || 0
        console.log(
          `✅ Deleted ${count || 0} conversations (batch ${i / batchSize + 1}/${Math.ceil(conversationIds.length / batchSize)})`,
        )
      }
    }

    console.log(
      `✅ Cleanup complete: ${totalConversationsDeleted} conversations, ${totalMessagesDeleted} messages deleted`,
    )
