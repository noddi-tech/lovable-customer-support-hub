...
import { buildWizardUrl, FALLBACK_ORIGIN, isAllowedOrigin } from "../_shared/meta-origin.ts"
...
Deno.serve(async (req) => {
  ...
  const { data: stateRow } = await admin
    ...
    .select(
      "id, nonce, origin, expires_at, consumed_at, organization_id, mode, existing_integration_id",
    )
    .eq("id", stateId)
    .maybeSingle()

  const safeOrigin =
    stateRow && isAllowedOrigin(stateRow.origin) ? (stateRow.origin as string) : FALLBACK_ORIGIN
  ...
  if (errorParam) {
    await admin
      ...
      .eq("id", stateId)
    const code =
      errorReason === "user_denied" || errorParam === "access_denied" ? "user_denied" : "fb_error"
    return redirect(
      buildWizardUrl(safeOrigin, {
