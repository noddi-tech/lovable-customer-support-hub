...
async function callNoddi(path: string, init: RequestInit = {}) {
  ...
  if (!res.ok) {
    console.error(`[noddi-tickets] ${init.method || "GET"} ${path} -> ${res.status}`, text.slice(0, 500));
...
function buildListQuery(payload: Record<string, unknown>): string {
  ...
  for (const id of asIntList(payload.service_department_ids)) qs.append("service_department_ids", String(id));
    ...
    switch (action) {
      ...
      case "create": {
        ...
        const body: Record<string, unknown> = {
          ...
          description: typeof payload.description === "string" ? payload.description.slice(0, 20000) : "",
          category,
          priority,
          type,
      ...
      case "patch": {
        ...
        if (typeof patch.description === "string") body.description = patch.description.slice(0, 20000);
        if (typeof patch.category === "string" && TICKET_CATEGORIES.includes(patch.category.toUpperCase())) {
          body.category = patch.category.toUpperCase();
        }
        if (typeof patch.priority === "string" && TICKET_PRIORITIES.includes(patch.priority.toUpperCase())) {
        ...
        return await callNoddi(`/v1/tickets/${id}/`, { method: "PATCH", body: JSON.stringify(body) });
      }
      ...
      case "assign": {
        ...
        return await callNoddi(`/v1/tickets/${id}/assign/`, {
          method: "POST",
          body: JSON.stringify({ assignee_id: assignee === null || !Number.isInteger(assignee) ? null : assignee }),
        });
      }
      ...
      case "resolve": {
        const id = ticketId(payload);
        if (!id) return json({ error: "ticket_id required" }, 400);
        const note = typeof payload.resolution_note === "string" ? payload.resolution_note.slice(0, 5000) : "";
