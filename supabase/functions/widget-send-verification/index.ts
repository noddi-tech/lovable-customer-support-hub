...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!widgetKey || !phoneNumber) {
      return new Response(JSON.stringify({ error: "widgetKey and phoneNumber are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    try {
      ...
      console.warn(
        "[widget-send-verification] MCP failed, falling back to direct API:",
      ...
      if (!resp.ok) {
        ...
        return new Response(
          ...
          {
            status: isClientError ? resp.status : 502,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
        )
      }
