...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version, x-supabase-api-version, x-app-version, x-requested-with, accept, accept-profile, content-profile, prefer, range, x-region, x-cron-key",
    ...
    for (const organizationId of organizationIds) {
      ...
      if (batch.length > 0) {
        ...
        for (const identity of (identities || []) as any[]) {
          ...
          if (identity.identity_type === "email" && identity.value)
            entry.emails.push(identity.value)
          if (identity.identity_type === "phone" && identity.value)
            entry.phones.push(identity.value)
      ...
      for (const { row, phone } of batch) {
        ...
        try {
          ...
          const pushEmail = (value?: string | null) => {
            ...
            if (clean?.includes("@") && !emailValues.includes(clean)) emailValues.push(clean)
          }
          pushEmail(row.email)
