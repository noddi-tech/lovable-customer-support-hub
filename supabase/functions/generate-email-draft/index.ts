  ...
  try {
    ...
    try {
      ...
      if (identifier) {
        ...
        if (summary?.summary_text) {
          ...
          console.log(
            `[generate-email-draft] Injected customer memory for ${identifierType}=${identifier}`,
          )
        }
    ...
    if (customerText) {
      try {
        ...
        if (embedding) {
          const { data: similarResponses } = await supabase.rpc("find_similar_responses", {
    ...
    const systemPrompt =
      SYSTEM_INSTRUCTIONS + customerContextPrompt + customerMemoryContext + knowledgeContext
    ...
    const { data: draftMessage, error: insertErr } = await supabase
      ...
      .insert({
        ...
        metadata: {
          ...
          knowledge_matches: !!knowledgeContext,
          has_customer_memory: !!customerMemoryContext,
        },
      })
      .select("id")
      .single()
    ...
    console.log(
      `[generate-email-draft] Draft created: ${draftMessage?.id} for conversation: ${conversationId}`,
    )
