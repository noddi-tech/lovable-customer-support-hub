...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      /* no body = cron invocation, default to daily */
    }
    ...
    console.log(
      `Digest invoked: type=${digestType}, force=${force}, Oslo hour=${currentHour}, day=${currentDay}`,
    )
    ...
    for (const integration of integrations) {
      ...
      if (!force) {
        ...
        if (currentHour !== configuredHour) {
          console.log(
            `Skipping org ${integration.organization_id}: configured hour ${configuredHour}, current Oslo hour ${currentHour}`,
          )
          continue
        }
        ...
        if (digestType === "weekly" && currentDay !== 1) {
          console.log(
            `Skipping weekly for org ${integration.organization_id}: not Monday (day=${currentDay})`,
          )
          continue
        }
      }
      ...
      const { data: conversations, error: convError } = await supabase
        .from("conversations")
        .select(
          "id, status, channel, priority, subject, customer_id, inbox_id, created_at, updated_at",
        )
      ...
      const routingMap = new Map<
        string,
        { digest_channel_id: string; digest_use_secondary: boolean }
      >()
      if (routingEntries) {
        for (const r of routingEntries) {
          // Skip inboxes where digest is explicitly disabled
          if (r.digest_enabled === false) {
            routingMap.set(r.inbox_id, {
              digest_channel_id: "__disabled__",
              digest_use_secondary: false,
            })
            continue
          }
          if (r.digest_channel_id) {
            routingMap.set(r.inbox_id, {
              ...
              digest_use_secondary: r.digest_use_secondary ?? false,
            })
            ...
            routingMap.set(r.inbox_id, {
              digest_channel_id: r.channel_id,
              digest_use_secondary: r.use_secondary_workspace ?? false,
            })
          }
        }
      }
      ...
      const sendDigestForGroup = async (
        ...
        if (conversationIds.length > 0) {
          ...
          if (msgError) {
            ...
            const convMap = new Map(
              (groupConversations || []).map((c) => [c.id, c.subject || "(no subject)"]),
            )
        ...
        const urgentCount = (groupConversations || []).filter(
        ...
        if (openaiApiKey && customerMessages.length > 0) {
          try {
            const messagesForAI = customerMessages
              .slice(0, 80)
              .map((m, i) => `[${i + 1}] Subject: "${m.conversation_subject}" — "${m.content}"`)
              .join("\n")
...
Max 150 words total. No extra sections.`
            ...
            const aiResponse = await fetch("https://api.openai.com/v1/chat/completions", {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  ...
                  {
                    role: "user",
                    content: `Here are ${customerMessages.length} customer messages from the last ${periodDays} day(s):\n\n${messagesForAI}`,
                  },
                ],
                max_tokens: 500,
                temperature: 0.3,
        ...
        if (urgentCount > 0) {
          blocks.push({
            type: "section",
            fields: [{ type: "mrkdwn", text: `*🔴 Urgent/High priority:*\n${urgentCount}` }],
          })
        }

        blocks.push({
          type: "section",
          fields: [{ type: "mrkdwn", text: `*By channel:*\n${channelBreakdown}` }],
        })
        ...
        console.log(
          `${periodLabel} digest sent for org ${orgId}${inboxLabel ? ` (${inboxLabel})` : ""}`,
        )
        return { sent: true }
      }
      ...
      for (const [inboxId, groupConvs] of inboxGroups) {
        ...
        const token =
          routing.digest_use_secondary && integration.secondary_access_token
            ? integration.secondary_access_token
            : integration.access_token
        const inboxName = inboxNames.get(inboxId) || "Unknown Inbox"
        const result = await sendDigestForGroup(
          groupConvs,
