...
const PARTNER_BASE_URL =
  Deno.env.get("PARTNER_BASE_URL")?.replace(/\/+$/, "") || "https://partner.noddi.co"

const cors = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version, x-app-version",
...
function primaryBookingDateIso(
  booking: any,
  type: "upcoming" | "completed" | null | undefined,
): string | null {
  ...
  if (type === "completed") {
    return pick(booking, "completed_at", "finished_at", "date", "updated_at", "created_at") || null
  }
  return (
    pick(
      booking,
...
function extractMoney(b: any) {
  ...
  const paid_state: "paid" | "partially_paid" | "unpaid" | "unknown" = b?.order?.is_fully_paid
    ? "paid"
    : b?.order?.is_partially_paid
      ? "partially_paid"
      : outst > 0
        ? "unpaid"
        : "unknown"
...
function extractOrderSummary(b: any) {
  ...
  const normLines = lines.map((l: any) => {
    ...
    return {
      ...
      subtotal: isDiscount
        ? -Math.abs(subtotal || unitAmount * quantity)
...
function textFromBooking(b: any): string {
  ...
  push(b?.vehicle?.label)
  push(b?.vehicle?.name)
  push(b?.car?.label)
  push(b?.car?.make)
  push(b?.car?.model)
  push(b?.car?.notes)
  push(b?.booking_reference)
  push(b?.metadata?.summary)
  push(b?.notes)
...
function buildPartnerUrls(userGroupId: number | null, booking: any) {
  ...
  return {
    customer_url:
      userGroupId != null && Number.isFinite(userGroupId)
        ? `${PARTNER_BASE_URL}/customers/${userGroupId}`
        : null,
    booking_url:
      bookingId != null && Number.isFinite(bookingId)
        ? `${PARTNER_BASE_URL}/bookings/${bookingId}`
        : null,
...
async function getUserByPhone(phone: string) {
  ...
  const response = await fetch(
    `${API_BASE}/v1/users/get-by-phone-number/?phone_number=${encodedPhone}`,
    {
      headers: noddiAuthHeaders(),
    },
  )
...
async function enrichBookingIfNeeded(pb: any): Promise<{ tags: string[]; bookingForCache: any }> {
  ...
  try {
    if (DEBUG)
      console.log(
        `[noddi] Enriching booking ${pb.id} (missing: tags=${tags.length === 0}, service=${!hasService}, vehicle=${!hasVehicle})`,
      )
...
function buildResponse(params: {
  ...
  const safePriorityBookingType = ["upcoming", "completed"].includes(
    priority_booking_type as string,
  )
  ...
  const status_label_computed = priority_booking
    ? statusLabel(priority_booking.status ?? priority_booking.booking_status)
    : null
  ...
  return {
    ...
    data: {
      ...
      ui_meta: {
        ...
        location_type:
          priority_booking?.booking_location_type || priority_booking?.location_type || null,
        comments_unable_to_complete_public:
          priority_booking?.comments?.user ||
          priority_booking?.comments_unable_to_complete_public ||
          null,
        comments_unable_to_complete_internal:
          priority_booking?.comments?.admin ||
          priority_booking?.comments_unable_to_complete_internal ||
          null,
...
function mapCacheRowToUnified(
  ...
  const booking_date_iso = primaryBookingDateIso(
    priorityBooking,
    cacheRow?.priority_booking_type ?? null,
  )
  const status_label_computed = priorityBooking
    ? statusLabel(priorityBooking.status ?? priorityBooking.booking_status)
    : null
  ...
  const order_tags = Array.isArray(cacheRow.cached_order_tags)
    ? cacheRow.cached_order_tags
    : extractOrderTags(priorityBooking)
  const hasLines = Array.isArray(order_summary?.lines) && order_summary.lines.length > 0
  ...
  return {
    ...
    data: {
      ...
      ui_meta: {
        ...
        location_type:
          priorityBooking?.booking_location_type || priorityBooking?.location_type || null,
        comments_unable_to_complete_public:
          priorityBooking?.comments?.user ||
          priorityBooking?.comments_unable_to_complete_public ||
          null,
        comments_unable_to_complete_internal:
          priorityBooking?.comments?.admin ||
          priorityBooking?.comments_unable_to_complete_internal ||
          null,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.forceRefresh) {
      ...
      for (const emailToCheck of emailsToTry) {
        try {
          ...
          if (cachedData) {
            console.log(
              "✅ Returning cached data (matched email):",
              `${emailToCheck.substring(0, 3)}***`,
            )

            const cacheAge = Math.floor(
              (Date.now() - new Date(cachedData.last_refreshed_at).getTime()) / 1000,
            )
    ...
    console.log(
      `📧 Will try ${emailsToTry.length} email(s): ${emailsToTry.map((e) => `${e?.substring(0, 3)}***`).join(", ")}`,
    )
    ...
    for (const endpoint of ENDPOINTS) {
      ...
      for (let i = 0; i < emailsToTry.length; i++) {
        ...
        console.log(
          `📧 [${i + 1}/${emailsToTry.length}] Trying lookup with email: ${emailToTry?.substring(0, 3)}***`,
        )
        ...
        if (response.ok) {
          ...
          console.log(
            `✅ Found user with ${endpoint.label} endpoint, email: ${emailToTry?.substring(0, 3)}***`,
          )
          break
        }
        ...
        if (response.status === 400 || response.status === 404) {
          ...
          if (isNotFound && i < emailsToTry.length - 1) {
            console.log(
              `⚠️ User not found with ${emailToTry?.substring(0, 3)}***, trying next email...`,
            )
            continue
          }
    ...
    if (!lookupResponse?.ok && body.noddi_user_id) {
      ...
      try {
        ...
        if (userResp.ok) {
          ...
          if (userGroupId) {
            try {
              const ugResp = await fetch(`${API_BASE}/v1/user-groups/${userGroupId}/`, {
                headers: noddiAuthHeaders(),
              })
              if (ugResp.ok) userGroupData = await ugResp.json()
            } catch (e) {
              console.warn("Failed to fetch user group:", e)
            }
          }
    ...
    if (!lookupResponse.ok) {
      ...
      if (lookupResponse.status >= 500) {
        ...
        try {
          ...
          if (!legacyUser) {
            ...
            return json({
              ...
              data: {
                ...
                unpaid_bookings: [],
                ui_meta: {
                  display_name: successfulEmail
                    ? successfulEmail.split("@")[0]
                    : phone || "Unknown",
                  user_group_badge: null,
                  unpaid_count: 0,
                  status_label: null,
          ...
          // Fetch user groups
          const groupsResponse = await fetch(`${API_BASE}/v1/user-groups/?user=${legacyUser.id}`, {
            headers: noddiAuthHeaders(),
          })
          ...
          const primaryGroup =
            userGroups.find((g: any) => g.is_default_user_group) ||
            userGroups.find((g: any) => g.is_personal) ||
            userGroups[0]

          // Fetch unpaid bookings for all groups
          const allUnpaidBookings: any[] = []
          ...
          console.log(
            `✅ Legacy lookup successful. User: ${legacyUser.id}, Groups: ${userGroups.length}, Unpaid: ${allUnpaidBookings.length}`,
          )
          ...
          console.log(
            `User: ${noddihUser.id}, Groups: ${userGroups.length}, Total bookings: ${lookupData.metadata?.total_bookings_count}, Unpaid: ${lookupData.metadata?.total_unpaid_count}`,
          )
          ...
          for (const group of userGroups) {
            ...
            if (upcoming) {
              ...
              if (
                priorityBooking.deliveryWindowStartsAt &&
              ...
              console.log(
                `Legacy priority booking ${priorityBooking.id} from group ${group.id} (type: ${priorityBookingType})`,
              )
              break
            }
          }
          ...
          if (!priorityBooking && allUnpaidBookings.length > 0) {
            ...
            const groupUnpaid = allUnpaidBookings.filter(
              (b: any) => b.user_group_id === priorityGroup?.id,
            )
            ...
            if (groupUnpaid.length > 0) {
              ...
              priorityBookingType = "completed"
              console.log(
                `Using first unpaid booking ${priorityBooking.id} as priority booking (type: ${priorityBookingType})`,
              )
            }
          }
          ...
          console.log(
            `Selected group ${selectedGroup.id} has ${pendingBookings.length} unpaid bookings`,
          )
          ...
          const allUserGroupsFormatted = userGroups.map((g: any) => ({
            ...
            coupons: (() => {
              const raw = g.coupons || []
              if (raw.length > 0)
                console.log(`Raw coupons for group ${g.id}:`, JSON.stringify(raw.slice(0, 2)))
    ...
    console.log(
      `✅ Customer lookup successful. User: ${lookupData.user?.id}, Groups: ${lookupData.user_groups?.length}, Unpaid: ${lookupData.unpaid_bookings?.length}`,
    )
    ...
    console.log(
      `User: ${noddihUser.id}, Groups: ${userGroups.length}, Total bookings: ${metadata?.total_bookings_count}, Unpaid: ${metadata?.total_unpaid_count}`,
    )
    ...
    if (!priorityBooking && priorityGroup?.bookings_summary) {
      ...
      if (totalCount > 0) {
        console.log(
          `📊 Group ${priorityGroup.id} has ${totalCount} bookings but no priority_booking. Fetching recent bookings...`,
        )
        ...
        if (recentBookings.length > 0) {
          ...
          if (upcomingBooking) {
            ...
            priorityBookingType = "completed"
            console.log(
              `✅ Using most recent booking ${priorityBooking.id} as priority (completed)`,
            )
          }
        }
      }
    ...
    const liveResponse = {
      ...
      data: {
        ...
        ui_meta: {
          ...
          location_type:
            (bookingForCache || priorityBooking)?.booking_location_type ||
            (bookingForCache || priorityBooking)?.location_type ||
            null,
          comments_unable_to_complete_public:
            (bookingForCache || priorityBooking)?.comments?.user ||
            (bookingForCache || priorityBooking)?.comments_unable_to_complete_public ||
            null,
          comments_unable_to_complete_internal:
            (bookingForCache || priorityBooking)?.comments?.admin ||
            (bookingForCache || priorityBooking)?.comments_unable_to_complete_internal ||
            null,
