...
async function getLearnedPatterns(supabase: any, organizationId: string): Promise<string> {
  try {
    ...
    return (
      ...
      patterns
        .map((p: any) => `- ${p.pattern_description} (used ${p.occurrence_count}x successfully)`)
        .join("\n") +
...
async function getCustomerMemoryContext(
  ...
  return (
    "\n\n" +
    buildCustomerMemoryPrompt(summary.summary_text, (memories || []) as CustomerMemory[]) +
    "\n"
  )
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { customerMessage, organizationId, customerIdentifier, conversationId } = await req
      .json()
      .catch(() => ({
        customerMessage: "",
        organizationId: null,
      ...
      try {
        ...
        if (resolved) {
          ...
          if (customerMemoryContext) {
            console.log(
              `[suggest-replies] Injected customer memory for ${resolved.type}=${resolved.identifier}`,
            )
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (resolved) {
          ...
          if (lookupResult.found) {
            ...
            console.log(
              `[suggest-replies] Injected live customer data for ${resolved.type}=${resolved.identifier}`,
            )
          }
        }
      } catch (e) {
    ...
    if (organizationId && SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY && supabaseClient) {
      try {
        ...
        if (embedding && supabaseClient) {
          ...
          const { data: similarResponses } = await supabaseClient.rpc("find_similar_responses", {
