...
function getDecodedEmailContent(message: any): { content: string; contentType: string } {
  ...
  if (!part?.body?.data) return { content: "", contentType: "text" }
  ...
  try {
    ...
    const isLikelyMislabeled = ["usascii", "ascii", "iso88591", "latin1", "windows1252"].includes(
      lowCharset,
    )
    ...
    if (isLikelyMislabeled) {
      ...
      if (utf8Replacements <= declaredReplacements && utf8Replacements < declaredReplacements) {
        console.log(
          `[gmail-sync] UTF-8 produced fewer replacement chars (${utf8Replacements} vs ${declaredReplacements}), using UTF-8`,
        )
      }
    ...
    if (decodedContent.includes("\uFFFD") && charset !== "utf-8") {
      console.log(
        `[gmail-sync] Detected replacement chars with charset '${charset}', retrying as utf-8`,
      )
      const utf8Result = decodeAndFix("utf-8")
      const utf8Replacements = (utf8Result.match(/\uFFFD/g) || []).length
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let query = supabaseClient.from("email_accounts").select("*").eq("is_active", true)
    ...
    for (const account of emailAccounts) {
      try {
        ...
        let sentResult: { success: boolean; messageCount?: number; error?: string } = {
          success: true,
          messageCount: 0,
        }
...
async function syncGmailMessages(
  ...
  try {
    ...
    console.log(
      `📧 Processing ${data.messages.length} messages from ${folder} for ${account.email_address}...`,
    )
    ...
    for (const message of data.messages) {
      ...
      try {
        console.log(
          `🔄 Processing message ID: ${message.id} (${processedCount + 1}/${data.messages.length})`,
        )
        ...
        try {
          const { data: extractedDate } = await supabaseClient.rpc("extract_email_date", {
          ...
          if (extractedDate) {
            ...
            console.log(
              `✅ Extracted date from headers: ${extractedDate} -> ${receivedAt.toISOString()}`,
            )
          } else {
          ...
          if (dateHeader) {
            try {
              ...
              console.log(
                `📅 Fallback: Using Date header: ${dateHeader} -> ${receivedAt.toISOString()}`,
              )
            } catch (e) {
              console.warn(`Failed to parse Date header: ${dateHeader}`, e)
              receivedAt = new Date(parseInt(fullMessage.internalDate, 10) || Date.now())
            }
          } else if (fullMessage.internalDate) {
            receivedAt = new Date(parseInt(fullMessage.internalDate, 10))
            console.log(
              `📅 Fallback: Using internalDate: ${fullMessage.internalDate} -> ${receivedAt.toISOString()}`,
            )
          } else {
        ...
        const extractAttachments = (parts: any[]) => {
          ...
          for (const part of parts) {
            ...
            const contentDisposition = headers.find(
              (h: any) => h.name === "Content-Disposition",
            )?.value
            ...
            if (part.body?.attachmentId) {
              ...
              const attachment = {
                ...
                contentId: contentId
                  ? contentId.replace(/^cid:/i, "").replace(/[<>]/g, "")
                  : undefined,
        ...
        if (attachments.length > 0) {
          ...
          for (const att of attachments) {
            ...
            try {
              ...
              if (!attResponse.ok) {
                console.warn(
                  `[Gmail-Sync] Failed to download attachment ${att.filename}: ${attResponse.status}`,
                )
                continue
              }
              ...
              if (uploadError) {
                console.warn(
                  `[Gmail-Sync] Failed to upload attachment ${att.filename}:`,
                  uploadError,
                )
                continue
              }
        ...
        const xOrigFrom =
          headers.find((h: any) => h.name === "X-Original-From")?.value ||
        ...
        const looksLikeGroup =
          fromEmail === accountAddress || / via /i.test(from) || senderEmail === accountAddress
        ...
        if (!conversation) {
          ...
          const { data: inboxId } = await supabaseClient.rpc("get_inbox_for_email", {
          ...
          if (!inboxId) {
            console.log(
              `⚠️ Skipping message ${messageId} - no inbox configured for ${recipientEmail}`,
            )
            continue
          }
          ...
          if (
            !existingConversation?.received_at ||
            new Date(existingConversation.received_at) < receivedAt
          ) {
            await supabaseClient
              .from("conversations")
        ...
        if (insertError) {
          ...
          console.log(
            `Successfully inserted message ${insertedMessage?.id} for conversation ${conversation.id}`,
          )
        }
