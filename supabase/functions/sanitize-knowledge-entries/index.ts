...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[sanitize] Processing ${entries.length} entries (dryRun=${dryRun}, concurrency=${CONCURRENCY})`,
    )
    ...
    async function processEntry(entry: any): Promise<void> {
      try {
        ...
        if (contextChanged || responseChanged) {
          console.log(
            `[sanitize] Sanitized entry ${entry.id} (ctx:${contextChanged} resp:${responseChanged})`,
          )
        } else {
