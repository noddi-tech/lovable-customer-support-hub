...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (fetchError) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch entries", detail: fetchError.message }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      ...
      try {
        ...
        if (composite > 0.7) {
          ...
          const embedding = await generateEmbedding(
            OPENAI_API_KEY,
            `${sanitizedContext} ${sanitizedResponse}`,
          )
    ...
    console.log(
      `[bulk-evaluate] Batch done: ${promoted} promoted, ${flagged} flagged, ${archived} archived, ${errors} errors. ${remaining ?? "?"} remaining.`,
    )
