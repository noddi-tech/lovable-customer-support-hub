    ...
    for (const integration of integrations) {
      ...
      for (const row of feedback) {
        ...
        if (row.matched_keyword) {
          ...
          if (row.conversation_id && stats.evidence.length < 10)
            stats.evidence.push(row.conversation_id)
        }
      ...
      const overrides =
        (integration.critical_keyword_overrides as { disabled?: string[]; added?: string[] }) || {}
