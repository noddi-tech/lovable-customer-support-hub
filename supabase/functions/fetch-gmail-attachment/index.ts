...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[fetch-gmail-attachment] Fetching attachment "${filename}" for message ${messageId}`,
    )
    ...
    const { data: message, error: msgError } = await supabaseClient
      .from("messages")
      .select(
        "id, external_id, attachments, conversation_id, conversations!inner(organization_id, email_account_id)",
      )
      .eq("id", messageId)
      .single()
    ...
    if (!account) {
      ...
      console.log(
        "[fetch-gmail-attachment] No direct account, searching org for Gmail OAuth account...",
      )
      const { data } = await supabaseClient
        .from("email_accounts")
        .select("id, access_token, refresh_token, token_expires_at, email_address")
    ...
    if (!account) {
      return new Response(
        JSON.stringify({
          error:
            "No Gmail account with OAuth access found. This attachment was received via forwarding and cannot be fetched on-demand. Please connect a Gmail account with OAuth to enable attachment downloads.",
          recoverable: false,
        }),
        {
          status: 400,
    ...
    if (!attachment) {
      return new Response(
        JSON.stringify({ error: `Attachment "${filename}" not found in message data` }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    console.log(
      `[fetch-gmail-attachment] Fetching from Gmail: message=${gmailMessageId}, attachment=${attachment.attachmentId}`,
    )
