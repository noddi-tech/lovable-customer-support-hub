  ...
  try {
    ...
    while (true) {
      ...
      for (const conv of conversations) {
        ...
        if (msgError) {
          console.error(
            `[cleanup-forwarding-echoes] Error fetching messages for ${conv.id}:`,
            msgError,
          )
          continue
        }
        ...
        for (const m of messages) {
          ...
          for (const agentMsg of agentTexts) {
            ...
            if (inboundText.includes(searchKey)) {
              echoIds.push(m.id)
              console.log(
                `[cleanup-forwarding-echoes] Echo found: message ${m.id} echoes agent message ${agentMsg.id} in conversation ${conv.id}`,
              )
              break
            }
          }
