...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!application_id || !target_stage_id) {
      return new Response(
        JSON.stringify({ error: "application_id and target_stage_id required" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const { data: reqRows } = await admin
      ...
      .select(
        "id, custom_field_id, position_id, requirement_type, block_stage_progression, recruitment_custom_fields!inner(display_name, type_id, recruitment_custom_field_types(type_key))",
      )
