...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!fromEmail)
      return json({ error: "Inbox has no active inbound route / sender address" }, 400)
    ...
    const subCtx = {
      caller: "send-recruitment-email",
      organization_id: applicant?.organization_id ?? orgRow?.id ?? null,
    }
    ...
    const toName = applicant
      ? `${applicant.first_name || ""} ${applicant.last_name || ""}`.trim()
      : null

    const fromName =
      callerProfile.email_display_name ||
