...
async function dispatchSendEmail(
  ...
  if (!applicationId) {
    return {
      success: false,
      error: "send_email requires application_id in trigger_context",
      duration_ms: 0,
    }
  }
  ...
  const recipient = (appData as { applicants?: { email?: string } | null } | null)?.applicants
    ?.email
  if (appErr || !recipient) {
    return {
      success: false,
...
async function dispatchAssignTo(
  ...
  if (!userId)
    return { success: false, error: "assign_to requires user_id in action_config", duration_ms: 0 }
  if (!applicationId)
    return { success: false, error: "assign_to requires application_id", duration_ms: 0 }
...
async function dispatchWebhook(
  ...
  if (!url)
    return { success: false, error: "webhook requires url in action_config", duration_ms: 0 }
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `[dispatch] queue_id=${queueId} execution_id=${claim.execution_id} action_type=${actionType}`,
    )
    ...
    return json(
      { ok: false, error: e?.message ?? "Unknown error", duration_ms: elapsed(startedAt) },
      200,
    )
  }
}
