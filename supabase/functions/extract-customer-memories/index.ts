  ...
  try {
    ...
    for (const mem of extracted) {
      ...
      if (isDuplicate && duplicateId) {
        ...
        memoriesUpdated++
        console.log(
          `[extract-memories] Updated existing memory ${duplicateId}, confidence: ${newConfidence}`,
        )
      } else {
        const { error: insertError } = await supabase.from("customer_memories").insert({
    ...
    if (allMemories && allMemories.length > 0) {
      const memoriesForSummary = allMemories
        .map((m: any) => `[${m.memory_type}] ${m.memory_text} (confidence: ${m.confidence})`)
        .join("\n")

      const summaryText = await callGpt(OPENAI_API_KEY, SUMMARY_PROMPT, memoriesForSummary, 200)
      ...
      if (sentimentMemories.length >= 2) {
        const sentiments = sentimentMemories.map((m: any) => {
          ...
          if (
            sd.toLowerCase().includes("positive") ||
            sd.toLowerCase().includes("happy") ||
            sd.toLowerCase().includes("satisfied")
          )
            return 1
          if (
            sd.toLowerCase().includes("negative") ||
            sd.toLowerCase().includes("frustrated") ||
            sd.toLowerCase().includes("unhappy")
          )
            return -1
          return 0
        })
      ...
      const { error: upsertError } = await supabase.from("customer_summaries").upsert(
        {
          organization_id: organizationId,
          customer_identifier: customerIdentifier,
          identifier_type: identifierType,
    ...
    console.log(
      `[extract-memories] Done: type=${conversationType}, extracted=${memoriesExtracted}, updated=${memoriesUpdated}, summary=${summaryGenerated}`,
    )
