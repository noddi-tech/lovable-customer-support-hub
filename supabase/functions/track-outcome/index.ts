...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const agentMessageTime = new Date(
      messages.find((m) => m.id === agentMessageId)?.created_at || 0,
    )
    const customerReplyTime = new Date(currentMessage.created_at)
    const replyTimeSeconds = Math.floor(
      (customerReplyTime.getTime() - agentMessageTime.getTime()) / 1000,
    )
    ...
    const { error: outcomeError } = await supabase.from("response_outcomes").insert({
