...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-service-signature, x-messente-signature",
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.warn(
      `[sms-status-callback/${providerName}] signature validation failed:`,
      (e as Error).message,
    )
  ...
  if (!updated || updated.length === 0) {
    console.warn(
      `[sms-status-callback] no message found for provider=${providerName} id=${parsed.providerMessageId}`,
    )
  }
