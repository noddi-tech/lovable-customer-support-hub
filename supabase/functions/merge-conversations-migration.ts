...
function extractHelpScoutThreadId(
  messageId?: string,
  inReplyTo?: string,
  references?: string,
): string | null {
...
async function migrateConversations() {
  ...
  for (const [threadId, convs] of helpScoutThreadGroups) {
    if (convs.length > 1) {
      ...
      console.log(
        `   🔴 Thread ${threadId} split across ${convs.length} conversations (${messageCount} messages)`,
      )
    }
  }
  ...
  if (splitHelpScoutThreads.size > 0) {
    ...
    for (const [threadId, convs] of splitHelpScoutThreads) {
      try {
        ...
        await supabase.from("conversations").update({ external_id: threadId }).eq("id", primary.id)
  ...
  if (!messages || messages.length === 0) {
    ...
    for (const msg of messages as Message[]) {
      ...
      const canonicalThreadId = getCanonicalThreadId(msg.email_message_id!, inReplyTo, references)
