...
type TokenStatus = "connected" | "expiring_soon" | "expiring_critical" | "expired" | "broken"
...
function classify(
  ...
  if (!isValid)
    return {
      status: "expired",
      message: "Tokenet er utløpt eller ugyldig.",
      missing,
      daysLeft: null,
    }
  if (missing.length > 0)
    return {
      status: "broken",
      message: `Mangler tilganger: ${missing.join(", ")}`,
      missing,
      daysLeft: null,
    }
  ...
  if (days < 7)
    return {
      ...
      message: `Tokenet utløper om ${Math.max(0, Math.floor(days))} dager.`,
      missing,
      daysLeft: days,
    }
  if (days < 30)
    return {
      ...
      message: `Tokenet utløper om ${Math.floor(days)} dager.`,
      missing,
      daysLeft: days,
    }
...
function alertTypeFor(status: TokenStatus): string | null {
  switch (status) {
    ...
    case "expiring_critical":
      return "token_expiring_critical"
    case "expired":
      return "token_expired"
    case "broken":
      return "integration_broken"
    default:
      return null
  }
}
...
function severityFor(status: TokenStatus): "info" | "warning" | "critical" {
  if (status === "expired" || status === "broken" || status === "expiring_critical")
    return "critical"
...
Deno.serve(async (req) => {
  ...
  for (const integ of integrations ?? []) {
    ...
    try {
      ...
      cls = classify(
        ...
        Array.isArray(info?.scopes) ? info.scopes : [],
      )
    } catch {
      cls = {
        status: "expired",
        message: "Kunne ikke kontakte Meta debug_token.",
        missing: [],
        daysLeft: null,
      }
    }
    ...
    const expiresAtIso =
      cls.daysLeft === null && cls.status === "connected"
        ? "infinity"
        : cls.daysLeft !== null
  ...
  return new Response(JSON.stringify({ checked, transitioned, alerts_created: alertsCreated }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  })
})
