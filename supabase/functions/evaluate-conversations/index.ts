...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[evaluate-conversations] ${toEvaluate.length} unevaluated out of ${candidates.length} total`,
    )
    ...
    if (scores.length >= 5) {
      ...
      if (flagErr) {
        ...
        console.log(
          `[evaluate-conversations] Flagged conversations with composite <= ${threshold.toFixed(3)}`,
        )
      }
    }
...
async function evaluateConversation(
  ...
  const flags = messages.filter((m: any) => m.quality_flag).map((m: any) => m.quality_flag)
  ...
  try {
    ...
    if (composite < 0.5) {
      await supabase
        ...
        .then(({ error: qErr }: any) => {
          if (qErr) console.warn("[evaluate-conversations] Review queue insert error:", qErr)
        })
    }
