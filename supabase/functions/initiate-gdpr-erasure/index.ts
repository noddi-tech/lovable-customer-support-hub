...
Deno.serve(async (req) => {
  ...
  if (applicant.anonymized_at) {
    return json({ error: "already_anonymized", anonymized_at: applicant.anonymized_at }, 409)
  }
  ...
  const fullName =
    [applicant.first_name, applicant.last_name].filter(Boolean).join(" ").trim() || "(uten navn)"
