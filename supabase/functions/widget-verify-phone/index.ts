        ...
        try {
          ...
          if (parsed.result) {
            if (parsed.result.isError)
              throw new Error(parsed.result.content?.[0]?.text || "MCP error")
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[widget-verify-phone] MCP failed, falling back to direct API:",
        (mcpErr as Error).message,
      )
      ...
      if (!NODDI_API_TOKEN) {
        ...
        return new Response(
          JSON.stringify({
            ...
            attemptsRemaining: PIN_MAX_ATTEMPTS - (attempts + 1),
          }),
    ...
    if (conversationId) {
      ...
      await supabase.from("widget_ai_conversations").update(updateData).eq("id", conversationId)
    }
