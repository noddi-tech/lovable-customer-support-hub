...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!responseTrackingId) {
      return new Response(JSON.stringify({ error: "responseTrackingId required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
...
async function processOne(
  ...
  const { data: rt, error } = await supabase
    ...
    .select(
      "id, organization_id, customer_message, agent_response, original_ai_suggestion, was_refined",
    )
  ...
  if (editDist < 0.1) {
    console.log(
      `[classify-agent-edit] Skipping trivial edit (${(editDist * 100).toFixed(1)}%): ${responseTrackingId}`,
    )
    return { skipped: "trivial_edit" }
  }

  const category = await classifyEdit(
    openaiKey,
    rt.customer_message,
    rt.original_ai_suggestion,
    rt.agent_response,
  )
  ...
  console.log(
    `[classify-agent-edit] Created preference pair: category=${category}, dist=${(editDist * 100).toFixed(1)}%`,
  )
...
async function processBackfill(supabase: any, openaiKey: string): Promise<number> {
  ...
  console.log(
    `[classify-agent-edit] Backfill complete: ${processed}/${candidates.length} processed`,
  )
  return processed
}
