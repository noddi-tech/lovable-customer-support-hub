...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (intError || !integrations || integrations.length === 0) {
      return new Response(
        JSON.stringify({
          message: "No orgs with critical channels configured",
          reviewed: 0,
          alerted: 0,
        }),
    ...
    for (const integration of integrations) {
      ...
      for (const conv of conversations) {
        ...
        if (conv.inbox_id) {
          ...
          if (routing) {
            ...
            if (routing.critical_channel_id) {
              ...
              convCriticalToken =
                routing.critical_use_secondary && integration.secondary_access_token
              ...
              convCriticalToken =
                routing.use_secondary_workspace && integration.secondary_access_token
                  ? integration.secondary_access_token
                  : integration.access_token
            }
          }
        ...
        try {
          ...
          console.log(
            `🚨 Alert sent for conv ${conv.id} (keyword: ${matchedKeyword}) ts=${critResult.ts}`,
          )
        } catch (err) {
    ...
    console.log(
      `✅ Batch review complete: ${summary.reviewed} reviewed, ${summary.alerted} alerted, ${summary.skipped_dedup} deduped`,
    )
