...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Invalid action. Use: list, update, stats" }), {
      status: 400,
...
async function handleUpdate(supabase: any, body: any) {
  ...
  if (!id || !status || !["reviewed", "dismissed"].includes(status)) {
    return new Response(JSON.stringify({ error: "id and status (reviewed|dismissed) required" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    })
  }
