...
(function() {
  ...
  // Map a host locale (BCP-47 'nb-NO' / 'en-US' / 'sv-SE', or the customer
  // frontend codes nb / en / se) onto a widget UI language code.
...
.noddi-chat-status-actions{display:flex;align-items:center;gap:4px}
.noddi-chat-resolve-button{background:none;border:none;color:#16a34a;font-size:12px;cursor:pointer;padding:4px 8px;border-radius:4px}
.noddi-chat-resolve-button:hover{background:#f0fdf4}
...
\`;
  ...
  async function endChatSession(sessionId, resolved) {
    try {
      await fetch(apiUrl + '/widget-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // resolved:true also closes the conversation in the support app
        body: JSON.stringify({ action: 'end', sessionId, resolved: !!resolved })
      });
    } catch (e) {}
  }
      ...
      if (state.showSuccess && state.activeSubmission) {
        ...
        if (!isEnded && session) {
          if (session.status === 'waiting') {
            ...
            html += '<div class="noddi-chat-status-actions">';
            html += '<button class="noddi-chat-resolve-button" data-action="resolve-chat">' + (t.markResolved || 'Mark as resolved') + '</button>';
  ...
  async function handleAction(action) {
    ...
    } else if (action === 'end-chat' || action === 'resolve-chat') {
      if (state.chatSession) {
        await endChatSession(state.chatSession.id, action === 'resolve-chat');
