...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { refinementInstruction, originalSuggestion, refinedSuggestion, organizationId } =
      await req.json()
    ...
    if (lowerInstruction.includes("discount") || lowerInstruction.includes("rabatt")) {
      ...
      lowerInstruction.includes("apolog") ||
      lowerInstruction.includes("sorry") ||
      lowerInstruction.includes("beklager")
    ) {
      patternKey = "more_apologetic"
      patternDescription = "Make response more apologetic"
    } else if (
      lowerInstruction.includes("timeline") ||
      lowerInstruction.includes("when") ||
      lowerInstruction.includes("tid")
    ) {
      ...
      lowerInstruction.includes("detalj")
    ) {
      ...
      lowerInstruction.includes("kort")
    ) {
