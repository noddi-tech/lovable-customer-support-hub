...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const existingWebhook = currentSettings.result?.find(
      (webhook: any) => webhook.hostname === targetHostname,
    )
    ...
    const updateResp = await fetch(
      `https://api.sendgrid.com/v3/user/webhooks/parse/settings/${existingWebhook.id}`,
      {
        method: "PATCH",
        headers: {
