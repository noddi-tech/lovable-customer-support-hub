...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!domain || !parse_subdomain) {
      return new Response(JSON.stringify({ error: "domain and parse_subdomain required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (profileErr || !profile?.organization_id) {
      return new Response(JSON.stringify({ error: "Profile/organization not found" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (!sgKey || !inboundToken) {
      return new Response(
        JSON.stringify({ error: "Missing SENDGRID_API_KEY or SENDGRID_INBOUND_TOKEN" }),
    ...
    async function ensureSenderAuth() {
      ...
      const check = await fetch(
        `https://api.sendgrid.com/v3/whitelabel/domains?domain=${encodeURIComponent(domain)}&limit=1`,
        {
    ...
    if (body.action === "validate" || body.action === "validate_and_retry") {
      ...
      if (!existing) {
        return new Response(JSON.stringify({ ok: false, error: "sender_auth_not_found" }), {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          "Add the MX record (and any sender auth records if shown). We will verify automatically.",
      }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    )
  } catch (error) {
