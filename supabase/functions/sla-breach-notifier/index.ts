  ...
  try {
    ...
    if (approachingBreach && approachingBreach.length > 0) {
      ...
      for (const conv of approachingBreach) {
        ...
        const timeUntilBreach = Math.round(
          (new Date(conv.sla_breach_at).getTime() - now.getTime()) / (60 * 1000),
        )
    ...
    if (breached && breached.length > 0) {
      ...
      for (const conv of breached) {
        ...
        for (const userId of userIdsToNotify) {
          ...
          if (existingNotif) {
            console.log(
              `⏭️ Breach notification already sent for conversation ${conv.id} to user ${userId}`,
            )
            continue
          }

          const breachDuration = Math.round(
            (now.getTime() - new Date(conv.sla_breach_at).getTime()) / (60 * 1000),
          )
