...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Insert feedback
    const { error: feedbackError } = await supabase.from("widget_ai_feedback").insert({
        ...
        .upsert(
          {
            ...
            details: feedbackText
              ? `Negative feedback: ${feedbackText}`
              : "Thumbs-down on AI response",
            status: "pending",
          },
          { onConflict: "conversation_id,reason", ignoreDuplicates: true },
        )
        .then(({ error: qErr }: any) => {
          if (qErr) console.warn("[widget-ai-feedback] Review queue error:", qErr)
        })
    }
