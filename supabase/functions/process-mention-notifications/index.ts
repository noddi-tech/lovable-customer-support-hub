...
async function sendSlackDM(params: {
  ...
  try {
    ...
    const contextLabel =
      params.contextType === "internal_note"
        ? "a note"
        : params.contextType === "ticket_comment"
          ? "a ticket comment"
          : params.contextType === "customer_note"
            ? "a customer note"
            : params.contextType === "call_note"
              ? "a call note"
              : "a message"
    ...
    const fallbackMentioner = params.mentionerSlackId
      ? `<@${params.mentionerSlackId}>`
      : params.mentionerName
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (slackAccessToken) {
      const allUserIds = [
        ...new Set([...mentionedUserIds.filter((id) => id !== mentionerUserId), mentionerUserId]),
      ]
      ...
      console.log(
        `Resolved ${slackIdMap.size} Slack IDs (mentioner: ${mentionerSlackId || "not found"})`,
      )
    }
