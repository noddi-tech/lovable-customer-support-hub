...
const tools = [
  ...
  // Re-enable only after data has been sanitized.
  {
    ...
    function: {
      ...
      description:
        "Look up a customer by phone number or email to find their account and bookings. Phone number is the primary identifier.",
      parameters: {
        type: "object",
        properties: {
          ...
          user_group_id: {
            type: "number",
            description:
              "Specific user group ID to use (when customer selected a group from the options)",
          },
        },
      },
    },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Reschedule a booking to a new date/time. Requires the booking ID and the new desired date/time. Always confirm with the customer before calling this.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "cancel_booking",
      description:
        "Cancel a booking. Requires the booking ID. Always ask the customer to confirm before calling this - cancellations may not be reversible.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "update_booking",
      description:
        "Update an existing booking. Can change address, cars, sales items, or delivery window. Always confirm changes with the customer first using the [BOOKING_EDIT] marker.",
      parameters: {
        type: "object",
        properties: {
          ...
          delivery_window_start: {
            type: "string",
            description: "New delivery window start (ISO 8601)",
          },
          delivery_window_end: {
            type: "string",
            description: "New delivery window end (ISO 8601)",
          },
          cars: {
            type: "array",
            items: {
              ...
              properties: {
                license_plate: {
                  type: "object",
                  properties: { number: { type: "string" }, country_code: { type: "string" } },
                },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Look up car details from a license plate number. Returns car ID, make, model, and year.",
      parameters: {
        type: "object",
        properties: {
          license_plate: {
            type: "string",
            description: "The license plate number (e.g., EC94156)",
          },
          country_code: {
            type: "string",
            description: "ISO-2 country code (default: NO)",
            default: "NO",
          },
        },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "List available service categories for booking at a specific address. Requires address_id. Returns category names and IDs.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get available sales items for a service category. Returns specific bookable items with prices.",
      parameters: {
        type: "object",
        properties: {
          ...
          sales_item_category_id: {
            type: "number",
            description: "The service category ID from list_available_services",
          },
        },
        required: ["address_id"],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get available delivery time windows for a booking. Returns dates and time slots with prices.",
      parameters: {
        type: "object",
        properties: {
          address_id: { type: "number", description: "The address ID" },
          ...
          selected_sales_item_ids: {
            type: "array",
            items: { type: "number" },
            description: "Array of selected sales item IDs",
          },
        },
        required: ["address_id"],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Create a booking via the shopping cart. Pass the full booking payload including address, car, items, and delivery window.",
      parameters: {
        type: "object",
        properties: {
          address_id: { type: "number", description: "The address ID" },
          ...
          sales_item_ids: {
            type: "array",
            items: { type: "number" },
            description: "Array of sales item IDs to book",
          },
  ...
  switch (toolName) {
    case "search_knowledge_base":
      return JSON.stringify({
        results: [],
        disabled: true,
        reason: "Knowledge search temporarily disabled",
      })
    case "lookup_customer": {
      ...
      if (mcpAuthToken) {
        try {
          ...
          console.warn(
            "[executeTool] MCP customer_lookup failed, falling back:",
      ...
      try {
        ...
        console.log(
          "[executeTool] Merged stored data into MCP result — addresses:",
          mcpResultObj.stored_addresses?.length || 0,
          "cars:",
          mcpResultObj.stored_cars?.length || 0,
        )
      } catch (mergeErr) {
    ...
    case "get_booking_details": {
      ...
      if (!bid || (typeof bid === "number" && bid <= 10) || bid === "1") {
        console.warn(
          `[widget-ai-chat] get_booking_details called with placeholder ID ${bid}, redirecting`,
        )
        return JSON.stringify({
          error:
            "The booking details are already available in the conversation history above. Do NOT call this tool with a placeholder ID. Use the booking data (address_id, car info, sales items, delivery window) already provided in the conversation to continue the flow.",
        })
      }
      ...
      if (mcpAuthToken) {
        try {
          ...
          const mcpResult = await callMcpTool("booking_details_get", {
            booking_id: bid,
          ...
          console.warn(
            "[executeTool] MCP booking_details_get failed, falling back:",
            (err as Error).message,
          )
        }
      }
    ...
    case "reschedule_booking": {
      ...
      if (mcpAuthToken) {
        try {
          console.log(
            "[executeTool] Using MCP booking_update for reschedule, booking_id:",
            args.booking_id,
          )
          ...
          console.warn(
            "[executeTool] MCP booking_update (reschedule) failed, falling back:",
            (err as Error).message,
          )
        }
      }
    ...
    case "cancel_booking": {
      if (mcpAuthToken) {
        try {
          ...
          console.warn(
            "[executeTool] MCP booking_cancel failed, falling back:",
            (err as Error).message,
          )
        }
      }
    ...
    case "lookup_car_by_plate":
      return callMcpWithFallback(
        ...
        {
          action: "lookup_car",
          country_code: args.country_code || "NO",
    ...
    case "get_available_items":
      return callMcpWithFallback(
        "sales_item_list",
        {
          ...
          sales_item_category_id: args.sales_item_category_id,
        },
        {
    ...
    case "get_delivery_windows": {
      ...
      if (
        !args.selected_sales_item_ids ||
        (Array.isArray(args.selected_sales_item_ids) && args.selected_sales_item_ids.length === 0)
      ) {
        console.warn(
          "[widget-ai-chat] get_delivery_windows called with empty selected_sales_item_ids, redirecting to [TIME_SLOT] marker",
        )
        return JSON.stringify({
          error:
            'DO NOT call this tool again. Instead, respond with ONLY the [TIME_SLOT] marker using the booking data already in the conversation. The widget component will fetch delivery windows automatically. Example: [TIME_SLOT]{"address_id": ' +
            (args.address_id || 0) +
            ', "car_ids": [], "license_plate": "", "sales_item_id": 0}[/TIME_SLOT]',
        })
      }
      return callMcpWithFallback(
        "delivery_window_get",
        {
          ...
          sales_item_ids: args.selected_sales_item_ids,
        },
        {
    ...
    case "create_shopping_cart": {
      if (mcpAuthToken) {
        try {
          ...
          const mcpResult = await callMcpTool("booking_create", {
            ...
            cars: args.car_id
              ? [{ car_id: args.car_id, selected_sales_item_ids: args.sales_item_ids }]
              : [],
            auth_token: mcpAuthToken,
            brand_domain: "noddi",
          })
          ...
          console.warn(
            "[executeTool] MCP booking_create failed, falling back:",
            (err as Error).message,
          )
        }
      }
      return executeBookingProxy({
        ...
        address_id: args.address_id,
        car_id: args.car_id,
        sales_item_ids: args.sales_item_ids,
        delivery_window_id: args.delivery_window_id,
      })
    }
    case "update_booking": {
      if (mcpAuthToken) {
        try {
          ...
          if (args.delivery_window_start)
            mcpArgs.delivery_window_starts_at = args.delivery_window_start
          ...
          console.warn(
            "[executeTool] MCP booking_update failed, falling back:",
            (err as Error).message,
          )
        }
      }
...
function selectModel(userMessage: string, conversationMessages: any[]): string {
  ...
  const frustration =
    /\b(frustr|irritert|sint|angry|upset|terrible|horrible|worst|verste|elendig|skandal)\b/i
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: actionFlowsData } = await supabase
      ...
      .select(
        "intent_key, label, description, trigger_phrases, requires_verification, flow_steps, is_active",
      )
    ...
    if (dbConversationId) {
      try {
        ...
        if (mcpAuthToken)
          console.log("[widget-ai-chat] Found MCP auth_token in conversation metadata")
      } catch {
        /* best effort */
      }
    }
    ...
    if (memoryIdentifier) {
      try {
        ...
        if (summary?.summary_text) {
          ...
          console.log(
            `[widget-ai-chat] Injected customer memory profile for ${identifierType}=${normalizedId} (${(memories || []).length} memories)`,
          )
        }
      } catch (e) {
        ...
        if (!useCache) {
          ...
          if (lookupResult.found) {
            ...
            supabase
              ...
              .then(() => {})
              .catch((err: any) =>
                console.warn("[widget-ai-chat] Failed to cache customer context:", err),
              )
          }
        }
      } catch (e) {
    ...
    const systemPrompt =
      buildSystemPrompt(language, isVerified, actionFlows, generalConfig) +
        ...
        if (m.role === "user" && m.content === "__VERIFIED__") {
          ...
          if (userIntent) {
            ...
            for (const flow of actionFlows) {
              ...
              const matches = triggers.some((p: string) => intentLower.includes(p.toLowerCase()))
              if (
                matches &&
                Array.isArray(flow.flow_steps) &&
                (flow.flow_steps as any[]).length > 0
              ) {
          ...
          return {
            role: "user",
            content: `I have just verified my phone number. Please look up my account and continue with the next step in the flow. REMEMBER: After lookup, you ALREADY KNOW if I am an existing customer — do NOT ask me. If I belong to multiple user groups, STOP and wait for me to select one — do NOT auto-select a group.${intentContext}${matchedFlowHint}`,
          }
        }
    ...
    while (maxIterations > 0) {
      ...
      if (!chatResp.ok) {
        ...
        await saveErrorDetails(
          supabase,
          dbConversationId,
          "openai_error",
          `Status ${chatResp.status}: ${errorText.slice(0, 500)}`,
        )
      ...
      if (!assistantMessage.tool_calls || assistantMessage.tool_calls.length === 0) {
        let rawReply =
          assistantMessage.content || "I apologize, I was unable to generate a response."
        ...
        if (reply.includes("[BOOKING_EDIT]")) {
          reply = reply.replace(
            /^.*(?:Gammel tid|Ny tid|gamle og nye|for bekreftelse|Bekrefter du|Her er endringene|gammel|ny).*$/gim,
            "",
          )
          reply = reply.replace(/\n{3,}/g, "\n\n").trim()
        }
        ...
        if (allToolsUsed.includes("search_knowledge_base") && !test) {
          try {
            ...
            console.error("[widget-ai-chat] Gap detection error:", e)
          }
        }
      ...
      for (const toolCall of assistantMessage.tool_calls) {
        ...
        if (toolCallCounts[toolName] >= maxCallsForTool) {
          console.warn(
            `[widget-ai-chat] Tool ${toolName} called ${toolCallCounts[toolName]} times, breaking loop`,
          )
          await saveErrorDetails(
            supabase,
            dbConversationId,
            "loop_break",
            `Tool ${toolName} called ${toolCallCounts[toolName]} times`,
          )
          loopBroken = true
          break
        }
        ...
        if (toolName === "cancel_booking") {
          const hasUserConfirmation = currentMessages.some(
            (m: any) =>
              m.role === "user" &&
              /\b(ja|yes|bekreft|confirm)\b/i.test(typeof m.content === "string" ? m.content : ""),
          )
      ...
      if (!loopBroken && eligibleCalls.length > 0) {
        ...
        console.log(
          `[widget-ai-chat] Executing ${eligibleCalls.length} tool call(s) in parallel, iteration ${8 - maxIterations}`,
        )
        const results = await Promise.all(
          eligibleCalls.map(async (toolCall: any) => {
            const toolName = toolCall.function.name
            const args = JSON.parse(toolCall.function.arguments)
            console.log(
              `[widget-ai-chat] Parallel tool call: ${toolName}(${toolCall.function.arguments})`,
            )
      ...
      if (loopBroken) {
        ...
        const answeredIds = new Set(
          currentMessages.filter((m: any) => m.role === "tool").map((m: any) => m.tool_call_id),
        )
          ...
          if (!answeredIds.has(tc.id)) {
            currentMessages.push({
              ...
              content: JSON.stringify({
                error:
                  "Tool call skipped due to safety limit. Use data already available in the conversation to answer the user. If the user wants to change time, output the [TIME_SLOT] marker.",
              }),
              tool_call_id: tc.id,
            })
          }
    ...
    console.log(
      '[widget-ai-chat] Loop exhausted/broken, making final forced-text call with tool_choice: "none"',
    )
    await saveErrorDetails(
      supabase,
      dbConversationId,
      "loop_exhaustion",
      "Max tool rounds exhausted, forcing text response",
    )
    try {
      ...
      if (!finalResp.ok) {
        ...
        await saveErrorDetails(
          ...
          dbConversationId,
          "recovery_call_error",
          `Status ${finalResp.status}: ${errBody.slice(0, 500)}`,
        )
      } else {
        ...
        if (finalContent && finalContent.trim().length > 0) {
          console.log(
            "[widget-ai-chat] Final forced-text response obtained, length:",
            finalContent.length,
          )
          let reply = await patchBookingSummary(
            finalContent,
            currentMessages,
            visitorPhone,
            visitorEmail,
    ...
    await saveErrorDetails(
      ...
      dbConversationId,
      "fallback_sent",
      "All recovery attempts failed, sent fallback message",
    )
    const fallback =
      language === "no"
...
const INQUIRY_TOOLS = new Set(["get_booking_details", "lookup_customer"])
...
async function quickQualityCheck(
  ...
  if (
    /^\[(?:BOOKING_SUMMARY|BOOKING_EDIT|BOOKING_CONFIRMED|ADDRESS_SEARCH|LICENSE_PLATE|TIME_SLOT|SERVICE_SELECT)\]/.test(
      reply.trim(),
    )
  ) {
    return
    ...
    if (!passed) {
      ...
      if (conversationId) {
        supabase
          ...
          .then(({ error: qErr }: any) => {
            if (qErr) console.warn("[widget-ai-chat] Review queue error:", qErr)
          })
      }
    }
  } catch (err) {
...
const GUARDRAIL_BILLING =
  /\b(refund|tilbakebetaling|faktura|invoice|kredittnota|credit\s*note|billing|betaling)\b/i
const GUARDRAIL_LEGAL =
  /\b(gdpr|personvern|privacy|klage|complaint|rettigheter|rights|datatilsynet)\b/i
...
function shouldForceHumanReview(reply: string, userMessage: string): boolean {
  ...
  return (
    GUARDRAIL_BILLING.test(combined) ||
...
function assessSelfConfidence(reply: string): number {
  ...
  const hedges =
    /\b(I'm not sure|I don't have|usikker|vet ikke|ikke sikker|unfortunately I cannot|dessverre|kan ikke finne|no relevant|ingen relevant)\b/i
  const strong =
    /\b(\[BOOKING_CONFIRMED\]|\[BOOKING_SUMMARY\]|create_shopping_cart|bestilling.*bekreftet)\b/i
...
async function computeConfidence(
  ...
  try {
    ...
    const initialW = { rag: 0.4, intent: 0.2, historical: 0.1, self: 0.3 }
    const matureW = { rag: 0.3, intent: 0.2, historical: 0.35, self: 0.15 }
    const w = {
    ...
    let composite =
      ragScore * w.rag + intentScore * w.intent + historicalRate * w.historical + selfScore * w.self
    ...
    console.log(
      `[widget-ai-chat] Confidence: ${(composite * 100).toFixed(1)}% (${intentCategory}) rag=${ragScore.toFixed(2)} intent=${intentScore.toFixed(2)} hist=${historicalRate.toFixed(2)} self=${selfScore.toFixed(2)}${forceHumanReview ? " [FORCED REVIEW]" : ""}`,
    )
  } catch (err) {
