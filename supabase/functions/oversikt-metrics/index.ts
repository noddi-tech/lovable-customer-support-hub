...
Deno.serve(async (req) => {
  ...
  let appsQ = sb
    .from("applications")
    .select(
      "id, applicant_id, position_id, current_stage_id, assigned_to, score, entered_stage_at, assigned_at, applied_at, updated_at",
    )
    .eq("organization_id", orgId)
  ...
  const assigneeIds = Array.from(
    new Set(allApps.map((a) => a.assigned_to).filter(Boolean) as string[]),
  )
  ...
  const stage_stalled = scopedApps
    .map((a) => {
      ...
      if (!s?.sla_enabled || !s.sla_hours) return null
      ...
      return {
        ...
        applicant_name: ap
          ? `${ap.first_name ?? ""} ${ap.last_name ?? ""}`.trim() || ap.email
          : "—",
        stage_id: s.id,
        stage_name: s.name,
        stage_color: s.color,
  ...
  const { data: fuApplicants } = fuApplicantIds.length
    ? await sb
        ...
        .in("id", fuApplicantIds)
    : { data: [] as any[] }
  const fuApBy = new Map((fuApplicants ?? []).map((x: any) => [x.id, x]))
  const fuAssigneeIds = Array.from(
    new Set((followups ?? []).map((f: any) => f.assigned_to).filter(Boolean)),
  )
  ...
  for (const a of allApps)
    stageCounts.set(a.current_stage_id, (stageCounts.get(a.current_stage_id) ?? 0) + 1)
  ...
  const inWindow = (iso: string | null | undefined) =>
    !iso || !sinceMs || new Date(iso).getTime() >= sinceMs
  ...
  const rejectedApps = allApps.filter(
    (a) => a.current_stage_id === "disqualified" && inWindow(a.updated_at),
  )
  ...
  return json({
    ...
    metrics: {
      ...
      new_applicants_by_source: Array.from(sourceBreakdown.entries()).map(([source, count]) => ({
        source,
        count,
      })),
