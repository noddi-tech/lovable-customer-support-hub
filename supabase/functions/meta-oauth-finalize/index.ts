...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: state } = await admin
      ...
      .select(
        "id, organization_id, long_lived_user_token, token_expires_at, oauth_user_id, oauth_user_name, mode, existing_integration_id",
      )
      .eq("id", stateId)
      .maybeSingle()
    ...
    if (!pageRes.ok || !pageData?.access_token) {
      return new Response(
        JSON.stringify({
          error:
            pageData?.error?.message ?? `Kunne ikke hente sidetilgang (HTTP ${pageRes.status})`,
        }),
        {
          status: 502,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: existing } = isReconnect
      ? await admin
          ...
          .eq("id", state.existing_integration_id)
          .maybeSingle()
      : await admin
          .from("recruitment_meta_integrations")
          .select("id, verify_token")
