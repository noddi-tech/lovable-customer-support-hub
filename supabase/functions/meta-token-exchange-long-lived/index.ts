...
function mapMetaError(msg: string | null | undefined, code?: number): string {
  ...
  if (
    ...
    m.includes("app_secret") ||
    m.includes("client_secret")
  ) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isValid) {
      return json(
        { error: "Det avledede side-tokenet ble ikke akseptert av Meta. Prøv igjen." },
        400,
      )
    }
    ...
    const statusMessage =
      missingScopes.length > 0 ? `Mangler tilganger: ${missingScopes.join(", ")}` : null
