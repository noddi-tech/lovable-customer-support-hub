...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!TOKEN) {
      const result: HealthResult = {
        auth: {
          ...
          error: "Ingen page access token lagret",
        },
        webhook: {
          subscription_active: false,
          last_event_at: integration.last_event_at,
          events_24h: { success: 0, failed: 0, duplicate: 0, invalid: 0 },
        },
        lead_retrieval: {
          can_fetch_leads: false,
          last_success_at: null,
          last_error: "Ingen token",
          tested_lead_id: null,
        },
    ...
    const [meRes, debugRes, subsRes] = await Promise.all([
      ...
      fetchJson(
        `${GRAPH}/debug_token?input_token=${encodeURIComponent(TOKEN)}&access_token=${encodeURIComponent(APP_ACCESS)}`,
      ),
      fetchJson(
        `${GRAPH}/${integration.page_id}/subscribed_apps?access_token=${encodeURIComponent(TOKEN)}`,
      ),
    ])
    ...
    if (subsRes.ok && Array.isArray(subsRes.data?.data)) {
      ...
      if (!leadgen_subscribed) {
        leadgen_subscribed = subsRes.data.data.some(
          (app: any) =>
    ...
    const last_event_at = candidates.length ? candidates.sort().reverse()[0] : null
