...
async function evaluateOrganization(supabase: any, orgId: string): Promise<number> {
  ...
  for (const f of feedback || []) {
    ...
    if (f.rating === "negative") {
      ...
      if (
        !negFeedbackByConv[f.conversation_id] ||
        f.created_at > negFeedbackByConv[f.conversation_id]
      ) {
  ...
  for (const [intent, data] of Object.entries(byIntent)) {
    ...
    const avgConfidence =
      totalResponses > 0 ? data.confidences.reduce((a, b) => a + b, 0) / totalResponses : 0
    ...
    // Upsert
    const { error: upsertErr } = await supabase.from("topic_autonomy_levels").upsert(
      {
    ...
    if (upsertErr) {
      ...
      if (newLevel !== currentLevel) {
        console.log(
          `[evaluate-autonomy] ${intent}: level ${currentLevel} → ${newLevel} (acc=${(acceptanceRate * 100).toFixed(0)}% eval=${(avgEvalScore * 100).toFixed(0)}% n=${totalResponses})`,
        )
      }
      updated++
    }
...
function computeLevel(
  ...
  const demotionThresholds: Record<number, number> = { 1: 0.6, 2: 0.75, 3: 0.85 }
  if (currentLevel > 0 && acceptanceRate < (demotionThresholds[currentLevel] || 0)) {
    const newLevel = currentLevel - 1
    console.log(
      `[evaluate-autonomy] Demoting ${currentLevel} → ${newLevel}: acceptance ${(acceptanceRate * 100).toFixed(0)}%`,
    )
    return newLevel
  }
  ...
  if (currentLevel === 0 && acceptanceRate > 0.7 && totalResponses >= 20 && avgEvalScore > 0.6) {
    targetLevel = 1
  }
  ...
  if (
    ...
    avgEvalScore > 0.8 &&
    noRecentNeg &&
    (overrideMax === null || overrideMax >= 3)
  ) {
    targetLevel = 3
  }
