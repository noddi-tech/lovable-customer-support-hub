  ...
  try {
    ...
    for (const account of emailAccounts) {
      try {
        ...
        console.log(
          `Checking ${account.email_address}: ${timeSinceLastSyncSeconds}s since last sync, interval is ${intervalSeconds}s (${intervalMinutes} minutes)`,
        )

        if (timeSinceLastSync < intervalMs) {
          console.log(
            `Skipping sync for ${account.email_address} - need to wait ${intervalSeconds - timeSinceLastSyncSeconds} more seconds`,
          )
          continue
        }

        console.log(
          `🚀 Triggering sync for ${account.email_address} - ${timeSinceLastSyncSeconds}s >= ${intervalSeconds}s threshold`,
        )
