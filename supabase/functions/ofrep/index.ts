...
function evaluate(flag: FlagRow, context: Record<string, unknown>) {
  ...
  const serve = (variant: string, reason: string) => {
    ...
    return {
      ...
      variant,
      reason,
      metadata: { valueType: flag.value_type },
    }
  }
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET" && path.startsWith("/ofrep/v1/configuration")) {
    return json({
      name: "support-hub-ofrep",
      capabilities: {
        cacheInvalidation: { polling: { enabled: true, minPollingIntervalMs: 30000 } },
        flagEvaluation: { supportedTypes: ["boolean", "string", "number", "object"] },
      },
    })
  }
  ...
  let query = supabase
    .from("feature_flags")
    .select(
      "key, organization_id, enabled, value_type, variants, default_variant, targeting, updated_at",
    )
  ...
  if (singleKey) {
    ...
    if (!flag) {
      return json(
        { key: singleKey, errorCode: "FLAG_NOT_FOUND", errorDetails: "Flag not found" },
        404,
      )
    }
