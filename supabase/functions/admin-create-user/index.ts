  ...
  try {
    ...
    if (!shouldSendInvite) {
      if (!password || password.length < 6) {
        return new Response(
          JSON.stringify({
            error: "Password must be at least 6 characters when not sending invite",
          }),
          {
            status: 400,
            headers: { "Content-Type": "application/json", ...corsHeaders },
    ...
    if (!isSuperAdmin) {
      ...
      if (orgsToAssign.length === 0) {
        return new Response(
          JSON.stringify({ error: "You can only add users to your own organization" }),
          {
            status: 403,
            headers: { "Content-Type": "application/json", ...corsHeaders },
          },
    ...
    if (shouldSendInvite) {
      ...
      const { error: inviteError } = await adminClient.from("organization_memberships").insert({
        email: email,
    ...
    if (upsertError) {
      ...
      return new Response(
        JSON.stringify({ error: `Database error saving new user: ${upsertError.message}` }),
        {
          status: 500,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        },
    ...
    console.log(
      ...
      orgsToAssign.length,
      "organization(s)",
      shouldSendInvite ? "(invite sent)" : "(password set)",
    )
    ...
    try {
      await adminClient.from("invite_email_logs").insert({
        ...
        metadata: {
          full_name,
          organizations: orgsToAssign.map((o) => ({ org_id: o.org_id, role: o.role })),
        },
      })
