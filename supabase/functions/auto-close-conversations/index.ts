...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: abandonData, error: abandonError } = await supabase.rpc(
    ...
    try {
      ...
      if (closedConvs && closedConvs.length > 0) {
        console.log(
          `[Auto-Close] Found ${closedConvs.length} agent conversations needing memory extraction`,
        )
        for (const conv of closedConvs) {
          supabase.functions
            .invoke("extract-customer-memories", {
              body: {
    ...
    try {
      ...
      const allStale = [...(staleAiConvs || []), ...(staleAiConvsEmail || [])].slice(0, 10)

      if (allStale.length > 0) {
        console.log(
          `[Auto-Close] Found ${allStale.length} widget AI conversations needing memory extraction`,
        )
        for (const conv of allStale) {
          supabase.functions
            ...
            .catch((err: any) =>
              console.warn(
                `[Auto-Close] Memory extraction failed for widget conv ${conv.id}:`,
                err,
              ),
            )
          widgetMemoryCount++
    ...
    if (agentMemoryCount > 0 || widgetMemoryCount > 0) {
      console.log(
        `[Auto-Close] Triggered memory extraction: ${agentMemoryCount} agent, ${widgetMemoryCount} widget AI`,
      )
    }
    ...
    try {
      ...
      for (const [category, days] of Object.entries(stalenessThresholds)) {
        ...
        if (stale && stale.length > 0) {
          ...
          console.log(
            `[Auto-Close] Deactivated ${stale.length} stale '${category}' knowledge entries`,
          )
        }
      }
    } catch (e) {
