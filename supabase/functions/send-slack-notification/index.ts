...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!enabledEvents.includes(event_type)) {
      ...
      return new Response(JSON.stringify({ skipped: true, reason: "Event type not enabled" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (inbox_id) {
      ...
      if (routing) {
        ...
        console.log(
          `Per-inbox routing: inbox ${inbox_id} → channel ${routing.channel_id} (secondary: ${routing.use_secondary_workspace})`,
        )
      }
    }
    ...
    if (!channelId) {
      ...
      return new Response(JSON.stringify({ skipped: true, reason: "No channel configured" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (inbox_id) {
      ...
      if (critRouting) {
        ...
        if (critRouting.critical_enabled === false) {
          ...
          criticalToken =
            critRouting.critical_use_secondary && integration.secondary_access_token
          ...
          criticalToken =
            critRouting.use_secondary_workspace && integration.secondary_access_token
              ? integration.secondary_access_token
              : integration.access_token
        }
      }
    ...
    if (config.critical_alerts_enabled && criticalChannelId && event_type !== "mention") {
      ...
      if (conversation_id) {
        ...
        if (existingAlert) {
          console.log(
            `⏭️ Critical alert already sent for conversation ${conversation_id} in last 24h, skipping`,
          )
          alreadyAlerted = true
        }
      }
      ...
      const aiThresholds =
        (integration.critical_ai_severity_thresholds as Record<string, number>) || {}
      ...
      if (shouldAlert) {
        const resolvedCategory =
          aiTriageResult?.category ?? inferCategoryFromKeyword(matchedKeyword)
        ...
        try {
          const fallbackText = mentionPrefix ? `${header.fallbackText}` : header.fallbackText
          ...
          if (!critResult.ok) {
            ...
            const triggerLabel = matchedKeyword
              ? `keyword: ${matchedKeyword}`
              : `AI: ${aiTriageResult?.category}`
            console.log(
              `Critical alert sent to ${criticalChannelId} (${triggerLabel}) ts=${critResult.ts}`,
            )
    ...
    return new Response(JSON.stringify({ success: true, ts: slackResult.ts }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    })
  } catch (error) {
