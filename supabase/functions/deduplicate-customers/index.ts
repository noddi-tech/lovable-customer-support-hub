...
serve(async (req) => {
  ...
  try {
    ...
    // Authorization: destructive merge/delete requires an admin of the target organization.
    ...
    console.log(
      `[Deduplicate] Starting deduplication for org: ${organizationId}, dryRun: ${dryRun}`,
    )
    ...
    result.duplicatesFound = duplicateGroups.reduce(
      (sum, [_, customers]) => sum + customers.length - 1,
      0,
    )

    console.log(
      `[Deduplicate] Found ${duplicateGroups.length} email addresses with duplicates (${result.duplicatesFound} duplicate records)`,
    )
    ...
    for (const [email, customers] of duplicateGroups) {
      try {
        ...
        console.log(
          `[Deduplicate] Merging ${email}: keeping ${keepCustomer.id} (${keepCustomer.full_name}), deleting ${duplicateIds.length} duplicates`,
        )
        ...
        for (const dupId of duplicateIds) {
          ...
          if (convError) {
            ...
            result.errors.push(
              `Failed to update conversations for customer ${dupId}: ${convError.message}`,
            )
            continue
          }
          ...
          console.log(
            `[Deduplicate] Updated ${updatedConvs?.length || 0} conversations from ${dupId} to ${keepCustomer.id}`,
          )
        }
    ...
    console.log(
      `[Deduplicate] Completed: ${result.customersMerged} emails merged, ${result.conversationsUpdated} conversations updated, ${result.customersDeleted} customers deleted`,
    )
