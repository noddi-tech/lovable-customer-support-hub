...
Return ONLY valid JSON, no markdown.`,
    ...
    const avgQualityScore =
      totalKnowledgeEntries > 0
        ? (knowledgeStats?.reduce((sum, e) => sum + e.quality_score, 0) ?? 0) /
    ...
    const avgSatisfaction =
      (outcomeStats || []).reduce(
        (sum: number, o: any) => sum + (o.customer_satisfaction_score || 0),
        0,
      ) / ((outcomeStats || []).length || 1)
    const avgReplyTime =
      (outcomeStats || []).reduce((sum: number, o: any) => sum + (o.reply_time_seconds || 0), 0) /
      ((outcomeStats || []).length || 1)
    ...
    const report = {
      ...
      knowledge_base: {
        ...
        acceptance_rate:
          totalUsage > 0 ? `${((totalAcceptance / totalUsage) * 100).toFixed(2)}%` : "0%",
      },
      response_tracking: {
      ...
      outcomes: {
        ...
        resolution_rate: outcomeStats?.length
          ? `${((resolvedCount / outcomeStats.length) * 100).toFixed(2)}%`
          : "0%",
