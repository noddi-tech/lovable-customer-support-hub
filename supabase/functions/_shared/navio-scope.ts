...
function hasIamGraph(claims: Record<string, unknown>): boolean {
  return (
    stringList(claims.navio_permissions).length > 0 || stringList(claims.navio_roles).length > 0
  )
}
...
function isClaimsNetworkSuperuser(claims: Record<string, unknown>): boolean {
  ...
  return activeRoles.includes("owner_superuser") || activeRoles.includes("viewer_superuser")
}
