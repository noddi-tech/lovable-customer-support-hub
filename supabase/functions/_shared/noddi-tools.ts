...
import { API_BASE, extractPlateString, toOsloTime } from "./chat-utils.ts"
import { navioSourceHeaders } from "./navio-source.ts"

export async function executeLookupCustomer(
  phone?: string,
  email?: string,
  specifiedUserGroupId?: number,
): Promise<string> {
  ...
  try {
    ...
    if (!specifiedUserGroupId && userGroups.length > 1) {
      ...
      return JSON.stringify({
        ...
        customer: {
          name:
            `${noddihUser.first_name || ""} ${noddihUser.last_name || ""}`.trim() ||
            noddihUser.name ||
            "",
    ...
    for (const ub of lookupData.unpaid_bookings || []) {
      if (
        ub?.id &&
        !seenBookingIds.has(ub.id) &&
        (!ub.user_group_id || ub.user_group_id === userGroupId)
      ) {
        bookings.push(ub)
        seenBookingIds.add(ub.id)
      }
    ...
    console.log(
      `[lookup] Extracted ${bookings.length} bookings from customer-lookup-support response`,
    )
    ...
    if (userGroupId) {
      try {
        ...
        if (bfcResp.ok) {
          ...
          console.log(
            `[lookup] Full bookings from bookings-for-customer: ${results.length} results`,
          )
          if (results.length > 0) {
            console.log("[lookup] Sample booking keys:", Object.keys(results[0]))
            console.log(
              "[lookup] Sample booking car fields:",
              JSON.stringify({
                car: results[0].car,
                cars: results[0].cars,
    ...
    const name =
      `${noddihUser.first_name || ""} ${noddihUser.last_name || ""}`.trim() || noddihUser.name || ""
    ...
    for (const b of bookings) {
      if (b.address?.id) {
        ...
        const label = `${streetName} ${streetNum}, ${zip} ${city}`
          .replace(/\s+/g, " ")
          .trim()
          .replace(/^,|,$/g, "")
          .trim()
      ...
      if (b.car?.id) {
        storedCars.set(b.car.id, {
          ...
          make: b.car.make || "",
          model: b.car.model || "",
          license_plate: extractPlateString(
      ...
      if (Array.isArray((group as any).addresses)) {
        for (const addr of (group as any).addresses) {
          if (addr?.id && !storedAddresses.has(addr.id)) {
            const label =
              `${addr.street_name || ""} ${addr.street_number || ""}, ${addr.zip_code || ""} ${addr.city || ""}`
                .replace(/\s+/g, " ")
                .trim()
                .replace(/^,|,$/g, "")
                .trim()
    ...
    return JSON.stringify({
      ...
      customer: {
        ...
        allUserGroups: userGroups.map((g: any) => ({
          id: g.id,
          name: g.name,
      ...
      bookings: (() => {
        ...
        const mappedBookings = bookings
              ...
              typeof rawStatus === "number"
                ...
                : typeof rawStatus === "string"
                  ...
                  : typeof rawStatus === "object" && rawStatus !== null
                    ? rawStatus.name ||
                      rawStatus.slug ||
                      STATUS_MAP[rawStatus.id ?? rawStatus.value] ||
                      rawStatus.label ||
                      String(rawStatus.id ?? rawStatus.value ?? "")
                    : ""
            ).toLowerCase()
            if (
              ["completed", "cancelled", "canceled", "no_show", "expired", "draft"].includes(status)
            ) {
              return false
            }
            const endTime =
              b.end_time ||
              b.delivery_window_ends_at ||
              b.delivery_window?.ends_at ||
              b.deliveryWindowEndsAt
          ...
          .map((b: any) => {
            ...
            const statusStr =
              typeof rawSt === "number"
                ? STATUS_MAP[rawSt] || String(rawSt)
                : typeof rawSt === "string"
                  ? rawSt
                  : typeof rawSt === "object" && rawSt !== null
                    ? rawSt.name ||
                      rawSt.slug ||
                      STATUS_MAP[rawSt.id ?? rawSt.value] ||
                      rawSt.label ||
                      ""
                    : ""
            const startFull = toOsloTime(
              b.start_time ||
                b.scheduled_at ||
                b.delivery_window_starts_at ||
                b.delivery_window?.starts_at ||
                b.deliveryWindowStartsAt ||
                "",
            )
            const endFull = toOsloTime(
              b.end_time ||
                b.delivery_window_ends_at ||
                b.delivery_window?.ends_at ||
                b.deliveryWindowEndsAt ||
            ...
            return {
              ...
              vehicle: (() => {
                ...
                if (c) {
                  ...
                  return (
                    `${c.make || ""} ${c.model || ""} ${plate ? `(${plate})` : ""}`.trim() || null
                  )
                }
                if (Array.isArray(b.booking_items_car) && b.booking_items_car[0]?.car) {
                  ...
                  return (
                    `${bic.make || ""} ${bic.model || ""} ${plate ? `(${plate})` : ""}`.trim() ||
                    null
                  )
                }
                ...
                if (carId && storedCars.has(carId)) {
                  ...
                  return (
                    `${sc.make} ${sc.model} ${sc.license_plate ? `(${sc.license_plate})` : ""}`.trim() ||
                    null
                  )
                }
              ...
              car_ids: Array.isArray(b.cars)
                ? b.cars.map((c: any) => c.id).filter(Boolean)
                : b.car?.id
                  ? [b.car.id]
                  : [],
              license_plate:
                ...
                (Array.isArray(b.cars) && b.cars[0]
                  ? extractPlateString(
                      b.cars[0].license_plate_number ||
                        b.cars[0].license_plate ||
                        b.cars[0].registration,
                    )
                  : ""),
            }
          })
...
export async function executeGetBookingDetails(bookingId: number): Promise<string> {
  ...
  try {
    ...
    if (!resp.ok) {
      console.error(
        `[executeGetBookingDetails] Failed for booking_id=${bookingId}: status=${resp.status}`,
      )
      return JSON.stringify({
        error: resp.status === 404 ? "Booking not found" : `Booking lookup failed (${resp.status})`,
      })
    }
    ...
    const statusStr2 =
      typeof rawSt2 === "string"
        ? rawSt2
        : typeof rawSt2 === "object" && rawSt2 !== null
          ? rawSt2.name || rawSt2.slug || ""
          : ""
    ...
    return JSON.stringify({
      ...
      services:
        booking.order_lines?.map((ol: any) => ({
          name: ol.service_name || ol.name,
          price: ol.price,
        })) || [],
      sales_item_ids:
        booking.order_lines?.map((ol: any) => ol.sales_item_id || ol.id).filter(Boolean) || [],
      ...
      car_ids: Array.isArray(booking.cars)
        ? booking.cars.map((c: any) => c.id).filter(Boolean)
        : booking.car?.id
          ? [booking.car.id]
          : [],
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 400)
        return JSON.stringify({
          success: false,
          error: "The requested time slot is not available. Please try a different time.",
        })
      return JSON.stringify({
        success: false,
        error: "Rescheduling failed. Please try again or contact support.",
      })
    }
