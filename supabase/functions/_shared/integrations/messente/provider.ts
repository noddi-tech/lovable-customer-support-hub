  ...
  return {
    ...
    parseStatusUpdate(body: unknown): SmsStatusUpdate {
      ...
      return {
        ...
        segments: b.price_info?.parts_count ? Number(b.price_info.parts_count) : undefined,
      }
    },
...
async function validateMessenteSignature(req: Request, body: string): Promise<void> {
  ...
  if (!provided) {
    console.warn(
      "[messente] inbound webhook missing signature header — accepting (configure signing in dashboard)",
    )
    return
  }
