...
export async function collectApplicantData(
  ...
  const [
    ...
    safeSelect<any[]>(
      supabase.from("applicant_notes").select("*").eq("applicant_id", applicantId) as any,
      "notes",
    ),
    ...
    applicationIds.length
      ...
      : Promise.resolve([]),
    safeSelect<any[]>(
      supabase.from("candidate_form_tokens").select("*").eq("applicant_id", applicantId) as any,
      "form_tokens",
    ),
    applicationIds.length
      ? safeSelect<any[]>(
    ...
    safeSelect<any[]>(
      supabase.from("applicant_conversations").select("*").eq("applicant_id", applicantId) as any,
      "conversations",
    ),
    safeSelect<any[]>(
      supabase
