...
export async function patchBookingSummary(
  reply: string,
  messages: any[],
  visitorPhone?: string,
  visitorEmail?: string,
  ...
  try {
    ...
    console.warn(
      "[patchBookingSummary] Failed to parse BOOKING_SUMMARY JSON, attempting reconstruction from context",
    )
    summaryData = {}
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (msg.role === "user" && typeof msg.content === "string") {
        try {
          ...
          if (actionData.address_id && !summaryData.address_id)
            summaryData.address_id = actionData.address_id
          if (actionData.license_plate && !summaryData.license_plate)
            summaryData.license_plate = actionData.license_plate
          if (actionData.sales_item_ids && !summaryData.sales_item_ids)
            summaryData.sales_item_ids = actionData.sales_item_ids
          if (actionData.sales_item_id && !summaryData.sales_item_ids)
            summaryData.sales_item_ids = [actionData.sales_item_id]
        } catch {
          /* not JSON */
        }
      }
  ...
  if (visitorPhone || visitorEmail) {
    ...
    if (!foundInContext) {
      console.log(
        "[patchBookingSummary] No cached customer data, performing fresh lookup, selectedGroupId:",
        selectedGroupId,
      )
      try {
    ...
    console.warn(
      "[patchBookingSummary] Missing customer IDs but no visitorPhone/visitorEmail available",
    )
  }
  ...
  if (patched) {
    ...
    return (
      ...
      patchedJson +
      closingMarker +
      reply.slice(endIdx + closingMarker.length)
    )
  }
...
export function patchYesNo(reply: string, messages?: any[]): string {
  if (reply.includes("[YES_NO]") && !reply.includes("[/YES_NO]")) {
    ...
    if (qMatch) {
      ...
      return [before, `[YES_NO]${qMatch[1]}[/YES_NO]`].filter((s) => s.length > 0).join("\n")
    }
  }
  if (reply.includes("[YES_NO]") && reply.includes("[/YES_NO]")) {
  ...
  if (messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      try {
        const p = JSON.parse(messages[i].content)
        if (p.delivery_window_id) return reply
      } catch {}
      break
    }
  }
  ...
  if (!patterns.some((p) => p.test(reply))) {
    const confirmKeywords =
      /\b(riktig|korrekt|bekrefte|endre|correct|confirm|want to|ønsker|stemmer|passer|sikker)\b/i
    ...
    if (msg.role === "tool" && typeof msg.content === "string") {
      try {
        ...
        if (
          r.success &&
          (r.action === "cancelled" ||
            r.message?.toLowerCase().includes("cancelled") ||
            r.message?.toLowerCase().includes("kansellert"))
        )
          return true
      } catch {}
    }
...
export function patchBookingInfo(reply: string, messages: any[]): string {
    ...
    if (msg.role === "tool" && typeof msg.content === "string") {
      try {
        ...
        if (toolResult.bookings && toolResult.bookings.length > 1) {
          ...
          console.log(
            "[patchBookingInfo] Multiple bookings detected, showing BOOKING_SELECT carousel",
          )
          return marker
        }
  ...
  console.log(
    "[patchBookingInfo] CONTEXT-BASED trigger: found booking data in tool results, injecting [BOOKING_INFO]",
  )
  console.log(
    "[patchBookingInfo] bookingData keys:",
    ...
    "has order_lines:",
    !!bookingData.order_lines,
    "has items:",
    !!bookingData.items,
    "has sales_items:",
    !!bookingData.sales_items,
  )
  ...
  if (bookingData.address) {
    info.address =
      typeof bookingData.address === "string"
        ? bookingData.address
        : bookingData.address.address || bookingData.address.full_address || ""
  }
  ...
  if (bookingData.vehicle) {
    info.car =
      typeof bookingData.vehicle === "string"
        ? bookingData.vehicle
        : `${bookingData.vehicle.make || ""} ${bookingData.vehicle.model || ""} (${bookingData.vehicle.licensePlate || ""})`.trim()
    ...
    const plate = extractPlateString(
      car.license_plate_number || car.license_plate || car.registration,
    )
  ...
  if (
    !info.car &&
    Array.isArray(bookingData.booking_items_car) &&
    bookingData.booking_items_car[0]?.car
  ) {
    const bic = bookingData.booking_items_car[0].car
    const plate = extractPlateString(
      bic.license_plate_number || bic.license_plate || bic.registration,
    )
  ...
  if (!info.time && bookingData.delivery_window_starts_at && bookingData.delivery_window_ends_at) {
    try {
      ...
      if (!info.date)
        info.date = s.toLocaleDateString("nb-NO", {
          day: "numeric",
          month: "short",
          year: "numeric",
          timeZone: "Europe/Oslo",
  ...
  if (
    !info.service &&
    Array.isArray(bookingData.service_categories) &&
    bookingData.service_categories.length > 0
  ) {
    const catNames = bookingData.service_categories
      .map((sc: any) => sc.name || sc.label || "")
      .filter(Boolean)
  ...
  if (!info.service && Array.isArray(bookingData.booking_items_car)) {
    ...
    for (const bic of bookingData.booking_items_car) {
      if (Array.isArray(bic.sales_items)) {
        for (const si of bic.sales_items) {
          if (si.name) allSiNames.push(si.name)
        }
      }
    }
  ...
  if (
    !info.address &&
    bookingData.address &&
    typeof bookingData.address === "object" &&
    !bookingData.address.full_address &&
    !bookingData.address.address
  ) {
    const a = bookingData.address
    const addr =
      `${a.street_name || ""} ${a.street_number || ""}, ${a.zip_code || ""} ${a.city || ""}`
        .replace(/\s+/g, " ")
        .trim()
        .replace(/^,|,$/g, "")
        .trim()
  ...
  cleaned = cleaned.replace(
    /^.*(?:ikke fikk tilgang|couldn't access|ikke finne detalj|kunne ikke hente).*$/gim,
    "",
  )
  cleaned = cleaned.replace(/^.*(?:Her er|detaljer).*:?\s*$/gim, "")
  cleaned = cleaned.replace(
    /^.*(?:planlagt bestilling|har en bestilling|din bestilling|bestilling den).*$/gim,
    "",
  )
  cleaned = cleaned.replace(/\n{3,}/g, "\n\n").trim()
...
export function patchActionMenu(reply: string, messages: any[]): string {
  ...
  cleaned = cleaned.replace(
    /^.*(?:Vil du endre|Hva ønsker du|What would you like|What do you want to change|Vil du gjøre endringer|Hva vil du).*$/gim,
    "",
  )
  cleaned = cleaned.replace(/\n{3,}/g, "\n\n").trim()
    ...
    if (msg.role === "tool") {
      try {
        ...
        if (parsed.needs_group_selection && parsed.user_groups) {
          ...
          console.log(
            "[patchGroupSelect] Replacing reply with group select prompt for",
            parsed.user_groups.length,
            "groups",
          )
...
export function patchBookingConfirmed(reply: string, messages: any[]): string {
        ...
        try {
          ...
          if (toolResult.booking) {
            ...
            if (!data.car && b.car && typeof b.car === "object") {
              const plate = extractPlateString(
                b.car.license_plate_number || b.car.license_plate || b.car.registration,
              )
              data.car =
                `${b.car.make || ""} ${b.car.model || ""} ${plate ? `(${plate})` : ""}`.trim()
            }
            if (!data.car && b.cars?.[0]) {
              const c = b.cars[0]
              const plate = extractPlateString(
                c.license_plate_number || c.license_plate || c.registration,
              )
          ...
          if (booking) {
            ...
            if (!data.date && booking.scheduledAt)
              data.date = (booking.scheduledAt.split(",")[0] || booking.scheduledAt).trim()
          ...
          if (
            !data.booking_id &&
            (toolResult.booking_id || (toolResult.id && !toolResult.scheduledAt))
          ) {
      ...
      if (r.booking && (r.booking.id || r.booking.reference)) {
        ...
        if (!data.time && b.delivery_window_starts_at && b.delivery_window_ends_at) {
          try {
            ...
            if (!data.date)
              data.date = s.toLocaleDateString("nb-NO", {
                day: "numeric",
                month: "short",
                year: "numeric",
                timeZone: "Europe/Oslo",
  ...
  if (hasUpdateResult && Object.keys(data).length > 0) {
    console.log(
      "[patchBookingConfirmed] CONTEXT-BASED: update_booking detected, injecting [BOOKING_CONFIRMED]",
    )
    let cleaned = reply
    cleaned = cleaned.replace(
      /^.*(?:oppdatert|updated|endret|changed|bekreftet|confirmed).*$/gim,
      "",
    )
...
export function patchTimeSlotConfirmToEdit(reply: string, messages: any[]): string {
  ...
  const newTime =
    timeSlotSelection.start_time && timeSlotSelection.end_time
...
export async function patchBookingEdit(
  reply: string,
  messages: any[],
  visitorPhone?: string,
  visitorEmail?: string,
    ...
    if (!realBookingId) {
      ...
      if (phone || email) {
        try {
          ...
          console.error("[patchBookingEdit] Fresh lookup failed:", e)
        }
      }
    }
  }
  ...
  if (realBookingId) {
    console.log(
      "[patchBookingEdit] Setting booking_id to:",
      realBookingId,
      "(was:",
      editData.booking_id,
      ")",
  ...
  if (
    changes.delivery_window_id &&
    (!changes.delivery_window_start || !changes.delivery_window_end)
  ) {
    for (let i = messages.length - 1; i >= 0; i--) {
      const msg = messages[i]
      ...
      try {
        ...
        if (
          sel.delivery_window_id === changes.delivery_window_id &&
          sel.start_time &&
          sel.end_time
        ) {
          ...
          console.log(
            "[patchBookingEdit] Injected start/end from conversation:",
            sel.start_time,
            sel.end_time,
          )
          break
      ...
      if (msg.role === "tool" && typeof msg.content === "string") {
        try {
          ...
          const oldStart =
            ...
            booking.start_time ||
            booking.delivery_window?.starts_at
          if (oldStart) {
