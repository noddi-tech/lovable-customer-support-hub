 ...
 */
export const FEEDBACK_FOOTER_TEXT = "👍 nyttig · 👎 falsk alarm · 🔇 demp denne triggeren i 7 dager"

export function buildFeedbackFooterBlock(): {
  type: "context"
...
const KEYWORD_CATEGORY_HINTS: Array<{ patterns: RegExp; category: CriticalCategory }> = [
  {
    patterns: /\b(payment|billing|betalingsfeil|betaling feil|faktura|belastet)\b/i,
    category: "billing_issue",
  },
  { patterns: /\b(legal|advokat|sue|stevning|forbrukerrådet)\b/i, category: "legal_threat" },
  {
    patterns: /\b(injury|skadet person|safety|farlig|ulykke|brann)\b/i,
    category: "safety_concern",
  },
  {
    patterns: /\b(angry|frustrated|elendig|forferdelig|verste|aldri mer)\b/i,
  ...
  {
    patterns:
      /\b(app|appen|krasj|krasjer|logge inn|innlogging|nede|outage|broken|tom side|blank side|betalingsside|innloggingsside)\b/i,
    category: "app_failure",
  },
  // Physical service quality (ops): noise, damage caused by service, faulty installation, poor workmanship
  {
    patterns:
      /\b(metallisk|lyd fra|skade på|skade etter|skadet etter|skadet under|feilmontert|feil montert|dårlig utført|reklamasjon|ødelagt etter)\b/i,
