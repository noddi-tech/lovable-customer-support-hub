...
export function buildStructuredMessageId(conversationId: string, fromEmail: string): string {
  ...
  const rand = (crypto as unknown as { randomUUID?: () => string }).randomUUID?.().slice(0, 12) ||
...
export function extractConversationIdFromBody(...bodies: Array<string | null | undefined>): string | null {
