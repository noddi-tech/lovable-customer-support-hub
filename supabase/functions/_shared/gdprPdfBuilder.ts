...
export async function buildGdprPdf(
  ...
  const fullName =
    `${applicant.first_name ?? ""} ${applicant.last_name ?? ""}`.trim() || "(uten navn)"
  ...
  drawKeyValue(
    c,
    "GDPR-samtykke",
    applicant.gdpr_consent ? `Ja (${fmtDate(applicant.gdpr_consent_at)})` : "Nei",
  )
  ...
  for (const s of data.scoring_history as any[]) {
    drawLine(c, `${fmtDate(s.created_at)} — Score: ${s.score ?? "-"} (modell: ${s.model ?? "-"})`, {
      bold: true,
    })
  ...
  for (const m of data.messages as any[]) {
    ...
    drawLine(c, `${fmtDate(m.created_at)} [${m.sender_type ?? "?"}]${subject}`, { bold: true })
    if (m.content) {
  ...
  c.y -= LINE
  c.page.drawText(`Generert ${fmtDate(data.exported_at)} for GDPR-forespørsel ${meta.requestId}`, {
    x: MARGIN,
    y: c.y,
    size: 8,
    font,
