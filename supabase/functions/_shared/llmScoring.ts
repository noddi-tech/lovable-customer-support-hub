...
function buildPrompt(input: ScoringInput): string {
  ...
  const applicantBlock = [
    ...
    applicant.certifications?.length
      ? `Sertifiseringer: ${applicant.certifications.join(", ")}`
      : null,
    applicant.drivers_license_classes?.length
      ? `Førerkort: ${applicant.drivers_license_classes.join(", ")}`
      : null,
  ...
  const customFieldsBlock = custom_field_values.length
    ? custom_field_values
        .map(
          (f) =>
            `- ${f.field_name}: ${typeof f.value === "object" ? JSON.stringify(f.value) : String(f.value ?? "")}`,
        )
        .join("\n")
    : "(ingen)"
  ...
  const stageBlock = stage
    ? `Nåværende steg: ${stage.name}${stage.description ? ` — ${stage.description}` : ""}`
    : ""
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "Du er en upartisk rekrutteringsekspert. Returner alltid strukturert JSON.",
        },
