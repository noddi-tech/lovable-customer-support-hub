...
async function fetchJson(url: string) {
  try {
    ...
    return {
      ...
      status: r.status,
      data,
      error: r.ok ? null : (data?.error?.message ?? `HTTP ${r.status}`),
    }
  } catch (e: any) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      candidateToken.length < 20
    ) {
      return new Response(
        JSON.stringify({ error: "integration_id and candidate_token required" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const [meRes, debugRes] = await Promise.all([
      ...
      fetchJson(
        `${GRAPH}/debug_token?input_token=${encodeURIComponent(candidateToken)}&access_token=${encodeURIComponent(APP_ACCESS)}`,
      ),
    ])
