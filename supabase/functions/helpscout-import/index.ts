...
async function fetchHelpScout(accessToken: string, endpoint: string, retries = 3): Promise<any> {
  ...
  if (email) {
    ...
    if (existingByEmail) {
      ...
      if (
        ...
        (!existingByEmail.full_name || existingByEmail.full_name.toLowerCase() === email)
      ) {
        await supabase
          .from("customers")
        ...
        console.log(
          `[HelpScout Import] Updated customer name: ${existingByEmail.id} -> ${fullName}`,
        )
      }
...
async function importConversation(
  ...
  try {
    ...
    // Import customer
    const { customerId, isNew } = await importCustomer(
      supabase,
      organizationId,
    ...
    const threadsData = await fetchHelpScout(
      accessToken,
    ...
    progress.errors.push(
      `Error importing conversation ${conversation.id}: ${error instanceof Error ? error.message : String(error)}`,
    )
  ...
  try {
    ...
    if (test) {
      try {
        ...
        return new Response(JSON.stringify({ success: true, message: "Credentials are valid" }), {
          status: 200,
    ...
    if (resume && existingJobId) {
      ...
      console.log(
        `Resuming import job: ${jobId} at mailbox ${checkpoint?.currentMailboxIndex}, page ${checkpoint?.currentPage}`,
      )
    } else {
        ...
        try {
          ...
          if (checkpoint) {
            ...
            if (existingJob) {
              progress.totalMailboxes =
                existingJob.total_mailboxes || checkpoint.mailboxIds?.length || 0
              ...
              progress.conversationsProcessed =
                existingJob.metadata?.progress?.conversationsProcessed || 0
              progress.conversationsSkipped =
                existingJob.metadata?.progress?.conversationsSkipped || 0
          ...
          for (let mIdx = startMailboxIndex; mIdx < targetMailboxes.length; mIdx++) {
            ...
            if (completedMailboxIds.has(mailbox.id)) {
              console.log(
                `Mailbox ${mailbox.name} (${mailbox.id}) already completed, skipping to next`,
              )
              continue
            }
            ...
            if (!inboxId) {
              if (targetInboxId === "create_new") {
                console.log(
                  `Creating new inbox for mailbox ${mailbox.name} (${mailbox.id}), org: ${effectiveOrganizationId}`,
                )
                ...
                if (inboxError || !newInbox) {
                  ...
                  progress.errors.push(
                    `Failed to create inbox for mailbox ${mailbox.name}: ${inboxError?.message || "Unknown error"}`,
                  )
                  continue
                }
            ...
            while (hasMore) {
              ...
              if (shouldPause()) {
                console.log(
                  `Pausing at mailbox ${mIdx}, page ${page} after ${Date.now() - startTime}ms`,
                )
              ...
              for (const conversation of conversations) {
                ...
                if (
                  mIdx === startMailboxIndex &&
                  page === startPage &&
                  skipConvIds.has(conversation.id)
                ) {
                ...
                if (shouldPause()) {
                  console.log(
                    `Pausing mid-page at mailbox ${mIdx}, page ${page}, conv ${conversation.id}`,
                  )
                ...
                if (progress.conversationsProcessed % 5 === 0) {
                  ...
                  console.log(
                    `Progress: ${progress.conversationsProcessed} processed (${progress.conversationsImported} new, ${progress.conversationsSkipped} skipped)/${progress.totalConversations}`,
                  )
                }
              }
              ...
              if (!hasMore || (consecutiveSkippedPages >= 3 && conversations.length > 0)) {
                ...
                if (!hasMore) {
                  ...
                  console.log(
                    `Mailbox ${mailbox.name} completed early - ${consecutiveSkippedPages} consecutive pages with all conversations already imported`,
                  )
                }
