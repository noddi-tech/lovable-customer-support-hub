...
| `enableAiChat`          | `boolean`                         | —               | Host gate for the AI assistant button on the widget home screen. `false` always hides it; `true` shows it only if the admin widget config also enables the AI assistant; omit to keep the admin config alone. Works on `init` and `update`.                                                              |
