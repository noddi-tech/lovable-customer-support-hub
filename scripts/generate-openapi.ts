...
function detectBodyFields(source: string): string[] {
  ...
  for (const m of source.matchAll(/const\s*\{([^}]+)\}\s*(?::[^=]+)?=\s*(?:await\s+)?(?:req\.json\(\)|body|payload)/g)) {
...
const names = readdirSync(FUNCTIONS_DIR)
  .filter((n) => !n.startsWith('_') && !n.includes('.') && statSync(join(FUNCTIONS_DIR, n)).isDirectory())
  .sort();
...
for (const name of names) {
  ...
  const description = [
    describe(source) || 'No description available. Add a leading comment to the edge function to document it.',
    '',
    requiresAuth
...
const spec = {
  ...
  servers: [{ url: '{supabaseUrl}', variables: { supabaseUrl: { default: 'https://<project-ref>.supabase.co' } } }],
