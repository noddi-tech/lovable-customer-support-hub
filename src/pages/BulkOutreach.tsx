...
import { type Recipient, RecipientReview } from "@/components/bulk-outreach/RecipientReview"
...
export default function BulkOutreach() {
  ...
  const [sendResult, setSendResult] = useState<{ sent_count: number; failed_count: number } | null>(
    null,
  )

  const organizationId = profile?.organization_id

  const handlePlateLookup = useCallback(
    async (plates: string[]) => {
      if (!organizationId) {
      ...
      try {
        ...
        toast.success(
          `Looked up ${plates.length} plates, found ${results.filter((r) => r.matched).length} matches`,
        )
      } catch (err) {
            ...
            <Tabs defaultValue="plates">
              ...
              {recipients.length > 0 && (
                <p className="mt-4 text-sm text-muted-foreground">
                  {recipients.length} plates loaded ({recipients.filter((r) => r.matched).length}{" "}
                  matched). Click Next to review.
                </p>
              )}
            </Tabs>
          )}
