...
import { User, Clock, AlertTriangle, CheckCircle, Tag } from 'lucide-react';
...
const ServiceTicketsPage: React.FC = () => {
  const renderDetail = (ticketId: string) => {
    const ticket = mockTicketDetails[ticketId as keyof typeof mockTicketDetails];
  ...
  const handleReply = (ticketId: string, message: string) => {
    console.log('Adding comment to ticket:', ticketId, 'Message:', message);
    // Here you would typically save the comment to your backend
  };

  return (
    <InboxLayout
      conversations={mockServiceTickets}
