...
export const captureBodyLockSnapshot = (): BodyLockSnapshot => {
  ...
  const overlays = document.querySelectorAll(
    "[data-radix-popper-content-wrapper], [data-state][data-radix-overlay], [data-radix-dialog-overlay], [data-radix-alert-dialog-overlay]",
  )
  const contents = document.querySelectorAll(
    "[data-radix-dialog-content], [data-radix-alert-dialog-content], [data-radix-dropdown-menu-content]",
  )
...
export const noteDebug = (event: string, payload: NoteEventPayload = {}, source?: string): void => {
...
export const scheduleInteractionLockWatchdog = (
  ...
  Promise.resolve().then(() => {
    ...
    ;(locked ? noteDebugWarn : noteDebug)(
      event,
      { phase: "tick0", ...context, snapshot: snap },
      source,
    )
  })
  ...
  setTimeout(() => {
    ...
    ;(locked ? noteDebugWarn : noteDebug)(
      event,
      { phase: "tick250", ...context, snapshot: snap },
      source,
    )
  }, 250)
}
