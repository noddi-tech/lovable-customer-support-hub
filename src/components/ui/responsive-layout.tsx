...
const ResponsiveLayout = React.forwardRef<HTMLDivElement, ResponsiveLayoutProps>(
  (
    { className, children, header, sidebar, bottomTabs, leftDrawer, rightDrawer, ...props },
    ref,
  ) => {
    ...
    if (sidebar) {
      return (
        <SidebarProvider>
          <div ref={ref} className={cn("app-root", ...rootClasses)} {...props}>
            {/* Header */}
            ...
            <div
              className={cn(
                "app-main flex min-h-0",
                ...mainClasses,
                isMobile && bottomTabs && "pb-16",
              )}
            >
              {/* Sidebar */}
              <Sidebar className="border-r">{sidebar}</Sidebar>

              {/* Main Content */}
              <SidebarInset className="flex-1 min-h-0 m-0">{children}</SidebarInset>
            </div>
            ...
            {/* iPhone Bottom Tabs */}
            {bottomTabs && isMobile && <div className="bottom-tabs">{bottomTabs}</div>}
          </div>
        </SidebarProvider>
      )
    }
