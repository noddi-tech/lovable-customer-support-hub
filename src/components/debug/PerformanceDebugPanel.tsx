      ...
      <Card className="shadow-xl bg-background/95 backdrop-blur border-2">
        ...
        <div className="flex items-center justify-between p-3 border-b bg-muted/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">Press Ctrl+Shift+D to toggle</span>
          </div>
          <div className="flex gap-1">
            <Button size="sm" variant="ghost" onClick={() => logger.clearDebugMetrics()}>
              Clear
            </Button>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              ...
              <div className="p-2 bg-muted/30 rounded">
                ...
                <div className="text-lg font-bold">{metrics?.parseCache?.size || 0}</div>
              </div>
            </div>
            {metrics?.parseCache && (
              <div className="mt-2 p-2 bg-muted/30 rounded text-xs">
                ...
                <div className="font-mono text-sm">
                  {metrics.parseCache.hits + metrics.parseCache.misses > 0
                    ? (
                        (metrics.parseCache.hits /
                          (metrics.parseCache.hits + metrics.parseCache.misses)) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
                ...
                metrics.slowOperations
                  ...
                  .map((op: any, idx: number) => (
                    <div key={idx} className="p-2 bg-destructive/10 rounded">
                      ...
                      {op.component && (
                        <div className="text-muted-foreground text-xs mt-1">in {op.component}</div>
                      )}
                    </div>
                  ))
              ) : (
