...
import { type CustomerListRow, customerSortName, useCustomersList } from "@/hooks/useCustomersList"
...
export function CaseCustomerPicker({ value, onChange, disabled }: CaseCustomerPickerProps) {
  ...
  const sorted = [...customers].sort((a, b) =>
    customerSortName(a).localeCompare(customerSortName(b)),
  )
  const term = search.trim()
  const canCreate =
    term.length > 1 && !sorted.some((c) => labelFor(c).toLowerCase() === term.toLowerCase())
        ...
        >
          ...
          >
            ...
            <span className="truncate">
              {value ? value.label : "Search customer by name, email or phone"}
            </span>
          </span>
