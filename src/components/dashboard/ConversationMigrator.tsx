...
export const ConversationMigrator: React.FC<ConversationMigratorProps> = ({
  ...
  const [fromInbox, setFromInbox] = useState(
    sourceInboxId || "9255819b-e8a5-44e9-bcbd-38ca5445663f",
  )
  ...
  const migrationMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: t(
          "dashboard.migrationDescription",
          `Moved ${data?.length || 0} conversations to the target inbox.`,
        ),
      })
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description: t(
          "dashboard.migrationErrorDescription",
          "Failed to move conversations. Please try again.",
        ),
        variant: "destructive",
      })
    },
  ...
  const handleMigrate = () => {
    if (fromInbox === toInbox) {
      toast({
        ...
        description: t(
          "dashboard.migrationSameInboxDescription",
          "Please select different source and target inboxes.",
        ),
        variant: "destructive",
      })
      return
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t(
            "dashboard.migratorDescription",
            "Move conversations from one inbox to another. This action cannot be undone.",
          )}
        </CardDescription>
      </CardHeader>
