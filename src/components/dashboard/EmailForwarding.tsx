...
import { AlertCircle, Check, Clock, Copy, Edit, Mail, Plus, RefreshCw, Save, X } from "lucide-react"
...
export function EmailForwarding({
  ...
  mode?: "full" | "addAliasOnly" | "gmailAndAccounts"
}) {
  ...
  const addEmailMutation = useMutation({
    mutationFn: async ({
      ...
      emailAddress: string
      inboxId: string
      providerType: "forwarding" | "google-group"
    }) => {
    ...
    return (
      <div className="space-y-6">
        <Alert className="bg-primary-muted border-primary/20">
          ...
          <AlertDescription>
            <strong>Authentication Required:</strong> Please log in to set up email forwarding and
            manage your inbox settings.
          </AlertDescription>
        </Alert>
      </div>
    )
  ...
  const autoSyncEnabledCount = accounts.filter(
    (acc) => acc.provider === "gmail" && acc.auto_sync_enabled,
  ).length
    ...
    <div className="space-y-6">
      {mode !== "addAliasOnly" && (
        <Alert className="bg-primary-muted border-primary/20">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            <strong>Gmail Integration:</strong> Connect your Gmail account (OAuth) to sync emails on
            a schedule.
            {autoSyncEnabledCount > 0 ? (
              <div className="flex items-center gap-2 mt-2 text-success">
                <Clock className="h-4 w-4" />
                <span>
                  Auto-sync enabled for {autoSyncEnabledCount} Gmail account
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Note: Google Groups or forwarding-only addresses deliver instantly via their
              forwarding address — manual sync does not apply.
            </p>
            <div className="mt-3 flex gap-2">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 mr-2 ${syncEmailsMutation.isPending ? "animate-spin" : ""}`}
                />
      ...
      {mode !== "gmailAndAccounts" && (
        <Card className="bg-gradient-surface border-border/50 shadow-surface">
          <CardHeader>
            ...
            <CardDescription>
              Enter your email address to set up forwarding. We'll generate a unique forwarding
              address for you.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setConnectionType(v as "forwarding" | "google-group")}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="google-group">
                      Google Group (add our address as a member)
                    </SelectItem>
                  </SelectContent>
                </Select>
                {connectionType === "google-group" && (
                  <p className="mt-2 text-xs text-muted-foreground">
                    In Google Admin, open your Group → Members → Add members, paste the generated
                    forwarding address, and ensure external posting is allowed.
                  </p>
                )}
              </div>
              <div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm mb-6">
                  Add your first email account to get started with forwarding.
                </p>
              </div>
            ) : (
                        ...
                        {account.provider === "gmail" ? (
                          ...
                          <div className="space-y-2 border-t pt-3">
                            <p className="text-sm text-muted-foreground">
                              This address uses forwarding (e.g., Google Group). Messages arrive in
                              real time via the forwarding address; manual sync is not applicable.
                            </p>
                          </div>
                        )}
                                ...
                                {account.inbox_id ? (
                                  <div className="flex items-center gap-1">
                                    <div
                                      ...
                                      style={{
                                        backgroundColor:
                                          inboxes.find((i) => i.id === account.inbox_id)?.color ||
                                          "#3B82F6",
                                      }}
                                    />
                                    <span className="font-medium">
                                      {inboxes.find((i) => i.id === account.inbox_id)?.name ||
                                        "Unknown Inbox"}
                                    </span>
                                  </div>
                                ) : (
