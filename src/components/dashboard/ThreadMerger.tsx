...
import { canonicalizeEmail, extractMessageIds, normalizeSubject } from "@/lib/emailThreading"
...
export const ThreadMerger: React.FC<ThreadMergerProps> = ({ inboxId, onMergeComplete }) => {
  ...
  const { data: splitThreads, isLoading: isDetecting } = useQuery({
    ...
    queryFn: async () => {
      ...
      conversations.forEach((conv) => {
        ...
        if (conv.external_id?.includes("@") && messageIds.size === 0) {
      ...
      subjectGroups.forEach((convs, key) => {
        if (convs.length > 1) {
          ...
          if (recentConvs.length > 1) {
            ...
            const ungroupedConvIds = convIds.filter(
              (id) =>
                !Array.from(threadGroups.values()).some((g) => g.conversationIds.includes(id)),
            )
  ...
  const mergeMutation = useMutation({
    ...
    onSuccess: (results) => {
      ...
      toast({
        ...
        description: t(
          "dashboard.mergeSuccessDescription",
          `Merged ${results.length} split threads into single conversations.`,
        ),
      })
    ...
    onError: (error) => {
      ...
      toast({
        ...
        description: t(
          "dashboard.mergeErrorDescription",
          error instanceof Error ? error.message : "Failed to merge threads. Please try again.",
        ),
        variant: "destructive",
      })
    },
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t(
              "dashboard.threadMergerDescription",
              "Automatically detect and merge split email threads.",
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Alert>
            ...
            <AlertDescription>
              {t(
                "dashboard.noSplitThreads",
                "No split threads detected. All conversations are properly threaded!",
              )}
            </AlertDescription>
          </Alert>
        </CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t(
            "dashboard.foundSplitThreads",
            `Found ${splitThreads.length} split thread(s) that can be merged.`,
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {t(
              "dashboard.mergeWarning",
              "This will permanently merge conversations. This action cannot be undone.",
            )}
          </AlertDescription>
        </Alert>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Badge
                      variant={
                        thread.matchType === "email-headers"
                          ? "default"
                          : thread.matchType === "helpscout"
                            ? "secondary"
                            : "outline"
                      }
                      className="text-xs"
