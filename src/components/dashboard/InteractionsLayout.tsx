...
import { useIsDesktop, useIsMobile, useIsTablet } from "@/hooks/use-responsive"
...
export const InteractionsLayout: React.FC<InteractionsLayoutProps> = ({
  ...
  const { data: urlConversation, isLoading: urlConvLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(
        "[EnhancedInteractionsLayout] Fetching conversation from URL:",
  ...
  const handleSelectConversation = useCallback(
    (conversation: Conversation) => {
      ...
      const conversationIdsToFetch =
        conversation.thread_ids && conversation.thread_ids.length > 1
    ...
    [
      isMobile, // Navigate to conversation resource URL (pushes history entry)
      navigate,
    ],
  )
