...
import { AlertCircle, Loader2, RefreshCw, User } from "lucide-react"
...
import { AircallBlockedModal, AircallLoginModal } from "./voice"
...
export const VoiceInterface = () => {
  ...
  const { calls, callEvents, activeCalls, recentCalls, callsByStatus, isLoading, error } =
    useCalls()
  ...
  const { callbackRequests, pendingRequests, processedRequests, completedRequests } =
    useCallbackRequests()

  const { voicemails } = useVoicemails()

  // Initialize real-time notifications
  const { incomingCall, isIncomingCallModalOpen, closeIncomingCallModal, isWebSocketBlocked } =
  ...
  const getFilteredData = useMemo(() => {
    ...
    switch (selectedSection) {
      ...
      case "voicemails-pending":
        return {
          type: "voicemails",
          data: voicemails?.filter((vm) => vm.status === "pending") || [],
        }
      case "voicemails-assigned":
        return {
          type: "voicemails",
          data: voicemails?.filter((vm) => vm.status === "assigned") || [],
        }
      case "voicemails-closed":
        return {
          type: "voicemails",
          data: voicemails?.filter((vm) => vm.status === "closed") || [],
        }
      case "voicemails-all":
  ...
  const transformCallbackToEntity = (callback: any) => ({
    ...
    customer: {
      phone: callback.customer_phone || "Unknown",
      initials: callback.customer_phone
        ? callback.customer_phone.substring(0, 2).toUpperCase()
        : "UC",
    },
    status: callback.status,
  ...
  const renderVoiceSidebar = () => (
    <VoiceSidebar selectedSection={selectedSection} onSectionChange={handleSectionChange} />
  )
  ...
  const renderCallList = () => {
    ...
    if (selectedSection.startsWith("callbacks-")) {
      const statusFilter =
        selectedSection === "callbacks-pending"
          ? "pending"
          : selectedSection === "callbacks-assigned"
            ? "assigned"
            : selectedSection === "callbacks-closed"
              ? "closed"
              : "all"
    ...
    if (selectedSection.startsWith("voicemails-")) {
      const statusFilter =
        selectedSection === "voicemails-pending"
          ? "pending"
          : selectedSection === "voicemails-assigned"
            ? "assigned"
            : selectedSection === "voicemails-closed"
              ? "closed"
              : "all"
      ...
      return (
        <VoicemailsList
          statusFilter={statusFilter}
          onSelectVoicemail={(voicemail) =>
            handleEntitySelect(transformVoicemailToEntity(voicemail))
          }
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                {selectedEntity.type === "call" && (
                  <>
                    ...
                    <span>•</span>
                    <span>
                      {(selectedEntity as any).direction === "inbound" ? "Incoming" : "Outgoing"}
                    </span>
                    <span>•</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {selectedEntity.type === "call" && (selectedEntity as any).endedAt && (
                    <div>
                      ...
                      <span className="ml-2">
                        {format(new Date((selectedEntity as any).endedAt), "MMM d, HH:mm")}
                      </span>
                    </div>
                  )}
                  ...
                  {selectedEntity.type === "callback" && (selectedEntity as any).requestedAt && (
                    <div>
                      ...
                      <span className="ml-2">
                        {format(new Date((selectedEntity as any).requestedAt), "MMM d, HH:mm")}
                      </span>
                    </div>
                  )}
                  ...
                  {selectedEntity.type === "voicemail" && (selectedEntity as any).receivedAt && (
                    <div>
                      ...
                      <span className="ml-2">
                        {format(new Date((selectedEntity as any).receivedAt), "MMM d, HH:mm")}
                      </span>
                    </div>
                  )}
                  ...
                  {selectedEntity.type === "call" && (selectedEntity as any).agentPhone && (
                    <div>
                      ...
                      <span className="ml-2">
                        {formatPhoneNumber((selectedEntity as any).agentPhone)}
                      </span>
                    </div>
                  )}
                </div>
  ...
  const renderCustomerSidebar = () => {
    ...
    return <VoiceCustomerSidebar call={linkedCall} customerPhone={customerPhone} />
  }
  ...
  return (
    <AircallErrorBoundary>
      ...
      <ConnectionDiagnostic
        ...
        isOptedOut={
          error?.message?.includes("disabled for this session") ||
