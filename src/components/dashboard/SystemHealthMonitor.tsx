...
import { Activity, Database, FileText, RefreshCw, TrendingUp } from "lucide-react"
...
export function SystemHealthMonitor({ organizationId }: { organizationId: string }) {
    ...
    queryFn: async () => {
      ...
      const [entries, tracking, outcomes] = await Promise.all([
        supabase
          ...
          .select("quality_score, updated_at")
          .eq("organization_id", organizationId),
        supabase
          .from("response_tracking")
          .select("feedback_rating, created_at")
          .eq("organization_id", organizationId),
        supabase
          .from("response_outcomes")
          .select("customer_satisfaction_score, conversation_resolved, created_at")
          .eq("organization_id", organizationId),
      ])
      ...
      return [
        {
          ...
          avg_quality:
            entries.data?.reduce((sum, e) => sum + e.quality_score, 0) /
              (entries.data?.length || 1) || 0,
          high_quality_count: entries.data?.filter((e) => e.quality_score >= 4.0).length || 0,
          recently_updated:
            entries.data?.filter((e) => new Date(e.updated_at) > sevenDaysAgo).length || 0,
        },
        {
          ...
          avg_quality:
            tracking.data
              ...
              .reduce((sum, t) => sum + (t.feedback_rating || 0), 0) /
              (tracking.data?.filter((t) => t.feedback_rating).length || 1) || 0,
          high_quality_count:
            tracking.data?.filter((t) => t.feedback_rating && t.feedback_rating >= 4).length || 0,
          recently_updated:
            tracking.data?.filter((t) => new Date(t.created_at) > sevenDaysAgo).length || 0,
        },
        {
          metric: "response_outcomes",
          total_count: outcomes.data?.length || 0,
          avg_quality:
            outcomes.data?.reduce((sum, o) => sum + (o.customer_satisfaction_score || 0), 0) /
              (outcomes.data?.length || 1) || 0,
          high_quality_count: outcomes.data?.filter((o) => o.conversation_resolved).length || 0,
          recently_updated:
            outcomes.data?.filter((o) => new Date(o.created_at) > sevenDaysAgo).length || 0,
        },
      ] as HealthMetric[]
    },
  ...
  const getHealthStatus = (metric: HealthMetric) => {
    const activityRate =
      metric.total_count > 0 ? (metric.recently_updated / metric.total_count) * 100 : 0
