  ...
  const { data: stats, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const avgQualityScore =
        entries?.reduce((sum, e) => sum + e.quality_score, 0) / totalEntries || 0
      ...
      const aiSuggestionCount =
        tracking?.filter((t) => t.response_source === "ai_suggestion").length || 0
      const templateCount = tracking?.filter((t) => t.response_source === "template").length || 0
      const knowledgeBaseCount =
        tracking?.filter((t) => t.response_source === "knowledge_base").length || 0

      const resolvedCount = outcomes?.filter((o) => o.conversation_resolved).length || 0
      const avgSatisfaction =
        outcomes?.reduce((sum, o) => sum + (o.customer_satisfaction_score || 0), 0) /
          (outcomes?.length || 1) || 0
      const avgReplyTime =
        outcomes?.reduce((sum, o) => sum + (o.reply_time_seconds || 0), 0) /
          (outcomes?.length || 1) || 0
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{stats?.totalAcceptance || 0} accepted</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats?.totalOutcomes
                ? ((stats.resolvedCount / stats.totalOutcomes) * 100).toFixed(1)
                : 0}
              %
            </div>
            <p className="text-xs text-muted-foreground">
