...
export const ConversationTableRow = memo<ConversationTableRowProps>(
  ({
    ...
    const { dispatch, archiveConversation, toggleConversationRead, selectedInboxId } =
      useConversationList()
    ...
    const showNewBadge =
      !conversation.is_read && conversation.status !== "closed" && !conversation.is_archived
    ...
    const computedValues = useMemo(() => {
      ...
      const inbox = conversation.inbox_id
        ? inboxes.find((i: any) => i.id === conversation.inbox_id)
        : undefined
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mb-1">
                ...
                {conversation.thread_count && conversation.thread_count > 1 && (
                  <Badge
                    variant="outline"
                    className="px-1 py-0 text-[9px] shrink-0 border-primary/30 text-primary"
                  >
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {showNewBadge && (
                  <Badge className="bg-primary text-primary-foreground px-1.5 py-0 text-[9px]">
                    New
                  </Badge>
                )}
                ...
                {conversation.last_message_sender_type === "customer" &&
                  ...
                  (conversation.status === "open" || conversation.status === "pending") && (
                    <Badge
                      variant="outline"
                      className="px-1 py-0 text-[9px] bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/20 dark:text-orange-400 dark:border-orange-800"
                    >
                ...
                {conversation.channel === "widget" &&
                  (conversation.metadata as any)?.chatSessionStatus === "active" && (
                    <Badge
                      variant="outline"
                      className="text-[9px] px-1 py-0 bg-emerald-50 text-emerald-700 border-emerald-300 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-700 animate-pulse"
                    >
                      LIVE
                    </Badge>
                  )}
    ...
    if (style) {
      return (
        <ConversationStatusContextMenu
          ...
          brandLabel={
            (conversation.metadata as any)?.brand ??
            (conversation.metadata as any)?.brand_name ??
            null
          }
        >
          {/* biome-ignore lint/a11y/useSemanticElements: interactive container with nested controls; native <button> would be invalid HTML */}
          ...
          >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {computedValues.customerContact && (
                    <div
                      ...
                      title={computedValues.customerContact}
                    >
                      {computedValues.customerContact}
                    </div>
                  )}
            ...
            <div className="p-2 flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {conversation.thread_count && conversation.thread_count > 1 && (
                  <Badge
                    variant="outline"
                    className="px-1.5 py-0 text-[10px] shrink-0 border-primary/30 text-primary bg-primary/5"
                  >
                    {conversation.thread_count}
                  </Badge>
                )}
                {showNewBadge && (
                  <Badge className="bg-blue-500 text-white px-1.5 py-0 text-[10px] shrink-0">
                    New
                  </Badge>
                )}
                {conversation.last_message_is_internal && (
                ...
                {conversation.last_message_sender_type === "customer" &&
                  ...
                  (conversation.status === "open" || conversation.status === "pending") && (
                    <Badge
                      variant="outline"
                      className="px-1.5 py-0 text-[10px] shrink-0 bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/20 dark:text-orange-400 dark:border-orange-800"
                    >
            ...
            <div className="p-2 w-36 shrink-0">
              <span
                ...
                title={computedValues.receivedAt}
              >
