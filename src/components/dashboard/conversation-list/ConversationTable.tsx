...
import { AlmostThereBanner, InboxZeroCelebration } from "./InboxZeroCelebration"
...
export const ConversationTable = memo<ConversationTableProps>(
  ({ onSelectConversation, selectedConversation }) => {
    ...
    const orderedIds = useMemo(
      () => paginatedConversations.map((c) => c.id),
      [paginatedConversations],
    )
    const setSelection = useCallback(
      (ids: string[], selected: boolean) =>
        dispatch({ type: "SET_BULK_SELECTION", payload: { ids, selected } }),
      [dispatch],
    )
          ...
          <div className="flex items-center justify-center h-40 p-6">
            <div className="text-center max-w-sm space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                {hiddenByFiltersCount} conversation{hiddenByFiltersCount === 1 ? " is" : "s are"}{" "}
                hidden by the active filters in this view.
              </p>
              <Button
                variant="outline"
                size="sm"
