...
import { AlertTriangle, Check, Copy, Edit, Mail, RefreshCw, Save, Trash2, X } from "lucide-react"
...
export function ConnectedEmailAccountsContent() {
  ...
  const [pendingDisableAccount, setPendingDisableAccount] = useState<{
    id: string
                  ...
                  {account.provider === "gmail" ? (
                    <div className="space-y-2 border-t pt-3">
                      <div className="flex items-center justify-between">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Automatically sync this account on a schedule.
                          </p>
                        </div>
                        <Switch
                          checked={account.auto_sync_enabled}
                          onCheckedChange={(checked) =>
                            handleAutoSyncToggle(account.id, checked, account.sync_interval_minutes)
                          }
                          disabled={
                            updateAccountSyncMutation.isPending ||
                            cleanupAccountDataMutation.isPending
                          }
                        />
                      </div>
                      ...
                      {account.auto_sync_enabled && !account.inbox_id && (
                        <Alert variant="destructive" className="mt-3">
                          ...
                          <AlertDescription>
                            <strong>Sync blocked:</strong> Auto-sync is enabled but no inbox is
                            assigned. Emails will <strong>not be synced</strong> until you assign an
                            inbox below.
                          </AlertDescription>
                        </Alert>
                      )}
                    ...
                    <div className="space-y-2 border-t pt-3">
                      <p className="text-sm text-muted-foreground">
                        This address uses forwarding (e.g., Google Group). Messages arrive in real
                        time via the forwarding address; manual sync is not applicable.
                      </p>
                    </div>
                  )}
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          {account.inbox_id ? (
                            ...
                            >
                              {account.auto_sync_enabled
                                ? "⚠️ Not assigned (using default inbox)"
                                : t("admin.notAssigned")}
                            </span>
                          )}
                        </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                ...
                <strong>{pendingDisableAccount?.conversationCount || 0} conversations</strong>{" "}
                synced.
              </p>
              <p>Do you want to delete all conversations and messages synced from this account?</p>
              <Alert variant="warning" className="mt-2">
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  <strong>Warning:</strong> This action cannot be undone. All messages and
