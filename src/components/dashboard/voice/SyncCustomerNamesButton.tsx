  ...
  const handleSyncAll = async () => {
    ...
    try {
        ...
        try {
          ...
          if (!lookupError && customer) {
            ...
            if (!updateError) {
              ...
              console.log(
                `✅ Matched ${callIds.length} calls to local customer:`,
                customer.full_name,
              )
            }
          }
      ...
      const remainingUnknowns = callsToSync.filter((call) => {
        const hasCustomerData = phoneGroups[call.customer_phone!]?.some(
          (c) => localMatchCount > 0, // If we matched any, assume this phone was matched
        )
        return !hasCustomerData && call.customer_phone
      })

      // FILTER: Remove already-cached phone numbers before processing
      const uniqueRemainingPhones = [
        ...new Set(
          remainingUnknowns
            ...
            if (noddiData?.data?.found) {
              ...
              if (noddiData.data.user && callsWithPhone.length > 0) {
                ...
                for (const call of callsWithPhone) {
                  await syncCustomerFromNoddi(noddiData, phone, organizationId, call.id)
                }
              ...
              const { error: cacheError } = await supabase.from("noddi_customer_cache").insert({
                phone,
                email: null,
