...
import { AlertCircle, Clock, Mail, MessageSquare, Phone, XCircle } from "lucide-react"
...
export const VoiceCustomerSidebar: React.FC<VoiceCustomerSidebarProps> = ({
  ...
  const stats = noddiData?.data?.found
    ? {
        totalBookings:
          (noddiData.data.unpaid_bookings?.length || 0) + (noddiData.data.priority_booking ? 1 : 0),
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {customerPhone && (
                    <p className="text-sm text-muted-foreground font-mono">{customerPhone}</p>
                  )}
                  {customer?.email && (
              ...
              {stats && (
                <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t">
                  ...
                  <div className="text-center">
                    <div
                      className={`text-2xl font-bold ${stats.unpaidBookings > 0 ? "text-destructive" : "text-success"}`}
                    >
                  ...
                  <div className="text-center">
                    <div className="text-2xl">{stats.hasPriority ? "⭐" : "—"}</div>
                ...
                <CardContent className="space-y-3">
                  ...
                  {call.agent_phone && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium font-mono">{call.agent_phone}</span>
                    </div>
                  )}
                  {call.end_reason && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">End Reason</span>
                      <span className="text-sm">{call.end_reason.replace(/_/g, " ")}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
            ) : (
              <div className="text-center py-8 text-muted-foreground text-sm">No call selected</div>
            )}
          </TabsContent>
