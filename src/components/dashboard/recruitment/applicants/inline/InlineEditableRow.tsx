...
import { type ApplicantPatch, useUpdateApplicant } from "../hooks/useUpdateApplicant"
...
const InlineEditableRow: React.FC<Props> = ({
  ...
  const handleSave = (next: string) => {
    ...
    if (type === "number") {
      ...
      next === "" &&
      (field === "phone" || field === "location" || field === "availability_date")
    ) {
  ...
  return (
    <div className={cn("group flex flex-col gap-1 py-2 border-b last:border-0")}>
