  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>
            Send SMS til {applicant.first_name} {applicant.last_name}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {noInboxes && (
            <div className="text-sm text-destructive border border-destructive/40 rounded-md p-3 bg-destructive/5">
              Ingen rekrutteringsinnboks har SMS aktivert. Konfigurer dette under Admin →
              Integrasjoner.
            </div>
          )}
                ...
                <SelectContent>
                  {(inboxes || []).map((i) => (
                    <SelectItem key={i.id} value={i.id}>
                      {i.name}{" "}
                      {i.sms_provider_phone_number ? `(${i.sms_provider_phone_number})` : ""}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
