...
import { Check, Clock, Copy, FileText, Mail, MessageSquare, Send, Sparkles, X } from "lucide-react"
...
function TokenRowCard({
  ...
  const channelLabel =
    token.channel === "sms" ? "SMS" : token.channel === "email" ? "E-post" : "Manuell"
  ...
  return (
    <li className="px-4 py-3 space-y-1.5">
      ...
      <div className="text-xs text-muted-foreground">
        ...
        {(status === "sent" || status === "opened") && <> · {token.attempts} av 5 forsøk brukt</>}
      </div>
      {isLive && (
    ...
    <Card>
        ...
        {onSendForm && (
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              ...
              <TooltipContent>
                Be kandidaten fylle ut manglende informasjon via skjema
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <p className="px-4 py-3 text-xs text-muted-foreground">Ingen skjemaer sendt ennå.</p>
        ) : (
