...
import { ExternalLink, Info, Loader2, Mail, MessageSquare, Send, ShieldAlert } from "lucide-react"
...
const SendCandidateFormDialog: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const def = emailTemplates.find((t: any) => t.name === DEFAULT_TPL_NAME) ?? emailTemplates[0]
    if (def) {
    ...
    <Dialog open={open} onOpenChange={(o) => !send.isPending && onOpenChange(o)}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Søkeren får en lenke for å fylle ut manglende informasjon selv. Lenken er personlig og
            krever bekreftelse av telefonnummer.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          <div className="flex items-start gap-2 rounded-md border bg-muted/40 p-2.5 text-xs text-muted-foreground">
            ...
            <p className="leading-relaxed">
              Send dette etter at kandidaten er kvalifisert eller flyttet til «Forhåndsscreening».
              Lenken lar dem fylle inn data som mangler, og AI-vurderingen blir automatisk oppdatert
              når de sender inn.
            </p>
          </div>
            ...
            <>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={inboxId} onValueChange={setInboxId}>
                  <SelectTrigger>
                    <SelectValue placeholder="Velg innboks..." />
                  </SelectTrigger>
                  <SelectContent>
