...
export function RetentionConfigDialog({ open, onOpenChange, organizationId }: Props) {
  ...
  const handleSave = async () => {
    if (days < 30 || days > 7300) {
      toast({
        title: "Ugyldig verdi",
        description: "Må være mellom 30 og 7300 dager.",
        variant: "destructive",
      })
      return
    }
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        ...
        <div className="space-y-6 py-6">
          ...
          <div className="rounded-md border p-3 space-y-1">
            ...
            <div className="text-sm font-medium">
              {data?.recruitment_audit_last_cleanup_at
                ? new Date(data.recruitment_audit_last_cleanup_at).toLocaleString("nb-NO", {
          ...
          <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 flex gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Reduserer du oppbevaringstiden, slettes data som er eldre enn ny grense ved neste
              opprydding. Dette kan ikke reverseres.
            </p>
          </div>
