...
export function useStartMetaOAuth() {
  return useMutation({
    mutationFn: async (input: {
      ...
      const data = await invokeOrThrow<{ auth_url: string; state_id: string }>("meta-oauth-init", {
  ...
  return useQuery<MetaOAuthStateRow | null>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "id, oauth_user_id, oauth_user_name, token_expires_at, mode, existing_integration_id",
        )
        .eq("id", stateId)
        .maybeSingle()
