...
export type EditorState = { mode: "create" } | { mode: "edit"; rule: AutomationRule } | null
...
export function RuleEditor({ state, onClose }: Props) {
  ...
  }, [sheetOpen, defaultValues, form.reset])
  ...
  const headerTitle = state?.mode === "edit" ? `Rediger regel: ${state.rule.name}` : "Ny regel"
      ...
      <SheetContent side="right" className="w-full sm:max-w-xl p-0 flex flex-col gap-0">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="border-t bg-background px-6 py-3 flex items-center justify-end gap-2 shrink-0">
            ...
            <Button type="submit" size="sm" disabled={!form.formState.isValid || isSaving}>
