  ...
  switch (action.action_type) {
    ...
    case "assign_to":
      return [
        [
          "Ville tildelt",
          action.assigned_to_name ?? action.profile_name ?? action.assigned_to ?? "—",
        ],
      ]
    case "webhook":
      return [
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Varighet: {formatDuration(result.duration_ms)}
            </p>
          </div>
          <Badge variant="outline" className={cn("gap-1 font-normal", status.className)}>
            <span className={status.label === "Test-kjøring" ? "italic" : undefined}>
              {status.label}
            </span>
          </Badge>
        </div>
