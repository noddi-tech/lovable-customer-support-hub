...
export function EmailTemplateList({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      e.preventDefault()
      const nextIdx =
        e.key === "ArrowDown" ? Math.min(templates.length - 1, idx + 1) : Math.max(0, idx - 1)
  ...
  if (isLoading) {
    return <div className="text-xs text-muted-foreground p-4">Laster maler...</div>
  }
  ...
  if (!hasAnyTemplates) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-8 gap-3 text-muted-foreground">
        ...
        <p className="text-sm">Ingen maler ennå. Opprett den første med 'Ny mal'.</p>
      </div>
    )
  }
