...
export function EmailTemplateDeletedView({
  ...
  const stage = template.stage_trigger ? stages?.find((s) => s.id === template.stage_trigger) : null
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            Denne malen er slettet og kan ikke redigeres. Du kan gjenopprette den eller slette den
            permanent.
          </p>
        </div>
      </div>
      ...
      <Card className="p-4 space-y-3 bg-muted/20">
        ...
        <div className="space-y-1">
          ...
          <div
            ...
            // biome-ignore lint/security/noDangerouslySetInnerHtml: admin email template HTML preview
      ...
      <div className="sticky bottom-0 -mx-4 px-4 py-3 bg-background/95 backdrop-blur border-t border-input flex items-center justify-end gap-2">
        <Button type="button" variant="destructive" size="sm" onClick={onHardDelete}>
          <Trash2 />
          Slett permanent
        </Button>
        <Button
