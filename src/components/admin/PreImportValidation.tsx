...
export const PreImportValidation = ({
  organizationId,
  mailboxMapping,
  onValidationComplete,
}: PreImportValidationProps) => {
  ...
  useEffect(() => {
    ...
    const runValidation = async () => {
      ...
      try {
        ...
        updateCheck(
          ...
          "org",
          org ? "passed" : "failed",
          org ? undefined : "Organization not found",
        )
      } catch {
      ...
      if (mappedMailboxes.length === 0) {
        ...
        updateCheck(
          ...
          "mapping",
          "warning",
          `Only ${mappedMailboxes.length} of ${Object.keys(mailboxMapping).length} mailboxes mapped`,
        )
      } else {
      ...
      try {
        ...
        const newInboxesCount = Object.values(mailboxMapping).filter(
          (v) => v === "create_new",
        ).length
        updateCheck(
          newChecks,
          "inboxes",
          "passed",
          `${inboxes?.length || 0} existing + ${newInboxesCount} new`,
        )
      } catch {
      ...
      try {
        ...
        if (runningJob) {
          ...
          updateCheck(
            ...
            "conflicts",
            "warning",
            `${recentJobs.length} imports in last 24h (idempotent)`,
          )
        } else {
    ...
    void runValidation()
  }, [organizationId, mailboxMapping, onValidationComplete])
  ...
  return (
    <Card className="bg-gradient-surface border-border/50">
      <CardHeader>
        ...
        <CardDescription>Verifying configuration before starting import</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {failedChecks.length > 0 && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {failedChecks.length} critical validation{" "}
              {failedChecks.length === 1 ? "check" : "checks"} failed. Please fix these issues
              before proceeding.
            </AlertDescription>
          </Alert>
        )}
        ...
        {failedChecks.length === 0 && warningChecks.length > 0 && (
          <Alert>
            ...
            <AlertDescription>
              {warningChecks.length} validation{" "}
              {warningChecks.length === 1 ? "warning" : "warnings"} detected. You can proceed, but
