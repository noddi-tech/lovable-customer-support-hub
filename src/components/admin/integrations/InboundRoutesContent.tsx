  ...
  const routesQuery = useQuery({
    ...
    queryFn: async (): Promise<InboundRoute[]> => {
      const { data, error } = await supabase
        .from("inbound_routes")
        .select(
          "id,address,inbox_id,is_active,created_at,alias_local_part,group_email,sender_display_name",
        )
      ...
      <Collapsible open={howItWorksOpen} onOpenChange={setHowItWorksOpen}>
        <CollapsibleTrigger asChild>
          ...
          >
            ...
            <ChevronDown
              className={`w-4 h-4 ml-auto transition-transform ${howItWorksOpen ? "rotate-180" : ""}`}
            />
          </button>
        </CollapsibleTrigger>
          ...
          <div className="rounded-lg border border-border/50 bg-muted/30 p-4 space-y-3 text-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Customers send to your public address (e.g. hei@noddi.no) → Forwarded via SendGrid
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Agent replies → Sent via SendGrid API → Customer receives from your public address
                  with your sender name
                </p>
              </div>
            </div>
          </div>
      ...
      {routesQuery.isLoading || inboxesQuery.isLoading ? (
        ...
        <div className="space-y-2">
          {routesQuery.data.map((r) => (
            <div
              key={r.id}
              className="flex flex-col gap-2 rounded-md border border-border/50 p-3 bg-background/50"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <button
                  type="button"
                  className="font-mono text-sm text-left font-medium hover:underline focus:underline focus:outline-none"
                  onClick={() => {
                    setSelectedRoute(r)
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    size="sm"
                    type="button"
                    onClick={() => copy(r.group_email || r.address, r.id)}
                  >
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-foreground font-medium">
                    {r.sender_display_name || "Not configured"}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span>Inbox:</span>
                  <span className="text-foreground font-medium">
                    {r.inbox_id ? inboxMap[r.inbox_id] || "Inbox" : "Unassigned"}
                  </span>
                </div>
              </div>
            </div>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Set up bidirectional email routing for this address.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-1.5">
                    The address customers send emails to
                  </p>
                  <div className="flex gap-2">
                    <Input
                      id="public-email"
                      ...
                      placeholder={`${selectedRoute.alias_local_part}@yourdomain.com`}
                    />
                    <Button
                      onClick={() =>
                        updatePublicEmail.mutate({
                          id: selectedRoute.id,
                          group_email: publicEmail.trim(),
                        })
                      }
                      disabled={
                        updatePublicEmail.isPending ||
                        publicEmail.trim().length === 0 ||
                        (selectedRoute.group_email || "") === publicEmail.trim()
                      }
                      size="sm"
                    >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-1.5">
                    Set up forwarding from your public email to this address
                  </p>
                  <div className="flex items-center gap-2">
                    <code className="font-mono text-sm px-2 py-1.5 rounded bg-muted/50 flex-1 truncate">
                    ...
                    <Button
                      ...
                      size="sm"
                      type="button"
                      onClick={() => copy(selectedRoute.address, selectedRoute.id)}
                    >
                      <Copy className="w-4 h-4 mr-1" />{" "}
                      {copiedId === selectedRoute.id ? "Copied" : "Copy"}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-1.5">
                    Name shown to customers when you reply (e.g. "Noddi Support")
                  </p>
                  <div className="flex gap-2">
                    <Input
                    ...
                    <Button
                      onClick={() =>
                        updateSenderDisplayName.mutate({
                          id: selectedRoute.id,
                          sender_display_name: senderDisplayName.trim(),
                        })
                      }
                      disabled={
                        updateSenderDisplayName.isPending ||
                        (selectedRoute.sender_display_name || "") === senderDisplayName.trim()
                      }
                      size="sm"
                    >
                ...
                <div className="rounded-md bg-muted/30 p-3 text-xs text-muted-foreground">
                  ...
                  <p>
                    Replies will be sent as:{" "}
                    <span className="font-medium text-foreground">
                      {senderDisplayName || "Your Name"}
                    </span>{" "}
                    &lt;{selectedRoute.group_email || publicEmail || "email@domain.com"}&gt;
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <Button
                    onClick={() =>
                      selectedRoute &&
                      updateRouteInbox.mutate({ id: selectedRoute.id, inbox_id: selectedInboxId })
                    }
                    disabled={
                      updateRouteInbox.isPending ||
                      (selectedRoute && selectedRoute.inbox_id === selectedInboxId)
                    }
                    variant="outline"
                    size="sm"
                    type="button"
