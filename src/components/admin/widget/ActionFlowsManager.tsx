...
export const ActionFlowsManager: React.FC<ActionFlowsManagerProps> = ({
  widgetId,
  organizationId,
}) => {
  ...
  const createFlow = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("ai_action_flows").insert({
        ...
        trigger_phrases: newFlow.trigger_phrases
          .split(",")
          .map((p) => p.trim())
          .filter(Boolean),
  ...
  if (isLoading)
    return <div className="text-center py-8 text-muted-foreground">Loading flows...</div>

  const currentFlows = editingFlow
    ? flows.map((f) => (f.id === editingFlow.id ? editingFlow : f))
    : flows
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Define step-by-step flows for specific customer intents. The AI activates these when it
            detects matching intent.
          </p>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1 max-w-md">
              Create action flows to guide the AI through specific tasks like booking, cancellation,
              or rescheduling. Without flows, the AI will answer questions from the knowledge base
              only.
            </p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="mt-0.5">
                          {flow.description || "No description"} · {flow.flow_steps.length} step
                          {flow.flow_steps.length !== 1 ? "s" : ""}
                        </CardDescription>
                      </div>
                    </div>
                    <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          toggleActive.mutate({ id: flow.id, is_active: checked })
                        }
                      />
                      <Button
                        variant="ghost"
                  ...
                  <CardContent className="pt-0 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {displayFlow.flow_steps.length === 0 ? (
                        <p className="text-sm text-muted-foreground py-4 text-center">
                          No steps defined.{" "}
                          {isEditing ? "Add steps to define the flow." : "Edit to add steps."}
                        </p>
                      ) : (
                        <div className="space-y-2">
                          {displayFlow.flow_steps.map((step, idx) => (
                            ...
                            >
                              <span className="text-xs font-mono text-muted-foreground mt-2 w-6 shrink-0">
                                {idx + 1}.
                              </span>
                              {isEditing ? (
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {step.marker && (
                                      <Badge className="text-xs font-mono">{step.marker}</Badge>
                                    )}
                                    {step.field && (
                    ...
                    {isEditing && (
                      <div className="flex gap-2 justify-end pt-2">
                        ...
                        <Button size="sm" onClick={saveEditingFlow}>
                          Save Changes
                        </Button>
                      </div>
                    )}
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Define a new action flow that the AI will activate when it detects matching customer
              intent.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                placeholder="e.g., new_booking, cancel_booking"
                value={newFlow.intent_key}
                onChange={(e) => setNewFlow({ ...newFlow, intent_key: e.target.value })}
              />
            ...
            <div>
              ...
              <Input
                placeholder="e.g., Book New Service"
                value={newFlow.label}
            ...
            <div>
              <Label>Description</Label>
              <Textarea
                placeholder="When should this flow activate?"
            ...
            <div>
              <Label>Trigger Phrases (comma-separated)</Label>
              <Input
                placeholder="book a service, I want to order, bestille"
                value={newFlow.trigger_phrases}
                onChange={(e) => setNewFlow({ ...newFlow, trigger_phrases: e.target.value })}
              />
            </div>
            <div className="flex items-center gap-2">
              <Switch
                checked={newFlow.requires_verification}
                onCheckedChange={(checked) =>
                  setNewFlow({ ...newFlow, requires_verification: checked })
                }
              />
              <Label>Requires Phone Verification</Label>
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newFlow.intent_key || !newFlow.label}
            >
              Create
            </Button>
          </DialogFooter>
