    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          status={
            analytics.resolutionRate > 60
              ? "good"
              : analytics.resolutionRate > 30
                ? "warning"
                : "neutral"
          }
        />
        <MetricCard
          ...
          status={
            analytics.satisfactionRate > 80
              ? "good"
              : analytics.satisfactionRate > 50
                ? "warning"
                : "danger"
          }
        />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <MetricCard title="Total Messages" value={analytics.totalMessages} icon={MessageCircle} />
        <MetricCard
      ...
      {analytics.volumeByDate.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">AI Conversation Volume (Last 30 Days)</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart
                  data={analytics.feedbackByDate.filter((d) => d.positive > 0 || d.negative > 0)}
                >
                  <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
            ...
            <div className="flex flex-wrap gap-2">
              {analytics.topIntents.map((intent) => (
                <Badge key={intent.intent} variant="secondary" className="text-sm">
                  {intent.intent}{" "}
                  <span className="ml-1 text-muted-foreground">({intent.count})</span>
                </Badge>
              ))}
            </div>
          </CardContent>
