...
export function InboxSettingsPage({ inboxId }: { inboxId: string }) {
  ...
  const { data: inboundRoutes } = useQuery({
    ...
    queryFn: async (): Promise<InboundRoute[]> => {
      const { data, error } = await supabase.from('inbound_routes').select('id,inbox_id,address,group_email');
      if (error) throw error;
  ...
  const route = useMemo(() => (inboundRoutes || []).find((r) => r.inbox_id === inboxId) || null, [inboundRoutes, inboxId]);
  const accounts = useMemo(() => (emailAccounts || []).filter((a) => a.inbox_id === inboxId), [emailAccounts, inboxId]);

  useEffect(() => { if (inbox) setForm(inbox); }, [inbox]);
  useEffect(() => { setGroupEmail(route?.group_email || ''); }, [route]);
  ...
  const updateGroupEmail = useMutation({
    mutationFn: async () => {
      if (!route) return;
      const { error } = await supabase.from('inbound_routes').update({ group_email: groupEmail.trim() }).eq('id', route.id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="space-y-3">
        <Button variant="ghost" size="sm" className="-ml-2" onClick={() => navigate('/admin/inboxes')}>
          <ArrowLeft className="w-4 h-4 mr-2" /> Back to inboxes
        </Button>
        ...
        <p className="text-muted-foreground">Configure this inbox's details, routing and sending identity.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input id="inbox-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </div>
          <div>
              ...
              {INBOX_COLOR_PALETTE.map((color) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full border-2 transition-all ${
                    form.color === color.value ? 'border-foreground scale-110 ring-2 ring-offset-2 ring-primary' : 'border-transparent hover:scale-105'
                  }`}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">Override the organization-level sender name for this inbox</p>
          </div>
          <div>
            <Label htmlFor="inbox-purpose">Purpose</Label>
            <Select value={form.purpose || 'support'} onValueChange={(v: 'support' | 'recruitment') => setForm({ ...form, purpose: v })}>
              <SelectTrigger id="inbox-purpose"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="support">Support</SelectItem>
                <SelectItem value="recruitment">Recruitment</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground mt-1">Recruitment inboxes auto-link inbound emails to applicants by email.</p>
          </div>
            ...
            <div className="text-muted-foreground">
              {accounts.length + (route ? 1 : 0) === 0 ? (
                ...
                <ul className="list-disc pl-5 space-y-1">
                  ...
                  {route && (
                    <li>
                      <span className="font-medium">{route.group_email || 'Public email not set'}</span>
                      <span className="ml-2">→ forwards to </span>
                      <code className="font-mono text-xs px-1.5 py-0.5 rounded bg-muted/50">{route.address}</code>
                    </li>
                  )}
                </ul>
              )}
          ...
          <div>
            ...
            {route ? (
              <div className="mt-1 flex gap-2">
                <Input id="inbox-group-email" value={groupEmail} onChange={(e) => setGroupEmail(e.target.value)} placeholder="e.g., hei@noddi.no" />
                <Button onClick={() => updateGroupEmail.mutate()} disabled={updateGroupEmail.isPending || groupEmail.trim().length === 0}>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Used as the From address when replying. Must match your authenticated domain in SendGrid.
            </p>
          </div>

          <div className="flex items-center justify-between text-sm pt-2 border-t">
            <span className="flex items-center gap-2"><MessageSquare className="w-4 h-4" /> Conversations</span>
      ...
      <div className="flex justify-end gap-2 pb-6">
        <Button variant="outline" onClick={() => navigate('/admin/inboxes')}>Cancel</Button>
