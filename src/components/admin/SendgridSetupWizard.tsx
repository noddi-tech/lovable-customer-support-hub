...
export const SendgridSetupWizard = () => {
  ...
  const createInboundRoute = async () => {
    if (!domainRow?.id || !selectedInbox) {
      toast({
        title: "Missing data",
        description: "Select an inbox; ensure domain is created.",
        variant: "destructive",
      })
      return
    }
  ...
  const onSubmit = async (values: SetupFormValues) => {
    ...
    if (data?.ok === false) {
      toast({
        title: "Sender Authentication needed",
        description: "We created the sender auth config; add the DNS records then retry.",
        variant: "destructive",
      })
    } else {
      toast({
        title: "Parse route created",
        description: "Add the MX record to your DNS and we will verify automatically.",
      })
    }
  }
    ...
    try {
      ...
      if (!dns) {
        toast({
          ...
          description: "Click Create Parse Route to fetch expected DNS records.",
          variant: "destructive",
        })
        return
      }
      ...
      const mxRes = await fetch(`https://dns.google/resolve?name=${mxHost}&type=MX`)
        .then((r) => r.json())
        .catch(() => null)
      const mxOk = !!mxRes?.Answer?.some(
      ...
      if (mxOk && cnameOk) {
        toast({
          title: "DNS verified",
          description: "All records found. Validating with SendGrid...",
        })
          ...
          if (isValid) {
            ...
            for (let c = 1; c <= createAttempts; c++) {
              ...
              if (createData?.ok) {
                ...
                toast({
                  title: "Parse route created",
                  description: "MX is set and sender auth validated.",
                })
                return
              }
        ...
        toast({
          title: "Still pending",
          description:
            "SendGrid has not marked DNS as valid yet. Please wait a few minutes and try again.",
          variant: "destructive",
        })
      } else {
        const missing: string[] = []
        if (!mxOk) missing.push(`MX ${mxHost} -> mx.sendgrid.net`)
        cnameRecords.forEach((rec: any, i: number) => {
          if (!cnameChecks[i]) missing.push(`CNAME ${rec.host} -> ${rec.data}`)
        })
        toast({
          title: "DNS not ready",
          description: `Missing/invalid: ${missing.join(", ")}`,
          variant: "destructive",
        })
      }
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Card id="sendgrid-setup" className="bg-gradient-surface border-border/50 shadow-surface">
        <CardHeader>
          ...
          <CardDescription>
            Create an inbound parse route programmatically and get DNS records to add to your
            domain.
          </CardDescription>
        </CardHeader>
              ...
              <div className="sm:col-span-2 flex flex-col gap-2">
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={verifying}
                  >
                    {verifying ? "Verifying..." : "Verify DNS & Retry"}
                  </Button>
                </div>
                {retryStatus && (
                ...
                {dnsVerifiedButPending && !retryStatus && (
                  <div className="rounded-md border border-primary/30 bg-primary/5 p-3 text-sm">
                    ✅ DNS and sender auth are verified. SendGrid needs a few more minutes for
                    internal propagation. Click <strong>Create Parse Route</strong> to try again.
                  </div>
                )}
              </div>
            </form>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="rounded-md border border-border/50 p-3 text-sm bg-background/50">
                  <div className="flex gap-2">
                    ...
                    <span>
                      {result.hostname ||
                        `${form.getValues().parse_subdomain}.${form.getValues().domain}`}
                    </span>
                  </div>
                  <div className="flex gap-2">
              ...
              {(result?.dns_records?.sender_auth || result?.sender_auth?.record?.dns) && (
                <div>
                  <p className="text-sm text-muted-foreground">
                    Sender Authentication records (add these CNAMEs too):
                  </p>
              ...
              {result?.ok === false && (
                <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm">
                  We created the sender authentication config in SendGrid. Add the DNS records
                  above, wait for propagation, then click "Create Parse Route" again.
                </div>
              )}
            </div>
          )}
          ...
          {result?.ok && (
            <div className="space-y-4">
              <div className="text-sm text-muted-foreground">
                Next: create an inbound address and map it to an inbox.
              </div>
              ...
              <div className="flex items-center justify-between rounded-md border border-border/50 p-3 bg-background/50 text-sm">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={() =>
                      navigator.clipboard.writeText(
                        `${alias}@${form.getValues().parse_subdomain}.${form.getValues().domain}`,
                      )
                    }
                  >
                    Copy
                  ...
                  <Button
                    type="button"
                    disabled={!alias.trim() || !selectedInbox}
              ...
              {route && (
                <div className="space-y-4">
                  <div className="rounded-md border border-border/50 p-3 text-sm bg-background/50">
                    ...
                    Then set your public address to forward here.
                  </div>
                  <GoogleGroupSetup
                    alias={alias}
