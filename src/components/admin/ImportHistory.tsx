...
export const ImportHistory = ({ onResume }: ImportHistoryProps = {}) => {
  ...
  useEffect(() => {
    const hasActiveJob = jobs.some((job) => job.status === "running" || job.status === "paused")
    ...
    switch (status) {
      ...
      case "running":
        return (
          <Badge
            variant="default"
            className="bg-blue-500/10 text-blue-700 border-blue-500/20 animate-pulse"
          >
                  ...
                  <div className="pt-2 border-t border-border/50">
                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
                      {activeJob.metadata.currentMailboxName && (
                        <span>
                          <strong className="text-foreground">Mailbox:</strong>{" "}
                          {activeJob.metadata.currentMailboxName}
                        </span>
                      )}
                      ...
                      {activeJob.metadata.continuationCount !== undefined && (
                        <span>
                          <strong className="text-foreground">Continuation:</strong>{" "}
                          {activeJob.metadata.continuationCount} / 150
                        </span>
                      )}
                      {activeJob.metadata.completedMailboxIds && (
                        <span>
                          <strong className="text-foreground">Completed Mailboxes:</strong>{" "}
                          {activeJob.metadata.completedMailboxIds.length} /{" "}
                          {activeJob.total_mailboxes}
                        </span>
                      )}
                    </div>
                  </div>
