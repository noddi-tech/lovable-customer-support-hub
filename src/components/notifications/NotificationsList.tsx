...
import { CheckCheck, ExternalLink, Eye, EyeOff, MessageSquare, Trash2 } from "lucide-react"
  ...
  const deleteNotificationMutation = useMutation({
    ...
    onSuccess: (_, notificationId) => {
      ...
      queryClient.setQueryData(["notifications"], (old: Notification[] | undefined) => {
        ...
        console.log(
          "Optimistic update: removed notification",
          notificationId,
          "new length:",
          filtered.length,
        )
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {unreadCount > 0
                ? unreadCount === 1
                  ? t("dashboard.notifications.unreadNotification", { count: unreadCount })
                  : t("dashboard.notifications.unreadNotifications", { count: unreadCount })
                : t("dashboard.notifications.allCaughtUp")}
            </p>
          </div>
          {unreadCount > 0 && (
            <Button
