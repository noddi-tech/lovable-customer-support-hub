  ...
  return (
    <div className={cn(collapsed ? "flex justify-center" : "px-2", className)}>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            className={cn("w-full justify-start gap-2", collapsed && "w-8 h-8 p-0 justify-center")}
            disabled={isUpdating}
          >
      ...
      {!collapsed && otherAgents.length === 0 && !agentsLoading && (
        <p className="text-xs text-muted-foreground mt-1 px-2">No other agents online</p>
      )}
    </div>
  )
}
