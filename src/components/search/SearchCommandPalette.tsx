...
export const SearchCommandPalette: React.FC<SearchCommandPaletteProps> = ({
  ...
  const convResults = searchEnabled
    ...
    : [];
  const custResults = searchEnabled
    ? (customers.data?.pages?.[0]?.results ?? []).slice(0, 5)
    : [];
  const msgResults = searchEnabled
    ? (messages.data?.pages?.[0]?.results ?? []).slice(0, 5)
    : [];
              ...
              <CommandGroup heading="Conversations">
                {convResults.map((r) => (
                  <CommandItem
                    ...
                    value={`conv-${r.id}`}
                    onSelect={() =>
                      select(`/interactions/text/open?conversationId=${r.id}`)
                    }
                  >
