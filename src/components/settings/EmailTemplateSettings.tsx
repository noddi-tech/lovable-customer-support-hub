...
export function EmailTemplateSettings() {
  ...
  useEffect(() => {
    if (existingTemplate) {
      ...
      setTemplate({
        ...defaultTemplate,
        inbox_id: selectedInboxId === "default" ? null : selectedInboxId,
      })
    }
  ...
  const saveTemplateMutation = useMutation({
    mutationFn: async (templateData: EmailTemplate) => {
      ...
      const inboxId =
        activeTemplateType === "conversation_reply" && selectedInboxId !== "default"
          ? selectedInboxId
          : null
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-3 p-4 rounded-lg border border-primary/20 bg-primary/5">
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <p>
            These settings control how your outbound emails look to customers. Changes here affect
            the header, body, signature, and footer of all email replies sent by your team. Chat
            ...
            <TabsContent value={activeTemplateType} className="space-y-6 mt-6">
              ...
              <div className="border rounded-lg p-4 bg-muted/50">
                ...
                <div className="border rounded bg-background max-w-md mx-auto">
                  ...
                  >
                    <div style={{ lineHeight: "1.6" }}>{t("emailTemplate.yourReply")}</div>
              ...
              <div className="space-y-4">
                ...
                {!template.include_agent_name && (
                  <p className="text-sm text-muted-foreground bg-muted/50 px-3 py-2 rounded">
                    When disabled, the{" "}
                    <code className="text-xs bg-muted px-1 rounded">{"{{agent_name}}"}</code>{" "}
