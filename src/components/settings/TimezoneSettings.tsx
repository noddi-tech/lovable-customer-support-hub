...
const TIMEZONE_REGIONS = {
  ...
  Other: [{ value: "UTC", label: "UTC (Coordinated Universal Time)", offset: "UTC+0" }],
}
...
export function TimezoneSettings() {
      ...
      try {
        ...
        if (profile?.timezone) {
          ...
          try {
            ...
            detectedFormat =
              resolved.hourCycle === "h23" || resolved.hourCycle === "h24" ? "24h" : "12h"
          } catch {}
  ...
  const handleTimezoneChange = async (timezone: string) => {
    ...
    setIsUpdating(true)
    try {
      const { error } = await supabase.from("profiles").update({ timezone }).eq("user_id", user.id)
      ...
      <CardContent className="space-y-6">
                    ...
                    <SelectItem key={timezone.value} value={timezone.value}>
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {timezone.offset} •{" "}
                          {getCurrentTime(timezone.value, currentTimeFormat === "24h")}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
            ...
            <SelectContent>
              <SelectItem value="12h">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">Example: 2:30 PM</span>
                </div>
              </SelectItem>
              <SelectItem value="24h">
                <div className="flex flex-col">
                  <span>24-hour</span>
                  <span className="text-xs text-muted-foreground">Example: 14:30</span>
                </div>
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        {detectedTimezone && detectedTimezone !== currentTimezone && (
          <div className="p-3 rounded-lg border border-border bg-card">
            <div className="text-sm font-medium mb-1">{t("settings.timezone.detected")}</div>
