...
import { extractMentionNames, useNoteMutations } from "@/hooks/useNoteMutations"
...
export const InlineNoteEditor = ({
  ...
  useEffect(() => {
    ...
    for (const name of names) {
      const match = members.find((m) => m.full_name?.toLowerCase() === name.toLowerCase())
  ...
  }, [members.length, initialContent, members.find])
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className="flex items-center justify-end gap-2">
        <Button variant="ghost" size="sm" onClick={onCancel} disabled={isSaving} className="h-7">
          <X className="h-3.5 w-3.5 mr-1" />
          Cancel
        </Button>
        <Button size="sm" onClick={handleSave} disabled={!value.trim() || isSaving} className="h-7">
          {isSaving ? (
