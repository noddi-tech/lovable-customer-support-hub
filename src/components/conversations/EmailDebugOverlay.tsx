  ...
  useEffect(() => {
    ...
    const diagnosticData: Diagnostics = {
      ...
      blockquotes: {
        ...
        background: blockquotes[0]
          ? window.getComputedStyle(blockquotes[0]).backgroundColor
          : "N/A",
      ...
      <div className="space-y-3 text-yellow-900 dark:text-yellow-100">
        ...
        <div>
          ...
          <div className="ml-3 space-y-1 text-[10px]">
            <div>
              Classes:{" "}
              <code className="bg-yellow-200/50 px-1 rounded">
                {diagnostics.container.className}
              </code>
            </div>
            <div className="flex gap-4">
              <span>
                white-space: <strong>{diagnostics.container.whiteSpace}</strong>
              </span>
              <span>
                line-height: <strong>{diagnostics.container.lineHeight}</strong>
              </span>
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="font-semibold text-xs mb-1">
            📝 Paragraphs ({diagnostics.paragraphs.count})
          </div>
          <div className="ml-3 space-y-1 text-[10px]">
            <div>
              First &lt;p&gt; margin-bottom:{" "}
              <strong className="text-red-700 dark:text-red-300">
                {diagnostics.paragraphs.firstMarginBottom}
              </strong>
            </div>
            <div>
              First &lt;p&gt; line-height: <strong>{diagnostics.paragraphs.firstLineHeight}</strong>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="ml-3 text-[10px]">
            {diagnostics.links.count > 0 ? (
              <div>
                text-decoration: <strong>{diagnostics.links.textDecoration}</strong>
              </div>
            ) : (
        ...
        <div>
          <div className="font-semibold text-xs mb-1">
            💬 Blockquotes ({diagnostics.blockquotes.count})
          </div>
          <div className="ml-3 text-[10px]">
            {diagnostics.blockquotes.count > 0 ? (
              <>
                <div>
                  border-left: <strong>{diagnostics.blockquotes.borderLeft}</strong>
                </div>
                <div>
                  margin: <strong>{diagnostics.blockquotes.margin}</strong>
                </div>
              </>
            ) : (
