  ...
  return (
    <Tooltip delayDuration={200}>
      ...
      >
        ...
        <p>
          When a customer email lands in this inbox, the assistant writes a proposed answer using
          the conversation history, the customer's Noddi account and your knowledge base.
        </p>
        <p>
          It is internal only: the customer never sees it and nothing is sent automatically. You can
          Send it as-is, edit it first, or discard it — and every choice trains the assistant on
          your tone.
        </p>
        <p className="text-muted-foreground">
          Turn it on or off per inbox under Settings → Inboxes → “AI draft replies”. Tone, knowledge
          sources and autonomy thresholds live under Knowledge.
        </p>
      </TooltipContent>
    </Tooltip>
  )
...
function getMessageStyle(authorType: "agent" | "customer" | "system" | "ai_draft" = "customer") {
  if (authorType === "agent") {
    return {
      border: "border-l-2 border-primary/50",
  ...
  if (authorType === "customer") {
    return {
      ...
      bg: "bg-transparent hover:bg-muted/20",
...
const MessageCardComponent = ({
  ...
  useEffect(() => {
    ...
    logger.debug(
      `MessageCard render #${renderCount.current}`,
      {
  ...
  }, [defaultCollapsed, disableAnimation, isCollapsed])
  ...
  const isInternalNote =
    !isAiDraft && (message.isInternalNote || message.originalMessage?.is_internal === true)
  ...
  return (
    <article
      ...
      className={cn(
        ...
        // Continuous chat-style: light left accent, no heavy card chrome
        isInternalNote ? noteStyle?.bg : messageStyle?.bg,
        isInternalNote ? noteStyle?.border : messageStyle?.border,
        isInternalNote
          ? cn("border", noteStyle?.cardBorder)
          : "border-0 border-b border-border/40 last:border-b-0",
        disableAnimation && "disable-animation",
        effectiveCollapsed ? "py-0" : "py-1.5",
        isNewestMessage && "ring-1 ring-primary/25",
        isPinned && isInternalNote && "ring-2 ring-yellow-400/50 ring-offset-1",
      )}
      aria-label={
        isInternalNote
          ? `Internal note from ${display}`
          : `${isAgent ? "Agent" : "Customer"} message from ${display}`
      }
    >
      {/* Card Header - improved spacing */}
      <div className={cn("px-2 md:px-3", effectiveCollapsed ? "py-1.5" : "py-2")}>
        <div
          className={cn("flex", effectiveCollapsed ? "items-center gap-3" : "items-start gap-4")}
        >
          {/* Avatar */}
          <Avatar
            className={cn(
          ...
          <div className="min-w-0 flex-1">
            ...
            >
                  ...
                  const nameEl = (
                    <span
                      className={cn(
                        ...
                        effectiveCollapsed
                          ? "text-xs leading-none max-w-[180px]"
                          : "text-sm max-w-[260px]",
                      )}
                    >
                      {isAiDraft
                        ? "AI Draft"
                        : shortName(message.from.name) ||
              ...
              {!isInternalNote &&
                ...
                (() => {
                  const badgeEl = (
                    <Badge
                      className={cn(
                        "text-[10px] px-1.5 py-0 h-4 shrink-0 gap-1 font-medium",
                        isAiDraft && "cursor-help",
                        messageStyle.labelBadge,
                      )}
                    >
              ...
              {/* Recipient kept in title only — avoids email-client chrome in the thread */}
          ...
          <div className="flex items-center space-x-1 shrink-0">
            ...
            {!isAiDraft && (
              <div className="opacity-100 md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100 transition-opacity">
                <DropdownMenu>
          ...
          {isAiDraft && (
            <div className="flex items-center gap-2 mt-4 pt-3 border-t border-emerald-200 dark:border-emerald-800">
              ...
              <Button
                ...
                className="gap-1.5"
                onClick={() =>
                  onEditDraft?.(message.id, message.originalMessage?.content || message.visibleBody)
                }
              >
...
export const MessageCard = memo(MessageCardComponent, (prevProps, nextProps) => {
  ...
  const shouldUpdate = !(messageMatch && collapsedMatch && animationMatch && isFirstMatch)
