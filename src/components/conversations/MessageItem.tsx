        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            <Avatar className="h-8 w-8 flex-shrink-0">
              <AvatarFallback
                className={cn(isFromCustomer ? "bg-primary text-primary-foreground" : "bg-muted")}
              >
                {(() => {
                ...
                {message.to.length > 0 && (
                  <>
                    ...
                    {message.to
                      ...
                      .join(", ")}
                    {message.to.length > 3 ? ` +${message.to.length - 3}` : ""}
                  </>
                )}
                {message.cc && message.cc.length > 0 && (
                  <>
                    ...
                    {message.cc
                      ...
                      .map((r) => r.name || r.email)
                      .join(", ")}
                    {message.cc.length > 2 ? ` +${message.cc.length - 2}` : ""}
                  </>
                )}
        ...
        {hasQuoted && (
          <div className="mt-3 pt-2 border-t border-border">
            ...
            >
              ...
              {showQuoted
                ? t("conversation.hideQuoted", "Skjul sitert historikk")
                : t("conversation.showQuoted", "Vis sitert historikk")}
            </Button>
            {showQuoted && (
