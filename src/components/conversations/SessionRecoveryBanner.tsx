...
export function SessionRecoveryBanner({
  ...
  const { healthState, isRecovering, aggressiveRecovery, nuclearSessionReset } =
  ...
  const handleNuclearReset = async () => {
    const confirmed = window.confirm(
      "This will log you out and redirect to the login page. Continue?",
    )
    if (confirmed) {
  ...
  const shouldShow =
    show &&
    !isDismissed &&
    (showAlways || (!healthState.isHealthy && healthState.consecutiveFailures >= 2))
      ...
      <AlertDescription className="flex items-center justify-between">
        <div className="flex-1">
          ...
          <span className="text-sm">
            Conversations may not load properly. {healthState.consecutiveFailures} consecutive
            failures.
          </span>
        </div>
