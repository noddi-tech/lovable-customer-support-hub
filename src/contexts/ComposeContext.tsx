...
export function isDraftEmpty(d: ComposeDraft): boolean {
  return (
    !d.to.trim() && !d.toName.trim() && !d.bulkEmails.trim() && !d.subject.trim() && !d.body.trim()
  )
}
...
export const ComposeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const reopenDraft = useCallback((id: string) => {
    setDrafts((prev) => prev.map((d) => (d.id === id ? { ...d, open: true, minimized: false } : d)))
  }, [])
  ...
  const value = useMemo<ComposeContextValue>(() => {
    ...
    const savedDrafts = drafts.filter((d) => !d.open).sort((a, b) => b.updatedAt - a.updatedAt)
    return {
      drafts,
      openWindows,
      savedDrafts,
