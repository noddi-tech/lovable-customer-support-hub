...
import { type MentionContext, useMentionNotifications } from "@/hooks/useMentionNotifications"
...
export const useNoteMutations = () => {
  ...
  const updateNote = useCallback(
    async ({
      ...
      noteDebug(
        "update_mutation_started",
        { messageId, mentionCount: mentionedUserIds.length },
        "useNoteMutations",
      )
      try {
        ...
        noteDebug(
          "update_mutation_finished",
          { messageId, ok: false, error: err?.message },
          "useNoteMutations",
        )
