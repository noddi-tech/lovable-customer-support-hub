...
export const useNetworkErrorHandler = (options: NetworkErrorHandlerOptions = {}) => {
    ...
    async <T>(
      ...
      for (let attempt = 1; attempt <= retryAttempts; attempt++) {
        try {
          ...
          // Log the error
          logger.warn(
            `Network operation failed (attempt ${attempt}/${retryAttempts})`,
            {
              error: error.message,
              operationId,
              attempt,
      ...
      if (!shouldSuppressError(lastError)) {
        logger.error(
          `Network operation failed after ${retryAttempts} attempts`,
          {
  ...
  const createResilientQuery = useCallback(
    <T>(queryFn: () => Promise<T>, queryId: string, fallback?: T) => {
