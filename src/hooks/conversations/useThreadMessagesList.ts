  ...
  return messages.filter((m) => {
    ...
    for (const agentMsg of agentTexts) {
      ...
      if (inboundText.includes(searchKey)) {
        logger.debug(
          "Filtering forwarding echo (near-identical inbound copy of agent reply)",
          {
            messageId: m.id,
...
export function useThreadMessagesList(
  ...
  const messages = useMemo(() => {
    ...
    logger.debug(
      "useMemo RUNNING - processing messages",
      {
    ...
    const expandedMessages =
      ENABLE_QUOTED_EXTRACTION && context
        ? expandQuotedMessagesToCards(echoFiltered, context)
        : echoFiltered

    logger.debug(
      "Thread expansion stats",
      {
