  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const seedSel = supabase
        .from("messages")
        .select("id, email_headers, email_subject, email_status, created_at, sender_type, sender_id, content, content_type, is_internal, attachments, metadata, external_id, conversation:conversations(customer:customers(email, full_name), email_account:email_accounts(email_address), inbox:inboxes(sender_display_name, inbound_routes(group_email)))")
      ...
      let base = supabase
        .from("messages")
        .select("id, email_message_id, email_thread_id, email_headers, email_subject, email_status, created_at, sender_type, sender_id, content, content_type, is_internal, is_pinned, attachments, metadata, external_id, conversation:conversations(customer:customers(email, full_name), email_account:email_accounts(email_address), inbox:inboxes(sender_display_name, inbound_routes(group_email)))")
