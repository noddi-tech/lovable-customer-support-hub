...
import { extractMessageIds, normalizeSubject } from "@/lib/emailThreading"
import {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const seedSel = supabase
        .from("messages")
        .select(
          "id, email_headers, email_subject, email_status, created_at, sender_type, sender_id, content, content_type, is_internal, attachments, metadata, external_id, conversation:conversations(customer:customers(email, full_name), email_account:email_accounts(email_address), inbox:inboxes(sender_display_name, inbound_routes(group_email)))",
        )
      ...
      let base = supabase
        .from("messages")
        .select(
          "id, email_message_id, email_thread_id, email_headers, email_subject, email_status, created_at, sender_type, sender_id, content, content_type, is_internal, is_pinned, attachments, metadata, external_id, conversation:conversations(customer:customers(email, full_name), email_account:email_accounts(email_address), inbox:inboxes(sender_display_name, inbound_routes(group_email)))",
        )
      ...
      const inboxData = conversationData?.inbox as
        | { sender_display_name?: string; inbound_routes?: { group_email?: string }[] }
        | undefined
      ...
      if (!pageParam && rows && rows.length < 10) {
        confidence = "low"
        logger.debug(
          "Low confidence - small initial page",
          {
            conversationIds: ids,
            isThreaded: ids.length > 1,
