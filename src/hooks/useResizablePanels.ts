...
import { useIsDesktop, useIsMobile, useIsTablet } from "./use-responsive"
...
export const useResizablePanels = ({
  ...
  const getViewportAwareConstraints = useCallback(() => {
    ...
    Object.keys(defaultSizes).forEach((panelId) => {
      if (viewportWidth >= 1440) {
        ...
        constraints[panelId] = Math.max(
          constraints[panelId] || 0,
          Math.min(25, (350 / viewportWidth) * 100),
        )
        maxConstraints[panelId] = Math.min(
          maxConstraints[panelId] || 100,
          Math.min(30, (500 / viewportWidth) * 100),
        )
      } else if (viewportWidth >= 1024) {
        // Desktop: Max 35% width, min 300px
        constraints[panelId] = Math.max(
          constraints[panelId] || 0,
          Math.min(25, (300 / viewportWidth) * 100),
        )
        ...
        constraints[panelId] = Math.max(
          constraints[panelId] || 0,
          Math.min(30, (250 / viewportWidth) * 100),
        )
  ...
  }, [viewportAware, getViewportAwareConstraints, panelSizes])
