...
import { hasIamAuthorizationGraph, isNetworkSuperuser } from "@/lib/auth-access"
...
import { type OrganizationMembership, useOrganizationStore } from "@/stores/organizationStore"
...
export const useAuth = () => {
  ...
  const networkSuperuser = useMemo(
    () =>
      isNetworkSuperuser(
        navioClaims,
        [...localRoles, ...userRoles.map((role) => ({ role, organization_id: null }))],
        googleEmployee,
      ),
  ...
  const allowedLocalOrgIds = useMemo(() => getAllowedLocalOrgIds(effectiveScope), [effectiveScope])
  ...
  useEffect(() => {
    ...
    if (hasNavioGraph && !effectiveScope.isSuperuser) {
      ...
      if (
        ...
        !allowed.has(currentOrganizationId)
      ) {
        const next =
          memberships.find((m) => m.is_default)?.organization_id || [...allowed][0] || null
        if (next) {
        ...
        const next =
          memberships.find((m) => m.is_default)?.organization_id || memberships[0]?.organization_id
