...
const globalStateListeners = new Set<(state: any) => void>()
...
export const useRealtimeConnectionManager = () => {
  ...
  const attemptReconnection = useCallback(() => {
    ...
    updateGlobalState((prev) => {
      ...
      console.log(
        `🔄 Attempting reconnection in ${delay}ms (attempt ${currentAttempts + 1}/${config.maxRetries})`,
      )
  ...
  const createManagedSubscription = useCallback(
    (channelName: string, subscriptionFn: (channel: any) => any, dependencies: any[] = []) => {
      const createSubscription = () => {
        console.log(
          `[RealtimeConnectionManager] 🔌 Creating managed subscription: ${channelName}`,
          {
