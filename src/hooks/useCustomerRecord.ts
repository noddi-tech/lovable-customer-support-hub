  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.from("customer_summaries") as any)
        .select(
          sel(
            "id, summary_text, total_conversations, first_seen_at, last_seen_at, sentiment_trend",
          ),
        )
  ...
  const dbQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.from("customer_notes") as any)
        .select(
          sel(
            "id, content, is_pinned, created_by_id, created_at, updated_at, author:profiles(id, full_name)",
          ),
        )
        .eq("customer_id", customerId)
...
export function useAddCustomerIdentity(customerId?: string | null) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      type: CustomerIdentity["identity_type"]
      value: string
    }) => {
      if (!customerId || !profile?.organization_id) throw new Error("Missing customer")
