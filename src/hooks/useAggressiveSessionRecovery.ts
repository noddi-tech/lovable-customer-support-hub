...
export function useAggressiveSessionRecovery() {
  ...
  const performHealthCheck = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const { auth_uid, session_valid, organization_id, profile_exists } = sessionContext[0]
  ...
  const nuclearSessionReset = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log(
        "💾 Preserved Aircall keys:",
        Object.keys(aircallData).filter((k) => aircallData[k]),
      )
      ...
      setTimeout(() => {
        console.log("💥 [SessionRecovery] Nuclear reset complete - dispatching navigation event")
  ...
  const aggressiveRecovery = useCallback(async (): Promise<boolean> => {
    ...
    if (circuitBreakerUntil && Date.now() < circuitBreakerUntil) {
      console.log(
        "⚡ Circuit breaker active, skipping recovery until",
        new Date(circuitBreakerUntil),
      )
      return false
    }
    ...
    try {
      ...
      if (newFailures >= 8) {
        ...
        console.log(
          `⚠️ Recovery failed but not enough failures for nuclear reset (${newFailures}/8)`,
        )
        setIsRecovering(false)
        return false
      }
  ...
  useEffect(() => {
    ...
    if (IS_DEV_MODE) {
      console.log(
        "⚠️ Automatic session health checks disabled in dev mode (manual checks still work)",
      )
      return
    }
    ...
    const getCheckInterval = (failures: number): number => {
      ...
      const backoffMultiplier = Math.min(1.5 ** failures, 4)
