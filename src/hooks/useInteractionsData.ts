...
export function useInboxCounts(inboxId: InboxId) {
  return useQuery({
    ...
    refetchInterval: 30 * 1000, // Poll every 30s as fallback for missed realtime events
  })
}
    ...
    queryFn: async () => {
      ...
      try {
        ...
        logger.debug(
          ...
          { count: conversations.length, inboxId },
          "Interactions",
        )
        ...
        if (
          ...
          if (newSession) {
            ...
            try {
              const retryConversations = await listConversations({
                inboxId,
                status,
                q,
                currentUserProfileId,
...
export function useReply(
  ...
  return useMutation({
    ...
    onMutate: async (body: string) => {
      ...
      const previousThread = queryClient.getQueryData<ConversationThread>([
        "thread",
        conversationId,
      ])
...
export function useInfiniteConversations({
  ...
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => listConversations({ inboxId, status, q, page: pageParam }),
    initialPageParam: 0,
