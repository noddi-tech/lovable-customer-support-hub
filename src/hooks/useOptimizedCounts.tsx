...
export const useOptimizedCounts = (selectedInboxId?: string): OptimizedCounts => {
        ...
        try {
          if (selectedInboxId) {
            ...
            return {
              ...
              inboxes: Array.isArray(result.inboxes_data)
                ? sortInboxesByName(
                    result.inboxes_data as Array<{
                      id: string
                      name: string
                      color: string
                      conversation_count: number
          ...
          if (
            networkError.message?.includes("ERR_BLOCKED_BY_CLIENT") ||
  ...
  return {
    conversations: user
      ? countsQuery.data?.conversations || defaultCounts.conversations
      : defaultCounts.conversations,
    channels: user ? countsQuery.data?.channels || defaultCounts.channels : defaultCounts.channels,
    notifications: user
      ? countsQuery.data?.notifications || defaultCounts.notifications
      : defaultCounts.notifications,
    ...
    error:
      !user && !authLoading ? "Please log in to view data" : countsQuery.error?.message || null,
    prefetchData,
