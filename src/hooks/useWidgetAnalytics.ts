...
export function useWidgetAnalytics({ widgetId, days = 30 }: UseWidgetAnalyticsProps) {
  return useQuery({
    ...
    queryFn: async (): Promise<WidgetAnalytics> => {
      ...
      const totalSessions =
        widgetOpensCount > 0 ? widgetOpensCount : currentChatCount + contactFormCount

      // Chat completion rate
      const completedChats = chatSessions?.filter((s) => s.status === "ended").length || 0
      const chatCompletionRate =
        currentChatCount > 0 ? Math.round((completedChats / currentChatCount) * 100) : 0
      ...
      const { avgResponseTimeMinutes, avgChatDurationMinutes } = calculateTimeMetrics(
        chatSessions || [],
      )
...
function calculateTopPageUrls(sessions: any[]): { url: string; count: number }[] {
  ...
  sessions.forEach((session) => {
    ...
    const simplifiedUrl =
      url === "Unknown" ? url : url.replace(/^https?:\/\/[^/]+/, "").split("?")[0] || "/"
