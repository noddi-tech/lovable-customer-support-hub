...
export function useOversiktRealtime(): { connected: boolean } {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "applications", filter: orgFilter },
        invalidate,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "applicants", filter: orgFilter },
        invalidate,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "recruitment_followups", filter: orgFilter },
        invalidate,
      )
      .on(
        "postgres_changes",
