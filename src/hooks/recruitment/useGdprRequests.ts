...
export function useInitiateGdprExport() {
  ...
  return useMutation({
    mutationFn: async (vars: { applicant_id: string; reason?: string }) => {
      const { data, error } = await supabase.functions.invoke("initiate-gdpr-export", {
        body: vars,
      })
...
export function useInitiateGdprErasure() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (vars: { applicant_id: string; confirm: boolean; reason?: string }) => {
      const { data, error } = await supabase.functions.invoke("initiate-gdpr-erasure", {
        body: vars,
      })
