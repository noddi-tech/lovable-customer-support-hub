...
export const useCallAnalytics = (dateRange?: DateRange) => {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const agentStats = Object.values(agentEmailMap)
        .map((agent) => {
          return {
            ...
            answerRate:
              agent.calls.length > 0
                ? Math.round((agent.answered.length / agent.calls.length) * 100)
                : 0,
          }
        })
      ...
      return {
        metrics: {
          ...
          callsTrend: calculatePercentageChange(
            currentMetrics.totalCalls,
            previousMetrics.totalCalls,
          ),
          durationTrend: calculatePercentageChange(
            currentMetrics.avgDuration,
            previousMetrics.avgDuration,
          ),
          answerRateTrend: calculatePercentageChange(
            currentMetrics.answerRate,
            previousMetrics.answerRate,
          ),
          missedTrend: calculatePercentageChange(
            currentMetrics.missedCalls,
            previousMetrics.missedCalls,
          ),
        },
