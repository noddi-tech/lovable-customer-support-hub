      ...
      if (type === "conversations") {
        ...
        if (customerIds.length > 0) {
          queryBuilder = queryBuilder.or(
            `subject.ilike.%${safeQuery}%,preview_text.ilike.%${safeQuery}%,customer_id.in.(${customerIds.join(",")})`,
          )
        } else {
          queryBuilder = queryBuilder.or(
            `subject.ilike.%${safeQuery}%,preview_text.ilike.%${safeQuery}%`,
          )
        }
...
export const useGlobalSearchCounts = ({
  query,
  filters = {},
}: {
  query: string
    ...
    queryFn: async () => {
      ...
      const [conversationsResult, customersResult, messagesResult] = await Promise.all([
        ...
        (async () => {
          ...
          let qb = supabase.from("conversations").select("id", { count: "exact", head: true })
