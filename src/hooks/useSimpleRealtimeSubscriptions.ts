...
const activeChannels = new Map<
  string,
  { channel: any; refCount: number; status: ConnectionStatus }
>()
...
export const useSimpleRealtimeSubscriptions = (
  ...
  useEffect(() => {
    ...
    const setupSubscription = () => {
      ...
      logger.debug(
        "Creating new realtime subscription",
        {
          channelName,
          channelKey,
          tables: currentConfigs.map((c) => c.table),
  ...
  useEffect(() => {
    if (connectionStatus === "error" && enabled) {
      logger.debug(
        "Starting auto-retry interval (10s)",
      ...
      autoRetryIntervalRef.current = setInterval(() => {
        logger.info(
          "Auto-retry: Attempting reconnection...",
          { channelKey: channelKeyRef.current },
          "Realtime",
        )
