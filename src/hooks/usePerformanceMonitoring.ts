...
export const usePerformanceMonitoring = (options: PerformanceMonitoringOptions = {}) => {
  ...
  const checkReRender = useCallback(
    (props?: any, state?: any) => {
      ...
      const propsChanged =
        lastPropsRef.current && JSON.stringify(props) !== JSON.stringify(lastPropsRef.current)
      const stateChanged =
  ...
  const estimateComponentSize = useCallback(
    (props?: any, state?: any) => {
      try {
        ...
        if (enableLogging && (propsSize > 1024 || stateSize > 1024)) {
          // 1KB threshold
          logger.warn(
            "Large component data detected",
            {
              componentName,
