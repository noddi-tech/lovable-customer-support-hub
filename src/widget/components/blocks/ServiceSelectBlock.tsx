        ...
        for (const car of cars) {
          for (const item of car.sales_items || []) {
            salesItems.push({
              ...
              price:
                item.gross_price?.amount != null
                  ? item.gross_price.amount
                  : item.unit_price != null
                    ? item.unit_price
                    : item.price,
              category_type: item.booking_category_type || "",
              category_name:
                CATEGORY_LABELS[item.booking_category_type] || item.booking_category_type || "",
            })
          }
        }
  ...
  <div className="rounded-md bg-white dark:bg-background border p-2">
    ...
    <div className="grid gap-1">
      <div className="border rounded-md px-2 py-1.5 text-[9px] flex justify-between">
        <span>Dekkskift</span>
        <span className="font-semibold">699 kr</span>
      </div>
      <div className="border rounded-md px-2 py-1.5 text-[9px] flex justify-between">
        <span>Dekkhotell ink. lagring</span>
        <span className="font-semibold">1 999 kr</span>
      </div>
    </div>
  </div>
)
...
registerBlock({
  ...
  parseContent: (inner) => {
    ...
    return { address_id: Number.isNaN(num) ? "" : num, license_plate: "" }
  },
