...
const BookingEditConfirmBlock: React.FC<BlockComponentProps> = ({
        ...
        if (!payload.delivery_window_start || !payload.delivery_window_end) {
          for (let i = 0; i < localStorage.length; i++) {
            ...
            try {
              ...
              if (
                val.delivery_window_id === data.changes.delivery_window_id ||
      ...
      if (!resp.ok || respData.error) {
        if (resp.status >= 500) {
          ...
          if (details.includes("service_department_area_not_found")) {
            setError(
              "Dette tidspunktet er dessverre ikke tilgjengelig for din adresse. Vennligst velg et annet tidspunkt.",
            )
          } else {
  ...
  if (changes.address)
    rows.push({ label: "📍 Adresse", old: changes.old_address || "—", new_val: changes.address })
  ...
  const derivedDate =
    changes.date ||
    (changes.delivery_window_start ? formatOsloDate(changes.delivery_window_start) : "")
  if (derivedDate) {
  ...
  if (changes.service)
    rows.push({ label: "🛠️ Tjeneste", old: changes.old_service || "—", new_val: changes.service })
          ...
          rows.map((r, i) => (
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <span
                  style={{ textDecoration: "line-through", color: "#9ca3af", fontSize: "12px" }}
                >
                  {r.old}
                </span>
  ...
  <div className="rounded-md bg-white dark:bg-background border p-2">
    ...
    <div className="space-y-0.5 text-[8px] mb-1.5">
      <div className="flex justify-between">
        ...
        <span>
          <s className="text-muted-foreground">08:00</s> → <b>14:00</b>
        </span>
      </div>
    </div>
    <div className="flex gap-1">
      <div className="flex-1 bg-blue-500 text-white rounded-md py-0.5 text-center text-[8px] font-semibold">
        ✓ Confirm
      </div>
