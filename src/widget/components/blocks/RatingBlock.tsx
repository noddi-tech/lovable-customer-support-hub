...
const RatingBlock: React.FC<BlockComponentProps> = ({
  ...
  const selected = isUsed
    ? parseInt(localStorage.getItem(`noddi_action_${blockKey}`) || "0", 10)
    : 0
      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          style={{
            ...
            transform: !isUsed && hover === star ? "scale(1.2)" : "scale(1)",
          }}
        >
          <svg
            width="22"
            ...
            stroke={(hover || selected) >= star ? "#facc15" : "#d1d5db"}
            strokeWidth="2"
            strokeLinecap="round"
