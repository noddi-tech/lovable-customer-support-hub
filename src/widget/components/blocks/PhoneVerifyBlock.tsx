...
const PhoneVerifyBlock: React.FC<BlockComponentProps> = ({
  ...
  const handleVerifyPin = async (pinOverride?: string) => {
    ...
    const result = await verifyPhonePin(
      widgetKey,
      phoneInput.trim(),
      pin,
      ...
      <div className="noddi-ai-phone-prompt" style={{ margin: "8px 0" }}>
        ...
        <div className="noddi-ai-phone-input-row">
          ...
          >
            {isSendingCode ? (
              <svg
                className="noddi-widget-spinner"
                width="18"
                height="18"
                viewBox="0 0 24 24"
                fill="none"
            ...
            {[0, 1, 2].map((i) => (
              <input
                ...
                onPaste={(e) => {
                  ...
                  setPinInput(pasted)
                  const slots = (e.target as HTMLElement)
                    .closest(".noddi-otp-container")
                    ?.querySelectorAll(".noddi-otp-slot")
                  if (slots?.[Math.min(pasted.length, 5)])
                    (slots[Math.min(pasted.length, 5)] as HTMLInputElement).focus()
                }}
              />
            ))}
          </div>
...
const PhoneVerifyPreview: React.FC<FlowPreviewProps> = () => (
  <div className="rounded-md bg-white dark:bg-background border p-2 space-y-1.5">
    ...
    <div className="flex items-center gap-1.5">
      ...
      <div className="h-6 w-6 rounded-md bg-purple-500 flex items-center justify-center text-white text-[10px] font-bold">
        →
      </div>
    </div>
    <div className="flex items-center gap-1 justify-center">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="h-5 w-4 rounded border bg-muted/30 flex items-center justify-center text-[8px] text-muted-foreground"
        >
          •
        </div>
      ))}
...
registerBlock({
    ...
    endpoints: [
      {
        ...
        description:
          "Sends an SMS with a 6-digit PIN to the provided phone number via the Noddi API.",
      },
      {
        name: "Verify PIN",
  ...
  flowMeta: {
    ...
    icon: "📱",
    description:
      "The customer will see a phone number input. After entering their number, they'll receive an SMS with a 6-digit PIN code to verify their identity.",
