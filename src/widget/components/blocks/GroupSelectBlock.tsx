...
import { type BlockComponentProps, registerBlock } from "./registry"
...
const GroupSelectBlock: React.FC<BlockComponentProps> = ({
  ...
  const selectedName =
    storedValue?.name || groups.find((g) => String(g.id) === selected)?.name || ""
  ...
  if (showConfirmed && selectedName) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "8px" }}>
        ...
        <span style={{ fontSize: "13px", color: "#22c55e", fontWeight: 500 }}>{selectedName}</span>
      </div>
    )
  }
