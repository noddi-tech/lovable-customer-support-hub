...
const TimeSlotBlock: React.FC<BlockComponentProps> = ({
    ...
    void (async () => {
      try {
        ...
        const postJson = (body: any) =>
          fetch(`${apiUrl}/noddi-booking-proxy`, {
            method: "POST",
          ...
          for (let i = 0; i < localStorage.length; i++) {
            ...
            try {
              ...
              if (stored.license_plate) {
                ...
                console.log(
                  "[TimeSlotBlock] Recovered license plate from localStorage:",
                  resolvedPlate,
                )
                break
              }
        ...
        if (salesItemIds.length === 0 && (resolvedPlate || resolvedCarIds.length > 0)) {
          ...
          for (let i = 0; i < localStorage.length; i++) {
            ...
            try {
              ...
              if (stored.sales_item_id && allAvailableIds.has(Number(stored.sales_item_id))) {
                ...
                console.log(
                  "[TimeSlotBlock] Recovered selected service from localStorage:",
                  selectedItemId,
                )
                break
              }
          ...
          if (selectedItemId) {
            ...
            if (firstCategory) {
              ...
              console.log(
                "[TimeSlotBlock] Filtered to category:",
                firstCategory,
                "ids:",
                salesItemIds,
              )
        ...
        if (flatWindows.length > 0) {
          console.log(
            "[TimeSlotBlock] Sample window object keys:",
            Object.keys(flatWindows[0]),
            flatWindows[0],
          )
        }
  ...
  }, [addressId, salesItemId, licensePlate, carIds])

  const handleSlotSelect = (window: any) => {
    const currentDate = sortedDates[selectedIdx]
    const windowId =
      window.pk || window.delivery_window_id || window.id || window.delivery_window?.id
    ...
    onLogEvent?.(
      "time_slot_selected",
      `${currentDate} ${formatTime(window.start_time || window.starts_at)}`,
      "success",
    )
  }
    ...
    return (
      <div className="noddi-ai-verified-badge" style={{ margin: "8px 0" }}>
        ...
        <span style={{ fontSize: "12px" }}>
          {parsed
            ? `${formatDate(parsed.date)} ${formatTime(parsed.start_time)}–${formatTime(parsed.end_time)}`
            : submitted}
        </span>
      </div>
    )
  ...
  if (sortedDates.length === 0) {
    return (
      <div style={{ margin: "8px 0", fontSize: "12px", color: "#9ca3af" }}>
        Ingen ledige tider de neste 2 ukene
      </div>
    )
  }
      ...
      >
        <button
          ...
          style={{
            background: "none",
            border: "none",
            cursor: selectedIdx === 0 || isUsed ? "default" : "pointer",
            padding: "2px 6px",
            fontSize: "16px",
            color: selectedIdx === 0 ? "#d1d5db" : primaryColor,
          }}
        >
          ‹
        </button>
        ...
        <button
          ...
          style={{
            background: "none",
            border: "none",
            cursor: selectedIdx === sortedDates.length - 1 || isUsed ? "default" : "pointer",
            padding: "2px 6px",
            fontSize: "16px",
            color: selectedIdx === sortedDates.length - 1 ? "#d1d5db" : primaryColor,
          }}
        >
          ›
        </button>
        ...
        {windows.map((w, i) => {
          ...
          return (
            <button
              ...
              onMouseEnter={(e) => {
                if (!isUsed) (e.currentTarget as HTMLElement).style.borderColor = primaryColor
              }}
              onMouseLeave={(e) => {
            ...
            >
              ...
              {price !== undefined && (
                <div style={{ color: "#6b7280", fontSize: "11px" }}>{price} kr</div>
              )}
            </button>
          )
        })}
    ...
    endpoints: [
      ...
      },
      {
        ...
        requestBody: {
          ...
          from_date: "string",
          to_date: "string",
          selected_sales_item_ids: "number[]",
        },
