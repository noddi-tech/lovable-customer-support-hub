...
const AddressSearchBlock: React.FC<BlockComponentProps> = ({
  ...
  const handleSelect = useCallback(
    async (suggestion: AddressSuggestion) => {
      ...
      try {
        ...
        const label =
          `${address.street_name} ${address.street_number || ""}, ${address.city}`.trim()
  ...
  const handleStoredAddressSelect = useCallback(
    (addr: StoredAddress) => {
    ...
    return (
      <div className="noddi-ai-verified-badge" style={{ margin: "8px 0" }}>
        ...
        <span style={{ fontSize: "12px" }}>
          {addr}
          {inArea === false ? " (utenfor leveringsområde)" : ""}
        </span>
      </div>
    )
  }
  ...
  if (result) {
    const label =
      selected || `${result.street_name} ${result.street_number || ""}, ${result.city}`.trim()
    return (
      <div
        style={{
          margin: "8px 0",
      ...
      >
        <div
          style={{
            ...
            gap: "6px",
            fontWeight: 600,
            color: result.is_in_delivery_area ? "#15803d" : "#92400e",
          }}
        >
    ...
    <div ref={containerRef} style={{ margin: "8px 0", position: "relative" }}>
      ...
      <div
        style={{
          ...
          gap: "6px",
          ...(isOpen
            ? { borderBottomLeftRadius: 0, borderBottomRightRadius: 0, borderColor: primaryColor }
            : {}),
        }}
      >
        <svg
          ...
          {suggestions.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                {secondary && (
                  <span style={{ fontSize: "11px", color: "#9ca3af" }}>{secondary}</span>
                )}
              </button>
            )
          })}
