...
export type MessageBlock = { type: "text"; content: string } | { type: string; [key: string]: any }
...
export function parseMessageBlocks(content: string): MessageBlock[] {
  ...
  for (const marker of MARKERS) {
    if (marker.hasClosing && marker.closingTag) {
      if (!normalized.includes(marker.closingTag) && normalized.split(marker.tag).length > 2) {
        const lastIdx = normalized.lastIndexOf(marker.tag)
        normalized =
          normalized.substring(0, lastIdx) +
          marker.closingTag +
