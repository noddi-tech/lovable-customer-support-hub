...
export const CONVERSATION_STATUS_DESCRIPTIONS: Record<string, string> = {
  open: "Active and needs attention. The conversation stays in the Open inbox, counts towards your open/unread totals and SLA timers keep running.",
  pending:
...
export const PRIORITY_DESCRIPTIONS: Record<string, string> = {
  low: "Nice to have. Sorted last, no escalation and the longest response-time target.",
  normal:
    "Default priority for everyday requests. Standard sorting and standard SLA response target.",
  high: "Important. Sorted above normal traffic, highlighted in the list and given a shorter SLA response target.",
  urgent:
...
export const REPLY_SEND_STATUS_DESCRIPTIONS: Record<string, string> = {
  ...
  open: "Sends your reply and leaves the conversation open, so it stays in the active inbox and keeps counting as work in progress.",
  pending:
