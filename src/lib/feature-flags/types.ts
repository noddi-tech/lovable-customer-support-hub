...
export function evaluateFlagWithContext<T>(
  ...
  const pick = (variant: string, reason: EvaluationReason): FlagResolution<T> => {
    if (!(variant in variants)) {
      return { value: defaultValue, reason: 'ERROR', errorCode: 'GENERAL', errorMessage: `Unknown variant "${variant}"` };
    }
  ...
  const on = flag.targeting?.onVariant || (DEFAULT_ON in variants ? DEFAULT_ON : flag.default_variant);
